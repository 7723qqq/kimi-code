...
Apply this lens only when the user asks for it explicitly (for example "review this from a single level of abstraction", "check whether this function does too much", "errors should be handled above/below, right?"). Leave general reviews and refactors to other lenses unless the user names this one.
