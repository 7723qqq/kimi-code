...
- For tui2 polish work on the `tui2/rebased` branch, run `node scripts/audit-tui2.mjs` before opening a PR — it gates on typecheck, vitest, oxlint, sherif, locale parity, and a set of lint regression rules (escape-case, consistent-type-imports, prefer-string-slice, prefer-string-replace-all, prefer-at, no-array-sort, no-immediate-mutation, no-duplicates, no-immediate-mutation, prefer-code-point, promise/always-return, import/first). Status signals (working-tree-dirty, orphan-stash-survey) are non-blocking but should be reviewed.
