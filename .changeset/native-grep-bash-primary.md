...
- Bash: a new native process-lifecycle API (`nativeBashSpawn` / `nativeBashWait` / `nativeBashKill` / `nativeBashDispose`) streams stdout/stderr in real time and kills the process tree on demand; the Bash tool spawns through it and falls back to the host spawn.
