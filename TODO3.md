...
- [ ] 将仍缺服务层 model-tool 入口的旧 Agent-bound tools 迁完：`Agent`、media upload/read、goal tools、question tools。注意：`AgentSwarm` 已由 `SwarmModeService` 注册，model-invoked `Skill` 已由 `ModelSkillTool` 注册，`TodoList` / `Cron*` / `EnterPlanMode` / `ExitPlanMode` 也已经是 service-owned tools，不应再列入“未注册 builtin”。覆盖测试：`tool.test.ts`、`turn.test.ts`、`skill-tool-manager.test.ts`。
...
- [ ] 恢复 skipped 的 resume projection 覆盖：tool-store 状态恢复已补；goal completion replay card 已补。剩余重点是从历史记录重建 turn counter、compaction replay cards、延迟 reminders、pending tool results，以及中断 tool call 的合成结果。这里应按 v1.5 canonical records 断言；旧 records 只用于 migration 输入。覆盖测试：`resume.test.ts`、`context.test.ts`、`records/index.test.ts`。
...
- [ ] 排查 `activateAgentServices()` 懒代理“假预热”：审计已完成，裸 `accessor.get(...)` 对 delayed service 不能作为完整预热屏障。已补 `IPermissionRulesService.rules` 强制构造以恢复 session approval replay；已补 `ISwarmMode.isActive` 强制构造以恢复 `AgentSwarm` service-owned tool 注册；已补 `IProfileService.data()`、`IPermissionModeService.mode`、`IPermissionService.data()` 以激活 config / permission restore handlers 和初始 permission mode；已补 `IAgentSkillService.activate` 强制构造以恢复 `skill.activate` replay handler，覆盖 `skill-tool-manager.test.ts`。restore 预热已补 `IToolStoreService.data()`、`IUserToolService.register` 和 `IGoalService.getGoal()`。后续优先核对/修复构造期有 wire handler、hook、tool、event listener 的高风险服务：`IPlanModeService`、`IMcpRuntimeService`、`IFullCompaction`、`IMicroCompactionService`、`IBackgroundService`、`IContextUsageService`、`ILoopService`。覆盖测试按对应服务单文件收口，不做一次性大改。
...
- [ ] 恢复服务层 goal replay 语义：已恢复 `goal.*` records、`forked` boundary 清空 copied goal、fork-cleared reminder，以及 goal completion replay card。剩余重点是 goal injector boundary cadence、goal-cleared reminders 的更广泛交互，以及 goal-outcome cleanup。覆盖测试：`goal.test.ts`、`injection/goal.test.ts`、`records/index.test.ts`、`resume.test.ts`。
- [ ] 恢复 turn-flow hook / event parity：`PreToolUse` / `PostToolUse` 成功路径已补，same-step dedup 的 `PostToolUse` 真实输出也已恢复。剩余重点是 `PostToolUseFailure`、`StopFailure`、`Interrupt`、provider finish-reason details、model-not-configured diagnostics、LLM request logging，以及 duplicate tool-call telemetry。覆盖测试：`tool.test.ts`、`turn.test.ts`。
