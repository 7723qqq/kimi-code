...
- [ ] 清理 skipped 测试中的旧 wire snapshot。`context.test.ts` 当前 live/current 协议断言已清到 v1.5 `context.splice`，仅保留带旧 `protocol_version` 的 migration fixture。`resume.test.ts`、`plan.test.ts`、`turn.test.ts`、`permission.test.ts`、`compaction/full.test.ts`、`compaction/micro.test.ts` 里仍有旧 record 形状；解除 skip 前先判断是“迁移 fixture”还是“当前协议快照”。
