export default {
  app: {
    title: 'KIMI INSPECT',
    disconnect: '断开连接',
    failedToOpenSession: '打开会话失败：{{message}}',
    wsLabel: 'ws：{{state}}',
  },
  connection: {
    title: 'Kimi Inspect',
    description: '连接到服务器 (/api/v2)。将 URL 留空以使用同源开发代理 ({{target}})。',
    discovered: '在本机上发现',
    discoveredHome: '在本机上发现 ({{home}})',
    connecting: '连接到 {{url}}…',
    discovering: '正在发现本地服务器…',
    serverUrl: '服务器 URL',
    urlPlaceholder: 'http://127.0.0.1:58627（留空 = 开发代理）',
    tokenLabel: 'Bearer token（可选）',
    tokenPlaceholder: '~/.kimi-code/server.token',
    connectButton: '连接',
    debugSurfaceUnavailable: '调试接口不可用',
    retry: '重试',
    changeServer: '切换服务器',
    debugHint: '连接到以以下参数启动的服务器',
    proxyHint: 'URL 留空则使用同源开发代理',
    discoveredLocal: '在本机发现',
  },
  chat: {
    nonTextContent: '[非文本内容]',
    selectSessionHint: '在左侧选择一个会话以打开其对话。',
    loadingSession: '正在加载会话…',
    emptyContext: '上下文为空 — 在下方发送提示。',
    promptPlaceholder: '向活动 Agent 发送提示…（回车发送，Shift+回车换行）',
    agentLabel: 'agent：{{agentId}}',
    turnRunning: '回合运行中',
    idle: '空闲',
    send: '发送',
    cancel: '取消',
    tool: '工具',
    noHitsInSession: '此会话中没有匹配项。',
    searchHint: '跨所有会话搜索用户提示、助手回复和会话标题。',
    noHitsForQuery: '“{{query}}” 无匹配结果。',
    searchExamples: '示例…',
    auditEmpty: '暂无记录 — 初始会话记录加载中。',
  },
  inspector: {
    tabs: {
      app: 'App',
      session: '会话',
      agent: 'Agent',
      events: '事件',
    },
    dynamicChannelUnavailable: '动态通道列表不可用 — 仅显示手写面板',
    activeAgent: '活动 Agent',
    agentNotLoaded: '{{id}}（未加载）',
    agentNotMaterialized:
      '此 Agent 未在运行中的服务器实例化（例如在重启前创建）— 调用将失败；其持久化记录保留在磁盘',
    methodsCount: '{{n}} 个方法 ⇄ {{domain}}',
    noSessionSelected: '未选择会话。',
    loadingSession: '正在加载会话…',
    loadingTranscript: '正在加载会话记录 — 通道就绪后即可看到审计轨迹。',
    refresh: '刷新',
    load: '加载',
    clickLoadHint: '点击 Load 读取此 Service',
    argNotValidJson: 'arg 不是有效的 JSON',
    noResult: '(无结果)',
    noCallableMembers: '无可调用成员',
    argJsonPlaceholder: 'arg (JSON)',
    nothingPending: '无待处理项',
    filterJsonPlaceholder: '过滤 JSON…',
    resume: '继续',
    pause: '暂停',
    eventsCount: '{{count}} 个事件（最新优先）',
    payload: '(负载)',
    pendingInteractions: '待处理交互',
    pendingInteractionsCount: '({{count}})',
    approve: '批准',
    reject: '拒绝',
    other: '其他…',
    dismiss: '忽略',
    clear: '清空',
    tool: '工具',
    get: '获取',
  },
  server: {
    proxy: ' ⇄ 代理',
    localServers: '本地服务器（{{home}}）',
    custom: '自定义：{{url}}',
    pid: ' ⇄ pid {{pid}}',
  },
  sidebar: {
    workingDirectoryPrompt: '新会话的工作目录：',
    newSessionNoWorkspace: '新建会话（无工作区）',
    newSessionInWorkspace: '在此工作区新建会话',
    loading: '加载中…',
    noSessions: '无会话',
    workspaces: '工作区',
    sessions: '会话',
    all: '(全部)',
    archived: '已归档',
    createSessionFailed: '创建会话失败：{{message}}',
  },
  navRail: {
    chat: '会话',
    search: '搜索',
    modelCatalog: '模型目录',
    appServices: '应用服务',
    workspaceServices: '工作区服务',
    filesystemSuggest: '文件建议',
    bashParser: 'Bash 解析器',
    di: 'DI',
  },
  modelCatalog: {
    title: '模型目录',
    counts: '{{providers}} 个提供商 · {{models}} 个模型',
    refresh: '刷新',
    loading: '加载中…',
    empty: '未配置任何提供商或模型',
    noProvider: '（无提供商）',
    defaultBadge: '默认',
    ping: 'Ping',
    pinging: 'Ping 中…',
    createSession: '+ 会话',
    creating: '创建中…',
    effort: 'effort：{{effort}}',
    pong: 'pong · {{ms}}ms',
    pingFailed: 'ping 失败 · {{ms}}ms',
    workingDirectoryPrompt: '新会话的工作目录：',
    contextSize: '{{size}} ctx',
    contextSizeK: '{{size}}k ctx',
    contextSizeM: '{{size}}M ctx',
    contextSizeUnknown: '—',
  },
  panels: {
    reconnectServer: '重新连接服务器',
    serverName: '服务器名称',
    enterManual: '进入（手动）',
    cancelTurn: '取消当前轮次',
    steps: '步数',
    setTitle: '设置标题',
    setModel: '设置模型',
    stopTask: '停止任务',
    agentState: '智能体状态',
    sessionState: '会话状态',
    filterServices: '筛选服务…',
    jsonOrString: 'JSON 或纯字符串',
    changeSort: '点击切换排序',
  },
  search: {
    thisSession: '搜索当前会话…（回车搜索，Esc 关闭）',
    allSessions: '搜索所有会话的消息…',
    documentRole: '文档角色',
    substringMatch: '子串匹配，忽略大小写（较慢，可匹配 C++ 等符号）',
    openInChat: '在对话中打开',
    searching: '搜索中…',
  },
  views: {
    transcriptAudit: '记录审计',
    noEntrySelected: '未选择条目。',
    filesystemSuggest: '文件系统补全建议',
    planLookup: '计划查询',
    workspaceAssociation: '工作区关联',
    pendingInteractions: '待处理交互',
    history: '历史',
  },
  actions: {
    load: '加载',
    clear: '清空',
  },
  bashParser: {
    budgetExhausted: '解析预算已耗尽',
    budgetExhaustedHint: '无法分析该语法树；请提高预算或缩小输入。',
  },
} as const;
