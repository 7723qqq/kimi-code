export default {
  app: {
    title: 'KIMI INSPECT',
    disconnect: 'Disconnect',
    failedToOpenSession: 'Failed to open session: {{message}}',
    wsLabel: 'ws: {{state}}',
  },
  connection: {
    title: 'Kimi Inspect',
    description:
      'Connect to a server (/api/v2). Leave the URL empty to use the same-origin dev proxy ({{target}}).',
    discovered: 'Discovered on this machine',
    discoveredHome: 'Discovered on this machine ({{home}})',
    connecting: 'Connecting to {{url}}…',
    discovering: 'Discovering local servers…',
    serverUrl: 'Server URL',
    urlPlaceholder: 'http://127.0.0.1:58627 (empty = dev proxy)',
    tokenLabel: 'Bearer token (optional)',
    tokenPlaceholder: '~/.kimi-code/server.token',
    connectButton: 'Connect',
    debugSurfaceUnavailable: 'Debug surface unavailable',
    retry: 'Retry',
    changeServer: 'Change server',
  },
  chat: {
    nonTextContent: '[non-text content]',
    selectSessionHint: 'Select a session on the left to open its conversation.',
    loadingSession: 'Loading session…',
    emptyContext: 'Empty context — send a prompt below.',
    promptPlaceholder:
      'Send a prompt to the active agent… (Enter to send, Shift+Enter for newline)',
    agentLabel: 'agent: {{agentId}}',
    turnRunning: 'turn running',
    idle: 'idle',
    send: 'Send',
    cancel: 'Cancel',
    tool: 'tool',
    noHitsInSession: 'No hits in this session.',
    searchHint: 'Search user prompts, assistant replies and session titles across every session.',
    noHitsForQuery: 'No hits for “{{query}}”.',
    searchExamples: 'examples…',
    auditEmpty: 'Nothing recorded yet — the initial transcript load is still running.',
  },
  inspector: {
    tabs: {
      app: 'App',
      session: 'Session',
      agent: 'Agent',
      events: 'Events',
    },
    dynamicChannelUnavailable: 'dynamic channel list unavailable — showing handwritten panels only',
    activeAgent: 'Active agent',
    agentNotLoaded: '{{id}} (not loaded)',
    agentNotMaterialized:
      'this agent is not materialized in the running server (e.g. created before a restart) — calls will fail; its persisted records remain on disk',
    methodsCount: '{{n}} methods ⇄ {{domain}}',
    noSessionSelected: 'No session selected.',
    loadingSession: 'Loading session…',
    loadingTranscript:
      'Loading transcript — the audit trail appears once the channel is up.',
    refresh: 'Refresh',
    load: 'Load',
    clickLoadHint: 'click Load to read this Service',
    argNotValidJson: 'arg is not valid JSON',
    noResult: '(no result)',
    noCallableMembers: 'no callable members',
    argJsonPlaceholder: 'arg (JSON)',
    nothingPending: 'nothing pending',
    filterJsonPlaceholder: 'filter JSON…',
    resume: 'Resume',
    pause: 'Pause',
    eventsCount: '{{count}} events (newest first)',
    payload: '(payload)',
    pendingInteractions: 'Pending interactions',
    pendingInteractionsCount: '({{count}})',
    approve: 'Approve',
    reject: 'Reject',
    other: 'Other…',
    dismiss: 'Dismiss',
    clear: 'Clear',
    tool: 'tool',
    get: 'get',
  },
  server: {
    proxy: ' ⇄ proxy',
    localServers: 'Local servers ({{home}})',
    custom: 'custom: {{url}}',
    pid: ' ⇄ pid {{pid}}',
  },
  sidebar: {
    workingDirectoryPrompt: 'Working directory for the new session:',
    newSessionNoWorkspace: 'New session (no workspace)',
    newSessionInWorkspace: 'New session in this workspace',
    loading: 'loading…',
    noSessions: 'no sessions',
    workspaces: 'Workspaces',
    sessions: 'Sessions',
    all: '(all)',
    archived: 'archived',
    createSessionFailed: 'create session failed: {{message}}',
  },
  navRail: {
    chat: 'Chat',
    search: 'Search',
    modelCatalog: 'Model Catalog',
    appServices: 'App Services',
    workspaceServices: 'Workspace Services',
    filesystemSuggest: 'Filesystem Suggest',
    bashParser: 'Bash Parser',
    di: 'DI',
  },
  modelCatalog: {
    title: 'Model Catalog',
    counts: '{{providers}} providers · {{models}} models',
    refresh: 'Refresh',
    loading: 'loading…',
    empty: 'no providers or models configured',
    noProvider: '(no provider)',
    defaultBadge: 'default',
    ping: 'Ping',
    pinging: 'pinging…',
    createSession: '+ Session',
    creating: 'creating…',
    effort: 'effort: {{effort}}',
    pong: 'pong · {{ms}}ms',
    pingFailed: 'ping failed · {{ms}}ms',
    workingDirectoryPrompt: 'Working directory for the new session:',
    contextSize: '{{size}} ctx',
    contextSizeK: '{{size}}k ctx',
    contextSizeM: '{{size}}M ctx',
    contextSizeUnknown: '—',
  },
  panels: {
    reconnectServer: 'Reconnect server',
    serverName: 'Server name',
    enterManual: 'enter (manual)',
    cancelTurn: 'cancel turn',
    steps: 'Steps',
    setTitle: 'Set title',
    setModel: 'Set model',
    stopTask: 'Stop task',
    agentState: 'Agent state',
    sessionState: 'Session state',
    filterServices: 'Filter services…',
    jsonOrString: 'JSON or plain string',
    changeSort: 'Click to change sort',
  },
  search: {
    thisSession: 'Search this session… (Enter to search, Esc to close)',
    allSessions: 'Search messages across all sessions…',
    documentRole: 'Document role',
    substringMatch: 'Substring match, case-insensitive (slower, matches symbols like C++)',
    openInChat: 'Open in chat',
  },
} as const;
