  ...
  it('fetchFullHistory pages before_turn to the beginning and returns timeline order', async () => {
    ...
    const fetchImpl = (async (url: string | URL) => {
      ...
      const envelope =
        before === undefined
          ? pages['newest']
          : (pages[before] ?? okEnvelope({ messages: [], has_more: false }));
      return { json: async () => envelope };
    }) as unknown as typeof fetch;
    ...
    expect(messages.map((m) => ('turn_id' in m ? m.turn_id : ''))).toEqual([
      't1',
      't2',
      't3',
      't3',
...
describe('ChatStore', () => {
  ...
  it('skips an upsert whose timestamp is older than the held entity', () => {
    const store = makeStore();
    store.applyLive(
      assistantMsg('t1.1', 'hello world', 'streaming', Date.parse('2026-01-01T00:00:10.000Z')),
    );
    store.applyLive(
      assistantMsg('t1.1', 'hel', 'streaming', Date.parse('2026-01-01T00:00:05.000Z')),
    );
  ...
  it('replace installs the page as the window and keeps entries newer than the page', () => {
    ...
    store.applyHistoryPage(
      [
        turnMsg(1, 'completed', Date.parse('2026-01-01T00:00:01.500Z')),
        stepMsg('t1.1', 'completed', Date.parse('2026-01-01T00:00:02.000Z')),
      ],
      'replace',
    );
...
describe('recoverLoadedWindow', () => {
  const pageOf = (items: V3HistoryMessage[], hasMore: boolean): V3HistoryMessage[] => items;
...
describe('ChatChannel', () => {
  function scriptedFetch(script: {
    noCursor: unknown[];
    afterStep?: Record<string, readonly unknown[]>;
  }) {
    const calls: string[] = [];
    let noCursorIndex = 0;
    const fetchImpl = (async (url: string | URL) => {
      ...
      if (after !== undefined) {
        envelope = okEnvelope({
          messages: [...(script.afterStep?.[after] ?? [])],
          has_more: false,
        });
      } else {
  ...
  it('serializes the initial refresh with the ack catch-up behind one queue', async () => {
    ...
    expect(
      channel.trail.getEntries().some((e) => e.kind === 'event' && e.event === 'catchup-refresh'),
    ).toBe(false);
  ...
  it('probes the newest page for the anchor step or turn before falling back to a refresh', async () => {
    ...
    const probeWithTurn = okEnvelope({
      messages: [systemMsg('notice', 'sys_n1'), turnMsg(1)],
      has_more: false,
    });
    ...
    expect(
      aliveChannel.channel.trail
        .getEntries()
        .some((e) => e.kind === 'event' && e.event === 'catchup-refresh'),
    ).toBe(false);
    expect(
      aliveChannel.channel.trail
        .getEntries()
        .filter((e) => e.kind === 'rest' && e.mode === 'replace'),
    ).toHaveLength(1);
...
describe('projectPlans', () => {
  ...
  it('derives plan content and review from the linked approval interaction', () => {
    const messages: V3HistoryMessage[] = [
      turnMsg(1),
  ...
  it('falls back to the tool call display, then to the output body', () => {
    ...
    expect(fromDisplay[0]).toMatchObject({
      source: 'display',
      plan: '# Draft',
      path: '/tmp/draft.md',
    });
