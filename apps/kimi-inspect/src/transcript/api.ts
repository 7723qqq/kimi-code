 ...
 * time-ordered, same schemas as the WS stream). Paging is by page length: a
 * page shorter than `page_size` is the end in that direction, an empty page
 * is definitive.
 */

import { v3HistoryResponseSchema, type V3HistoryMessage } from '@moonshot-ai/protocol/v3';

export const HISTORY_PAGE_SIZE = 500;

export interface HistoryPage {
  readonly messages: readonly V3HistoryMessage[];
...
export async function fetchHistoryPage(opts: FetchHistoryPageOptions): Promise<HistoryPage> {
  ...
  const parsed = v3HistoryResponseSchema.safeParse(envelope.data);
  if (!parsed.success) {
...
export async function fetchFullHistory(opts: {
  ...
  readonly pageSize?: number;
  readonly fetchImpl?: typeof fetch;
}): Promise<readonly V3HistoryMessage[]> {
...
function historyEntityKey(message: V3HistoryMessage): string {
  switch (message.type) {
    case 'turn':
