 ...
 * the native `kimi-agent --serve` server with `--debug-endpoints` on a
 * loopback bind); the v2 surface
...
/** Mirror of the server's `ChannelDescriptor` (`GET /api/v1/debug/channels`). */
 ...
 * start the server with `--debug-endpoints`, or a rejected probe → check the
 * token).
 */
export async function probeDebugSurface(options: {
  ...
  try {
    ...
    throw new Error(`cannot reach ${options.baseUrl} — is the server running? (${reason})`, {
      cause: error,
    });
  }
  if (!res.ok) {
    throw new Error(
      `GET ${DEBUG_RPC_BASE}/channels answered HTTP ${res.status} — this server does not ` +
        'mount the debug RPC surface. Start the server with --debug-endpoints on a loopback bind.',
    );
  }
