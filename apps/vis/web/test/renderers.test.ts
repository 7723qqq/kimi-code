...
>[0];
type TrackedRecord = Parameters<(typeof WIRE_RENDERERS)['file_history.tracked']['headline']>[0];
...
describe('wire renderers', () => {
  // The assertions below read the English copy; pin the locale so a
  // Chinese-locale host does not flip them.
  beforeAll(() => {
    setLocale('en');
  });

  it('covers every durable record in the local wire contract', async () => {
    const contractPath = resolve(import.meta.dirname, '../../server/src/lib/v2-wire.ts');
    const contract = await readFile(contractPath, 'utf8');
    const body = /export interface WireRecordEvents \{\n([\s\S]*?)\n\}/.exec(contract)?.[1];
    expect(body).toBeDefined();

    const contractTypes = [...(body ?? '').matchAll(/^ {2}'?([\w.]+)'?:/gm)]
      .map((match) => match[1]!)
    ...
    expect(renderedCurrentTypes).toEqual(contractTypes);
  });
  ...
  it('preserves each compaction summary variant and legacy field names', () => {
    ...
    expect(renderToStaticMarkup(renderer.headline(contextOnly).main)).toContain('contextSummary');
    expect(renderToStaticMarkup(renderer.detail?.(contextOnly))).toContain('context-only summary');
