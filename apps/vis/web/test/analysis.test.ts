...
describe('analyzeWire', () => {
  ...
  it('keeps steering inside the active turn and folds the durable turn outcome', () => {
    ...
    const a = analyzeWire([
      e(
        { type: 'turn.prompt', input: [{ type: 'text', text: 'start' }], origin: { kind: 'user' } },
        1000,
      ),
      loop({ type: 'step.begin', uuid: 's1', turnId: '7', step: 0 }, 1100),
      loop(
        { type: 'content.part', stepUuid: 's1', part: { type: 'think', think: 'reasoning' } },
        1150,
      ),
      e(
        {
      ...
      e(
        { type: 'token_counting.turn_recorded', agentId: 'main', turnId: 7, length: 2, tokens: 50 },
        1501,
      ),
      e(
  ...
  it('uses a steer as the trigger when the next step belongs to a new turn', () => {
    ...
    const a = analyzeWire([
      ...
      loop(
        { type: 'tool.result', parentUuid: 'tc2', toolCallId: 'c2', result: { output: 'done' } },
        1500,
      ),
      loop(
        {
          ...
          step: 0,
          finishReason: 'end_turn',
          usage: { inputOther: 20, output: 4, inputCacheRead: 5, inputCacheCreation: 1 },
        },
        1600,
      ),
    ]);
