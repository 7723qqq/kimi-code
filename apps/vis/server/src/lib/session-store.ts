...
interface StateJson {
  ...
  //
  // v2 writes the REAL parent / swarm-item / profile labels under `labels`
...
async function scanWire(path: string): Promise<{ count: number; protocolVersion: string }> {
  ...
  for await (const line of rl) {
    ...
    if (protocolVersion === null) {
      // A wire with no `metadata` header is still readable: the wire-reader
      // recovers it as v1.4 and applies the 1.4 → 1.5 migration in memory, so
      // the list view must not call it broken.
...
function normalizeAgentType(value: unknown, agentId: string): AgentInfo['type'] {
