...
describe('task-store', () => {
  ...
  it('sanitizes type-corrupt optional fields on every current task kind', async () => {
    ...
    await writeTask(sessionDir, 'bash-aaaaaaaa.json', {
      ...
      endedAt: null,
      stopReason: {},
      terminalNotificationSuppressed: 'yes',
      resumeReminded: [],
      timeoutMs: '1000',
      parentToolCallId: {},
    });
    ...
    for (const task of tasks) {
      expect(Object.values(task).some((value) => value !== null && typeof value === 'object')).toBe(
        false,
      );
    }
  });
  ...
  it('skips task ids that disagree with their file key and keeps primary shadowing', async () => {
    ...
    await writeTask(mainDir, 'bash-aaaaaaaa.json', {
      taskId: 'bash-bbbbbbbb',
      kind: 'process',
      description: 'current mismatch',
      command: 'true',
      pid: 1,
      exitCode: 0,
      status: 'completed',
    ...
    await writeTask(sessionDir, 'bash-eeeeeeee.json', {
      taskId: 'bash-eeeeeeee',
      kind: 'process',
      description: 'fallback shadowed',
      command: 'true',
      pid: 2,
      exitCode: 0,
      status: 'completed',
    ...
    await writeTask(mainDir, 'bash-eeeeeeee.json', {
      taskId: 'bash-ffffffff',
      kind: 'process',
      description: 'primary mismatch',
      command: 'true',
      pid: 3,
      exitCode: 0,
      status: 'completed',
  ...
  it('falls back to session-root tasks for main and lets primary keys shadow fallback', async () => {
    ...
    await writeTask(mainDir, 'bash-cccccccc.json', {
      taskId: 'bash-cccccccc',
      kind: 'process',
      description: 'primary visible',
      command: 'primary',
      pid: 3,
      exitCode: 0,
      status: 'completed',
