...
- [`e62a592`](https://github.com/MoonshotAI/kimi-code/commit/e62a5926ad43c5537298eb55382f37c06c31299e) Thanks [@liruifengv](https://github.com/liruifengv)! - Add shell mode for running shell commands.
...
- [`d036310`](https://github.com/MoonshotAI/kimi-code/commit/d03631081b42f9a96a0d2c0d03441639b159c269) Thanks [@wbxl2000](https://github.com/wbxl2000)! - Add a confirmation prompt before installing third-party plugins.
...
- feat(i18n): complete i18n multi-language support for CLI, TUI, and Web UI
  feat(swarm): add Swarm Discussion multi-agent collaboration
  feat(native): Rust-native tool acceleration for grep, glob, edit, read, write, bash
  fix(build): Windows compatibility fixes and startup scripts

- [`503d70d`](https://github.com/MoonshotAI/kimi-code/commit/503d70db06f990c35b8fa332944ee8cdff4e14d6) - Complete the Rust native tools migration: Read uses streaming I/O, Grep filters sensitive files and skips VCS metadata, Bash calls native execution for foreground commands while preserving background task support.

- [`4709617`](https://github.com/MoonshotAI/kimi-code/commit/4709617ad38ca4a9f68c0c8d017e13a12fd76b75) Thanks [@wbxl2000](https://github.com/wbxl2000)! - Plugins can now declare hooks in their manifest to run scripts on lifecycle events.

- [`83da734`](https://github.com/MoonshotAI/kimi-code/commit/83da7341d544c7211c3f6587ec8ce6526756222b) - Apply agent-level retry uniformly across all model providers instead of skipping it for providers whose SDKs handle retry internally.

- [`cf8c4ce`](https://github.com/MoonshotAI/kimi-code/commit/cf8c4ce428cd0888c7cb48871c1ef37aed128d64) Thanks [@sailist](https://github.com/sailist)! - Add server authentication and safe `--host` exposure. The local server now
...
- [`43fc0c8`](https://github.com/MoonshotAI/kimi-code/commit/43fc0c85c07f8abf59af92a4b255b9f651ae28cb) Thanks [@RealKai42](https://github.com/RealKai42)! - Add an `update` alias for the `kimi upgrade` command. Run `kimi update` to upgrade to the latest version.

- [`dc86c7e`](https://github.com/MoonshotAI/kimi-code/commit/dc86c7ecbf040613476ea1e7bdae1adf06b8d0cd) Thanks [@liruifengv](https://github.com/liruifengv)! - Show the done / in progress / pending breakdown of hidden todos in the collapsed todo panel.

- [`2821dbb`](https://github.com/MoonshotAI/kimi-code/commit/2821dbb6791197c3b63ef0c6bf22509fd25d65b4) Thanks [@wbxl2000](https://github.com/wbxl2000)! - Cap compaction output at 128k tokens by default to avoid provider max_tokens errors.
...
- [`503d70d`](https://github.com/MoonshotAI/kimi-code/commit/503d70db06f990c35b8fa332944ee8cdff4e14d6) - Fix Bash tool background-disabled wording when the shell description template is checked out with CRLF line endings.

- [`07c7cec`](https://github.com/MoonshotAI/kimi-code/commit/07c7cece0c6f8628f4adaa1442882e62fbf74126) Thanks [@qkunio](https://github.com/qkunio)! - Fix compaction ignoring the configured max output size.

- [`fb498db`](https://github.com/MoonshotAI/kimi-code/commit/fb498db877851bf575a3139c00b4028f1b034496) Thanks [@liruifengv](https://github.com/liruifengv)! - Fix the terminal window repeatedly losing focus on Linux Wayland, which broke IME input.

- [`dd0ae57`](https://github.com/MoonshotAI/kimi-code/commit/dd0ae577aa8a3d77736966543549a6470e3b8c88) Thanks [@chengluyu](https://github.com/chengluyu)! - Fix resuming sessions whose original working directory no longer exists.

- [`e6db644`](https://github.com/MoonshotAI/kimi-code/commit/e6db6440e55aa00d787182ed87413fc8950fb82a) Thanks [@chengluyu](https://github.com/chengluyu)! - Fix prompt-mode goals so they run until completion and report invalid goal commands before sending prompts.
...
- [`1837c70`](https://github.com/MoonshotAI/kimi-code/commit/1837c70256bcc69dba4d7ddfd908bc8af604e2a6) Thanks [@RealKai42](https://github.com/RealKai42)! - Harden the default system prompt and built-in tool descriptions: stop the agent from blocking on background tasks it should let run, keep its guidance matched to the tools each profile actually provides, and surface tool-result details (fetched-page mode, Grep match totals) it previously missed.

- [`a5e3d4a`](https://github.com/MoonshotAI/kimi-code/commit/a5e3d4a69ea2a88bb792781cc6702af200bf27ea) - Replace hardcoded English strings across CLI subcommands, TUI components, and update prompts with i18n `t()` calls so they respect the configured locale. Also fix ZH locale key parity (1375/1375) and a `slashCommands` path mismatch that caused raw key fallback.

  web: Replace hardcoded strings in ServerAuthDialog, GoalStrip, CommandBar, Sheet, and Dialog with i18n calls.

- [`f1751bf`](https://github.com/MoonshotAI/kimi-code/commit/f1751bfa7a44377f2ff87146aad0cb0482c1032f) - Keep image-heavy sessions within provider request-size limits: model-read images now honor a 256 KB per-image budget and a 2000px downscale cap (configurable via `[image]` in config.toml or `KIMI_IMAGE_*` env vars), oversized WebP is compressed as well, HEIC/HEIF reads are refused with a platform-matched conversion command instead of poisoning the session, and a request-too-large rejection (HTTP 413) now recovers automatically — the request and /compact both retry with older media replaced by text markers instead of failing the session.

- [`2183ee7`](https://github.com/MoonshotAI/kimi-code/commit/2183ee73df2319e8593e780d24938960f33c343d) Thanks [@liruifengv](https://github.com/liruifengv)! - Add a hint to the per-turn step limit error pointing users to the loop_control.max_steps_per_turn config option.
...
- [`5bebb92`](https://github.com/MoonshotAI/kimi-code/commit/5bebb92420824ed4a53b24696e523ea96cbe679b) Thanks [@wbxl2000](https://github.com/wbxl2000)! - `/reload` now refreshes the assistant's view of plugin skills, so plugin changes take effect in the current session instead of requiring a new one.

- [`8e04890`](https://github.com/MoonshotAI/kimi-code/commit/8e0489076ff4fd921774e19d203950f11fee7265) - The `[image]` limits in config.toml now also apply to pasted images (CLI paste and ACP prompts), and each core now uses its own settings, so reloading one client's config no longer changes another client's image compression.
...
- [`7fd3b83`](https://github.com/MoonshotAI/kimi-code/commit/7fd3b8351d117882c0d4f05ae0671bd3cf9a1a5d) Thanks [@liruifengv](https://github.com/liruifengv)! - Keep the terminal responsive in long conversations by caching rendered message lines.

- [`7fd3b83`](https://github.com/MoonshotAI/kimi-code/commit/7fd3b8351d117882c0d4f05ae0671bd3cf9a1a5d) Thanks [@liruifengv](https://github.com/liruifengv)! - Keep long sessions responsive by retaining only recent turns in the transcript and collapsing older steps within each turn.

- [`41ac856`](https://github.com/MoonshotAI/kimi-code/commit/41ac8567d59553016fb3400e4dc3fd5a1b09a674) Thanks [@wbxl2000](https://github.com/wbxl2000)! - Add a Kimi WebBridge entry to the Official tab of the /plugins panel that opens the WebBridge install page in your browser.
...
- [`be77aed`](https://github.com/MoonshotAI/kimi-code/commit/be77aed13d2701c3dda67c6d70eb1d6f87f4c8da) Thanks [@liruifengv](https://github.com/liruifengv)! - web: Add notifications when a tool needs approval, and improve notification reliability.

- [`cee6a1d`](https://github.com/MoonshotAI/kimi-code/commit/cee6a1d836567251d1edd00de0b1e6797f2805e8) Thanks [@wbxl2000](https://github.com/wbxl2000)! - web: Fix an occasional "another turn is active" error when sending the first message of a new conversation, and show a starting state while it is being sent.

- [`ed72f7a`](https://github.com/MoonshotAI/kimi-code/commit/ed72f7a8a2685ffbc8f1165711394fed49bedad0) Thanks [@wbxl2000](https://github.com/wbxl2000)! - Fix the /web slash command not carrying the server token, so the opened web UI signs in automatically and the token is shown before the terminal exits.

- [`23a7c99`](https://github.com/MoonshotAI/kimi-code/commit/23a7c9985c18d01dccd4493ed69d29d19d1fc9a7) Thanks [@wbxl2000](https://github.com/wbxl2000)! - Fix duplicate session snapshot reloads in the bundled web UI during resync.

- [`a5832b9`](https://github.com/MoonshotAI/kimi-code/commit/a5832b93ced30e522a40c87c6aee188e4d99776b) - Make the wire transcript reader resilient to individual corrupted lines so the web UI and CLI can still load a session transcript even when a crash left the wire file in a partially-written state.
