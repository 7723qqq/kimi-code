...
describe('git status cache', () => {
  ...
  it('keeps the dirty flag correct when paths are non-ASCII or C-quoted', () => {
    mocks.spawnSync.mockImplementation((_cmd: string, args: string[]) => {
      if (args.includes('status')) {
        return {
          status: 0,
          // Neither raw non-ASCII paths nor C-quoted octal escapes may break
          // record counting: each record still starts with its status prefix.
