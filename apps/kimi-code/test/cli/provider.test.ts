...
import { credentialEnvHints, fetchCustomRegistry } from '@moonshot-ai/kimi-code-oauth';
...
function makeHarness(initial: KimiConfig): {
  ...
  const harness: FakeHarness = {
    ...
    // Only the fetch half is modelled: these tests assert which credential the
    // registry request carries, and the config half is covered by the cases
    // that run against a real harness (`makeRegistryHarness`).
    importCustomRegistry: async (options) => {
      ...
      return {
        providers: entryList.map((entry) => ({ id: entry.id })),
