...
describe('plugins command capability surface', () => {
  ...
  it('routes built-in entries through capabilities via the parser-proof builtIn flag', () => {
    const host = fakeHost({}).host;
    expect(
      isCapabilityEntry(host, { id: 'kimi-cu', source: 'capability:kimi-cu', builtIn: true } as never),
    ).toBe(true);
    expect(
      isCapabilityEntry(host, {
        id: 'kimi-webbridge',
    ...
    expect(
      isCapabilityEntry(host, { id: 'kimi-cu', source: 'https://example.test/plugin.zip' } as never),
    ).toBe(false);
    // A forged capability: source without the parser-proof flag is a plain row.
    expect(
      isCapabilityEntry(host, { id: 'kimi-cu', source: 'capability:kimi-cu' } as never),
    ).toBe(false);
  });
