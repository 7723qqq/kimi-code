/**
 * Layout / rendering constants for the v2 TUI.
 *
 * Mirrors `tui/constant/rendering.ts`. These describe cell geometry and
 * transcript/panel layout parameters that stay the same under both the
 * pi-tui string[] model and the opentui layout tree.
 *
...
export const SUBAGENT_TOOL_OUTPUT_MAX_CHARS = 8000;// Cap on individual string argument values kept in a record (Write/Edit
...
// AgentSwarm member rows: at most N rows render inline under the swarm tool
// card and the rest collapse into a "+N more" line; elapsed seconds tick
// once per second while any member is still active.
export const MAX_VISIBLE_SWARM_ROWS = 8;
export const SWARM_ELAPSED_TICK_MS = 1000;
