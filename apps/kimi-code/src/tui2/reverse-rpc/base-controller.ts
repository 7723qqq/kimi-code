 ...
 *
 * Mirrors `tui/reverse-rpc/base-controller.ts`. Pure logic — approval and
 * question flows wait for a UI action before returning a response; subclasses
 * only define the default cancellation response. When concurrent requests
 * arrive, only one panel shows at a time; the rest queue in arrival order.
 *
 * The UI interaction is injected via {@link ReverseRpcUIHooks} (showPanel /
 * hidePanel), so this controller is independent of the renderer that hosts
 * the panel.
 *
