/**
 * Approval request → panel data adapter.
 *
 * Mirrors `tui/reverse-rpc/approval/adapter.ts`. Converts an SDK
 * `ApprovalRequest` into the view-layer `ApprovalPanelData`, extracting
 * structured display blocks (shell / diff / file_content / url_fetch /
 * search) from generic tool args where possible, and maps a user's panel
 * response back into the SDK `ApprovalResponse` shape. Pure logic.
 *
 * The dialog panel imports (`#/tui2/components/dialogs/...`) resolve to the
 * tui2 stubs, which re-export the v1 panel types / helpers for now.
 *
...
import { t } from '#/i18n';
import type { ApprovalPanelResponse } from '#/tui2/components/dialogs/approval-panel';
import { goalStartOptions } from '#/tui2/components/dialogs/goal-start-permission-prompt';
import type { ApprovalPanelChoice, ApprovalPanelData, DisplayBlock } from '../types';
...
function extractFromArgs(
  ...
  // Only the builtin shell tool gets shell styling and danger scanning.
