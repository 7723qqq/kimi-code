/**
 * Modal coordination for reverse RPC dialogs.
 *
 * Mirrors `tui/reverse-rpc/modal-coordinator.ts`. Guarantees only one
 * approval/question modal is shown at a time; the other type waits in a queue
 * until the active modal is hidden. Pure logic — the actual show/hide is
 * delegated to injected UI hooks.
 *
