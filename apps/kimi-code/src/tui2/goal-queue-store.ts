/**
 * TUI2 goal queue store — durable queue of upcoming goals.
 *
 * Mirrors `tui/goal-queue-store.ts` — pure logic (node:fs + SDK errors), so
 * the tui2 version is a straight copy with imports converged onto the tui2
 * tree.
 *
 * Status: REAL (tui2). Mirrors `tui/goal-queue-store.ts`.
 */
