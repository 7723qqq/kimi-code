/**
 * TUI2 `/dance` easter egg — rainbow text coloring + animation state machine.
 *
 * Mirrors `tui/easter-eggs/dance.ts` with the v1 `currentTheme.boldFg`
 * helper replaced by a chalk composition over the tui2 theme's hex tokens.
 *
...
function getDanceRainbowPalette(): readonly [string, ...string[]] {
  return currentTheme.palette.family === 'light' ? LIGHT_RAINBOW : DARK_RAINBOW;
}
...
export function tryHandleDanceCommand(host: SlashCommandHost, parsed: ParsedSlashInput): boolean {
  ...
  const cmd = (text: string): string => chalk.hex(currentTheme.hex('primary')).bold(text);
