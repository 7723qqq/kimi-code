...
/**
 * TUI2 ReadMediaFile renderer.
 *
 * Mirrors `tui/components/messages/tool-renderers/media.ts`. The
 * ReadMediaFile tool `output` is the JSON-serialized array of content
 * parts the tool returned — which includes the full base64 of the
 * image/video. This renderer parses the envelope and surfaces just the
 * human-readable bits (kind, path, mime, size) via a header chip + a
 * tiny expanded body. It never emits the base64.
 *
...
export const readMediaSummary: ResultRenderer = (toolCall, result, ctx) => {
  ...
  const dim = currentTheme.color('textDim');
  return (
    <Box flexDirection="column" paddingLeft={2}>
      <Show when={summary.path !== undefined}>
        <Text fg={dim} wrapMode="word">
          {summary.path}
        </Text>
      </Show>
      <Text fg={dim} wrapMode="word">
        {readMediaTail(summary)}
      </Text>
    </Box>
  );
};

function readMediaTail(summary: ReadMediaSummary): string {
  const meta = metaSegments(summary);
