/**
 * TUI2 tool result renderer registry.
 *
 * Mirrors `tui/components/messages/tool-renderers/registry.ts`. Each tool
 * name maps to a `ResultRenderer` that turns the tool's
 * `ToolResultBlockData` into a SolidJS element. Tools without an explicit
 * entry fall through to `renderTruncated` (the original 3-line + ctrl+o
 * behavior).
 *
