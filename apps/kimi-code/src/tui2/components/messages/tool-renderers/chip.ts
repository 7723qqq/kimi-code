/**
 * TUI2 header chip providers — produce a short "stat" suffix appended to
 * the tool call header once a result has arrived.
 *
 * Mirrors `tui/components/messages/tool-renderers/chip.ts`. Chips own the
 * *numeric* summary (line counts, exit codes, byte sizes) as plain
 * strings — the header view applies the colour tokens. Pure logic, no
