/**
 * TUI2 streaming argument preview helpers.
 *
 * Mirrors `tui/components/messages/tool-call/streaming-preview.ts` with
 * imports converged onto the tui2 tree. `extractPartialStringField` pulls
 * the live value of a JSON string field out of partially-streamed
 * arguments, even if the closing quote hasn't arrived yet.
 * `parseStreamingArgs` is re-exported from event-payload for convenience.
 *
