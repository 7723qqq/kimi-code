 ...
 *
 * Mirrors `tui/theme/detect.ts`. The probe logic is unchanged — it runs before
 * any raw-mode framework (pi-tui or opentui) owns stdin, so the OSC reply is
 * not eaten by the input loop.
 *
 * Strategy, in priority order:
 *   1. Reject — non-TTY, NO_COLOR, FORCE_COLOR=0, CI → safe `'dark'`.
 *   2. OSC 11 — write `ESC ] 11 ; ? BEL`, parse `ESC ] 11 ; rgb:RR/GG/BB BEL`,
 *      compute relative luminance. Capped at `timeoutMs`.
...
import { OSC11_QUERY, TERMINAL_THEME_DETECT_TIMEOUT_MS } from '#/tui2/constant/terminal';
