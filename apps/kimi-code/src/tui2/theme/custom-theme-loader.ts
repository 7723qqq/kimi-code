 ...
 *
 * Mirrors `tui/theme/custom-theme-loader.ts`. The loader is pure I/O + schema
 * validation; framework-agnostic. The "don't print while pi-tui owns the
 * terminal" comment is generalised here: the same caveat applies while any
 * fullscreen renderer (pi-tui or opentui) owns the screen.
 *
...
async function readCustomTheme(name: string): Promise<ParsedCustomTheme | null> {
  try {
    ...
    // a fullscreen renderer owns the terminal, where raw stdout/stderr writes
    // corrupt the rendered screen. Authoring-time validation lives in the JSON
    // schema.
