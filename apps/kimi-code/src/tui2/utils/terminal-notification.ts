/**
 * TUI2 terminal notification — OSC 9 / BEL desktop notifications.
 *
 * Mirrors `tui/utils/terminal-notification.ts` with the pi-tui `Terminal`
 * dependency swapped for a plain `write(data)` function, so the tui2 shell
 * can wire it to opentui's output stream.
 *
 * Status: REAL (tui2). Replaces the v1 stub.
 */

import { BEL, ESC, MAX_TERMINAL_NOTIFICATION_MESSAGE_LENGTH, ST } from '../constant/terminal';
...
/** The slice of the shell the notifier needs. */
...
export function notifyTerminalOnce(
  state: TerminalNotificationState,
  key: string,
  notification: TerminalNotification,
): void {
  const { enabled, condition } = state.notifications;
  if (!enabled) return;
  ...
  emitTerminalNotification((data) => state.write(data), notification, {
