/**
 * TUI2 steer-input composition for `session.steer`: flattens queued items
 * (and the editor draft) into one payload — the historical `'\n\n'`-joined
 * string when nothing carries media, or a merged part list when any item has
 * extracted media parts (queued image messages, or the editor draft after
 * placeholder extraction). Media parts are self-contained references; no
 * machine `<media path>` tag is authored, so text parts always merge freely.
 *
 * Mirrors `tui/utils/steer-input.ts`.
 *
 * Status: REAL (tui2). Mirrors `tui/utils/steer-input.ts`.
 */
