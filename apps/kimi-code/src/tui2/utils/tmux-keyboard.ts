/**
 * Detect a tmux keyboard-capture misconfiguration and surface a warning so the
 * user knows modified Enter keys may not work.
 *
...
function readTmuxOptionFromProcess(option: string): Promise<string | undefined> {
  return new Promise((resolve) => {
    ...
    const finish = (value?: string | undefined) => {
      ...
      settled = true;
      clearTimeout(timer);
      // eslint-disable-next-line no-multiple-resolved
      resolve(value);
    };
