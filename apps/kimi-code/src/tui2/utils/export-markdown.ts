/**
 * Markdown session exporter: builds the `# Kimi Session Export` document from
 * a context history, grouping messages into turns, hiding internal protocol
 * messages, and summarizing tool calls / results for the markdown preview.
 *
...
export function buildExportMarkdown(input: BuildExportMarkdownInput): string {
  ...
  for (const [i, turn] of turns.entries()) {
    lines.push(formatTurnMd(turn, i + 1));
  }
