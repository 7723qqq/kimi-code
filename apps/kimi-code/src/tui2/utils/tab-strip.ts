/**
 * TUI2 shared tab strip renderer for tabbed dialogs.
 *
 * Mirrors `tui/utils/tab-strip.ts` with imports converged onto the tui2 tree.
 * The active tab is filled with the brand background, inactive tabs are
 * muted. When the strip is wider than the terminal, it scrolls to keep the
 * active tab visible, framed by `<`/`>` markers.
 *
