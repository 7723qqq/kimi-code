/**
 * MCP OAuth: opens the authorization URL surfaced by a `tool.progress` update
 * (deduplicated per tool-call + URL) so the user can complete an MCP server
 * OAuth flow from the TUI.
 *
