/**
 * TUI2 slash-command dispatch — entry point from handleUserInput.
 *
 * Mirrors `tui/commands/dispatch.ts` with imports converged onto the tui2
 * tree. The `SlashCommandHost` interface keeps the v1 shape (`state:
 * TUIState`) because the tui2 command handlers were migrated against it; the
 * tui2 shell satisfies it via an adapter.
 *
 * Status: REAL (tui2). Mirrors `tui/commands/dispatch.ts`.
 */
...
export interface SlashCommandHost {
  /** tui2: the minimal shell state shape commands read (the tui2 shell
   *  satisfies it via an adapter over the response store). */
  state: {
    appState: AppState;
  ...
  /** tui2: the response store (present on the tui2 shell; commands that need
   *  to mutate state use it instead of the v1 `state` object). */
  store?: Tui2Store;
