/**
 * `/plan` / `/yolo` / `/auto` / `/compact` / `/editor` / `/theme` /
 * `/model` / `/secondary-model` / `/effort` commands plus the settings,
 * permission, locale, update, experiments and astronaut config pickers.
 *
