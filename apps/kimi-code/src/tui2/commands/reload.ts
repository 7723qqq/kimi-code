/**
 * `/reload` and `/reload-tui` slash commands — reload config, session and TUI
 * runtime settings, and re-apply theme/LaTeX/markdown options.
 *
...
import { t } from '#/i18n';
import { currentTheme, lightColors } from '../theme';
...
export async function handleReloadTuiCommand(host: SlashCommandHost): Promise<void> {
  ...
  host.showStatus(t('tui.statusMessages.reloadTuiConfigReloaded'), 'success');
}
