/**
 * TUI2 banner provider — fetch + select the tips banner.
 *
 * Mirrors `tui/banner/banner-provider.ts` with imports converged onto the
 * tui2 tree. Pure logic — no rendering dependency.
 *
 * Status: REAL (tui2). Mirrors `tui/banner/banner-provider.ts`.
 */
...
import type { BannerDisplay, BannerState } from '../types';
