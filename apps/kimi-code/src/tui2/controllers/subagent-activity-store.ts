/**
 * TUI2 subagent activity store — per-agent activity records feeding the
 * background agent detail view (AgentActivityViewer).
 *
 * Mirrors `tui/controllers/subagent-activity-store.ts` — pure logic with no
 * TUI state or components, so the tui2 version is a straight copy with
 * imports converged onto the tui2 tree.
 *
 ...
 *
 * Status: REAL (tui2). Mirrors `tui/controllers/subagent-activity-store.ts`.
 */
