/**
 * TUI2 plugin update notifier — one-time "update detected" notices for
 * outdated plugins.
 *
 * Mirrors `tui/controllers/plugin-update-notifier.ts` — framework-free logic
 * (SDK session + marketplace utils only), so the tui2 version is a straight
 * copy with imports converged onto the tui2 tree. The `notify` callback is
 * injected by the caller (v1 wired it to a footer notice; tui2 callers can
 * write a store slice or show a dialog).
 *
 * Status: REAL (tui2). Mirrors `tui/controllers/plugin-update-notifier.ts`.
 */
