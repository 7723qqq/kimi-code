/**
 * TUI2 session replay renderer — resume replay → transcript entries.
 *
 * Mirrors `tui/controllers/session-replay.ts`. The v1 renderer mounted
 * pi-tui components (ToolCallComponent, ReplayTurnBoundaryComponent) into
 * the transcript Container; the tui2 version appends transcript entries to
 * the response store and the opentui reconciler renders them.
 *
...
export class SessionReplayRenderer {
   ...
   * `renderRecords` because the tool call entries only exist once the
  ...
  private hydrateBackgroundState(agent: ResumedAgentState): void {
    const { store, sessionEventHandler } = this.host;
    const projection = replayBackgroundProjection(agent.background, store.state.availableModels);
  ...
  private renderUserMessage(context: ReplayRenderContext, message: ContextMessage): void {
    ...
    if (message.origin?.kind === 'shell_command') {
      ...
      if (message.origin.phase === 'input') {
        ...
        this.host.appendTranscriptEntry({
          ...replayEntry(context, 'user', `$ ${cmd}`, 'plain', {
            bullet: '',
          }),
          color: 'shellMode',
        });
        ...
        this.host.appendTranscriptEntry({
          ...replayEntry(context, 'status', out.text, 'plain'),
          color: out.color,
        });
      }
      return;
    ...
    if (message.origin?.kind === 'system_trigger') {
      if (message.origin.name === 'goal_continuation') {
        ...
        this.advanceTurn(context);
        this.host.appendTranscriptEntry(
          replayEntry(context, 'user', '', 'plain', { bullet: '' }),
        );
      }
      return;
    }
  ...
  private renderPlanReviewResult(
    ...
    const detail =
      result.feedback !== undefined && result.feedback.length > 0
        ? t('tui.statusMessages.replayFeedback', { feedback: result.feedback })
        : undefined;
  ...
  private removeToolCall(toolCallId: string): void {
    ...
    store.setState('transcript', (entries) =>
      entries.filter((entry) => entry.toolCallData?.id !== toolCallId),
    );
  }
    ...
    const meta: BackgroundAgentMetadata = {
      ...
      model:
        task?.model === undefined
          ? undefined
          : modelDisplayName(task.model, this.host.store.state.availableModels[task.model]),
      effort:
