/**
 * TUI2 auth flow controller — login/logout session lifecycle.
 *
 * Mirrors `tui/controllers/auth-flow.ts` with the pi-tui `TUIState` host
 * swapped for the tui2 response store. The host callbacks (setSession,
 * syncRuntimeState, closeSession, …) stay injected — they are runtime
 * capabilities owned by the tui2 shell, not controller logic.
 *
...
export class AuthFlowController {
  ...
  async activateModelAfterLogin(model: string, effort?: string): Promise<void> {
    ...
    const options: MutableCreateSessionOptions = {
      ...
      permission: host.options.startup.auto
        ...
        : host.options.startup.yolo
          ? 'yolo'
          : undefined,
      planMode: host.store.state.planMode ? true : undefined,
    ...
    if (host.store.state.additionalDirs.length > 0) {
      options.additionalDirs = [...host.store.state.additionalDirs];
    }
