...
const grepChip: ChipProvider = (toolCall, result) => {
  ...
  if (stats.files === 0) return t('tui.statusMessages.chipNoMatches');
  if (stats.mode === 'files_with_matches')
    return pluralize(stats.files, 'file', undefined, stats.partial);
...
const globChip: ChipProvider = (toolCall, result) => {
  ...
  if (entries.length === 0) return t('tui.statusMessages.chipNoFiles');
