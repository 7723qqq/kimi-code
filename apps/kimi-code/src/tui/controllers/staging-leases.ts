...
/** Narrowed event shapes consumed by the tracker (see `handleTurnStarted`). */
...
export class StagingLeaseTracker {
  ...
  handleTurnStarted(event: TurnStartedEvent): void {
    ...
    // Exact binding: a turn that echoes the submission's client-chosen prompt
    // id binds that lease directly and skips the origin heuristic (and its
    // ambiguity warning) entirely. The engine protocol may not carry the
    // echo (older engine builds); the heuristic below remains the fallback.
    const promptId = (event as { promptId?: string }).promptId;
    if (promptId !== undefined) {
      const exact = this.leasesBySubmissionId.get(promptId);
