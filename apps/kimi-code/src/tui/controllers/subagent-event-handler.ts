...
function isSubagentLifecycleEvent(event: Event): event is SubagentLifecycleEvent {
  // `startsWith` rather than an explicit event list: the engine emits
  // `subagent.message` as well as the lifecycle events, and an allowlist
  // would drop it at the type guard before the handler's own switch runs.
