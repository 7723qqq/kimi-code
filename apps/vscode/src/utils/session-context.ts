...
export function stringifyContextHistory(history: readonly ContextMessage[]): string {
  ...
  for (const message of history) {
    ...
    const callId =
      message.role === 'tool' && message.toolCallId ? ` (call_id: ${message.toolCallId})` : '';
...
export function isSensitiveFile(fileName: string): boolean {
  ...
  return ['.env', 'credentials', 'secrets', '.pem', '.key', '.p12', '.pfx', '.keystore'].some(
...
function formatPartsMarkdown(parts: string | readonly ContentPart[]): string {
...
function stringifyParts(parts: string | readonly ContentPart[]): string {
  if (typeof parts === 'string') return parts;
  return parts
    .map((part) => {
      if (part.type === 'text') return part.text;
      if (part.type === 'think')
        return part.think.trim() ? `<thinking>\n${part.think}\n</thinking>` : '';
