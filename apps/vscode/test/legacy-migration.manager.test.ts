...
describe('legacy migration manager (discovery and migration coordination)', () => {
  ...
  it('does not duplicate data when the TUI already migrated the same source', async () => {
    ...
    await expect(readFile(existingSession, 'utf-8')).resolves.toContain('imported_from_kimi_cli');
  ...
  it('keeps the migrated target unchanged when an explicit retry is repeated', async () => {
    ...
    await expect(readFile(join(rig.targetHome, 'config.toml'), 'utf-8')).resolves.toBe(firstTarget);
  });
  ...
  it('reports an unparseable legacy config with a manual review action', async () => {
    ...
    expect(result.manualActions).toEqual([expect.stringContaining('review it manually')]);
  });
  ...
  it('returns a failed result with the original filesystem reason when the target is blocked', async () => {
    ...
    expect(result.manualActions).toEqual([expect.stringContaining('Migrate Legacy Data')]);
  });
...
async function writeCorruptLegacySession(sourceHome: string, workDir: string): Promise<void> {
  await mkdir(sourceHome, { recursive: true });
  await writeFile(
