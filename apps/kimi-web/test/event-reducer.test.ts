...
describe('reduceAppEvent sessionUsageUpdated deltas', () => {
  const USAGE = {
    inputTokens: 10,
    outputTokens: 20,
    cacheReadTokens: 30,
  ...
  function assistantMessage(sessionId: string, id: string): AppMessage {
    return {
      id,
      sessionId,
      role: 'assistant',
  ...
  it('folds deltas into the last assistant message and clears on a new one', () => {
    const state = {
      ...createInitialState(),
      sessions: [makeSession('s1', '2026-01-01T00:00:00.000Z')],
      messagesBySession: {
        s1: [assistantMessage('s1', 'm1')],
      },
    };
    ...
    expect(first.messagesBySession['s1']?.[0]?.usage).toEqual({
      inputTokens: 1,
      outputTokens: 2,
      cacheReadTokens: 3,
      cacheCreationTokens: 4,
    });
    expect(first.pendingUsageBySession['s1']).toEqual({
      inputTokens: 1,
      outputTokens: 2,
      cacheReadTokens: 3,
      cacheCreationTokens: 4,
    ...
    // A second delta accumulates onto the same turn/message.
    ...
    expect(second.messagesBySession['s1']?.[0]?.usage).toEqual({
      inputTokens: 6,
      outputTokens: 8,
      cacheReadTokens: 10,
    ...
    // A new assistant message starts a fresh turn and resets the pending pool.
    const nextTurn = reduceAppEvent(
      second,
      { type: 'messageCreated', message: assistantMessage('s1', 'm2') },
      { sessionId: 's1', seq: 3 },
    );
    expect(nextTurn.pendingUsageBySession['s1']).toBeUndefined();
    expect(nextTurn.messagesBySession['s1']?.[1]?.usage).toBeUndefined();
  });

  it('does not attach deltas to a trailing user message and updates usage only', () => {
    const state = {
      ...createInitialState(),
      sessions: [makeSession('s1', '2026-01-01T00:00:00.000Z')],
      messagesBySession: {
    ...
    expect(next.sessions[0]?.usage).toEqual(USAGE);
    expect(next.messagesBySession['s1']?.[0]?.usage).toBeUndefined();
    // Pending is kept so a subsequent assistant message can pick it up — but a
    // fresh assistant message resets it, so nothing leaks across turns.
    expect(next.pendingUsageBySession['s1']).toEqual({
      inputTokens: 1,
      outputTokens: 2,
      cacheReadTokens: 3,
      cacheCreationTokens: 4,
