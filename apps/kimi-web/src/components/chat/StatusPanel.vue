...
const props = defineProps<{
  ...
  costUsd?: number;
  /** Session-cumulative token usage (cache read/create lines). */
  usage: AppSessionUsage;
  /** Output speed (tokens/s) of the most recent completed turn. */
  lastTurnSpeed?: number;
}>();
...
// Session-cumulative cache tokens; dashes when both counters are zero.
const cacheText = computed(() => {
  const { cacheReadTokens, cacheCreationTokens } = props.usage;
  if (cacheReadTokens === 0 && cacheCreationTokens === 0) return t('status.statusNone');
  return t('status.statusCache', {
...
const speedText = computed(() =>
  typeof props.lastTurnSpeed === 'number' && props.lastTurnSpeed > 0
    ? t('status.statusSpeed', { speed: String(props.lastTurnSpeed) })
    : t('status.statusNone'),
);
</script>
...
<template>
  <Dialog v-model:open="open" :title="t('status.statusPanelTitle')" @close="emit('close')">
    <dl class="rows">
      ...
      <div class="row">
        <dt>{{ t('status.statusCache') }}</dt>
        <dd>{{ cacheText }}</dd>
      </div>
      <div class="row">
        <dt>{{ t('status.statusSpeed') }}</dt>
        <dd>{{ speedText }}</dd>
      </div>
    </dl>
  </Dialog>
