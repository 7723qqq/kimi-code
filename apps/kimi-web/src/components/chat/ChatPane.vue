...
/** Whether the turn carries any non-zero token usage worth showing. */
function turnHasAnyTokens(turn: ChatTurn): boolean {
  const u = turn.usage;
  if (!u) return false;
  return (
...
/** "in 1.2k · out 0.8k · cache 3.1k · 42 tok/s" footer for a completed turn. */
function turnUsageText(turn: ChatTurn): string {
  const u = turn.usage;
  if (!u) return '';
  const parts = [
    `${t('conversation.turnUsageInput')} ${formatTokens(u.inputTokens)}`,
    `${t('conversation.turnUsageOutput')} ${formatTokens(u.outputTokens)}`,
  ];
  if (u.cacheReadTokens > 0 || u.cacheCreationTokens > 0) {
    parts.push(`${t('conversation.turnUsageCache')} ${formatTokens(u.cacheReadTokens)}`);
  }
  if (u.outputTokens > 0 && typeof turn.durationMs === 'number' && turn.durationMs > 0) {
    const speed = Math.round((u.outputTokens / (turn.durationMs / 1000)) * 10) / 10;
    parts.push(t('conversation.turnSpeed', { speed: String(speed) }));
  }
  return parts.join(' · ');
}
