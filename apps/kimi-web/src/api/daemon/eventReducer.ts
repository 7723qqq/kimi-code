...
export interface KimiClientState {
  ...
  /** Per-session accumulated usage deltas for the assistant turn currently in
   *  flight. Attached to the last assistant message when a usage update arrives
...
export function reduceAppEvent(
  ...
  switch (event.type) {
    ...
    case 'sessionUsageUpdated': {
      ...
      // Fold the delta into the pending turn usage and attach it to the LAST
      // assistant message (the turn footer reads it off the turn). The
      // projector emits step-level deltas while a turn runs; the wire
      // `usage_updated` path carries its own delta. Updates without a delta
      // (snapshot / status refreshes) leave the pending value untouched; it is
      // reset when the next assistant message is created.
      if (event.delta !== undefined) {
        const prev = next.pendingUsageBySession[event.sessionId];
        const pending: TurnUsage =
          prev === undefined
            ? {
                inputTokens: event.delta.inputTokens,
                outputTokens: event.delta.outputTokens,
                cacheReadTokens: event.delta.cacheReadTokens,
                cacheCreationTokens: event.delta.cacheCreationTokens,
              }
            : {
                inputTokens: prev.inputTokens + event.delta.inputTokens,
                outputTokens: prev.outputTokens + event.delta.outputTokens,
                cacheReadTokens: prev.cacheReadTokens + event.delta.cacheReadTokens,
                cacheCreationTokens: prev.cacheCreationTokens + event.delta.cacheCreationTokens,
              };
        next.pendingUsageBySession[event.sessionId] = pending;
        const msgs = next.messagesBySession[event.sessionId] ?? [];
        const last = msgs.at(-1);
        if (last !== undefined && last.role === 'assistant') {
          next.messagesBySession[event.sessionId] = [
            ...msgs.slice(0, -1),
            { ...last, usage: pending },
          ];
        }
    ...
    case 'messageCreated': {
      ...
      // A new assistant message starts a fresh turn: any leftover pending usage
      // belongs to the previous turn, so drop it (it was already attached to
      // the previous turn's last assistant message, or there was nothing to
      // attach it to).
      if (event.message.role === 'assistant') {
        const { [sid]: _droppedPending, ...rest } = next.pendingUsageBySession;
        next.pendingUsageBySession = rest;
      }
