...
/** Per-update token deltas (mirror of `WireSessionUsageDelta`), attached to
 *  `sessionUsageUpdated` so the client can attribute a turn's token usage
 *  without tracking baselines itself. */
export interface AppSessionUsageDelta {
  inputTokens: number;
  outputTokens: number;
  cacheReadTokens: number;
  cacheCreationTokens: number;
...
/** Token usage attributed to one assistant turn (client-side accumulation of
 *  `sessionUsageUpdated` deltas while the turn was in flight). */
