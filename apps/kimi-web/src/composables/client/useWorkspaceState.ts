...
export function useWorkspaceState(rawState: ExtendedState, deps: UseWorkspaceStateDeps) {
  ...
  async function loadInitialSessionsByWorkspace(): Promise<AppSession[] | undefined> {
    ...
    for (const { id: workspaceId } of workspaces) {
      ...
      cursors[workspaceId] = page.items.length > 0 ? page.items.at(-1)!.id : undefined;
  ...
  async function loadMoreSessions(workspaceId: string): Promise<void> {
    ...
    try {
      ...
      rawState.sessionsCursorByWorkspace = {
        ...rawState.sessionsCursorByWorkspace,
        [workspaceId]: page.items.length > 0 ? page.items.at(-1)!.id : beforeId,
      };
  ...
  async function loadAllSessions(): Promise<void> {
    ...
    const result = await listAllSessionsGlobal().catch((error) => {
      console.warn('[kimi-web] loadAllSessions failed; search covers only loaded sessions', error);
      return null;
    });
  ...
  async function addWorkspaceByPath(root: string): Promise<boolean> {
    ...
    try {
      ...
      console.warn('[kimi-web] addWorkspaceByPath failed for', trimmed, error);
      return false;
    }
  }
  ...
  async function createGoal(objective: string): Promise<void> {
    ...
    try {
      ...
      pushOperationFailure('createGoal', error, {
        sessionId: sid,
        message: goalErrorMessage(error),
      });
      return;
    }
  ...
  function controlGoal(action: 'pause' | 'resume' | 'cancel'): void {
    ...
    void Promise.resolve(getKimiWebApi().updateSession(sid, { goalControl: action })).catch(
      (error) => {
        pushOperationFailure('controlGoal', error, {
          sessionId: sid,
          message: goalErrorMessage(error),
        });
      },
  ...
  async function deleteWorkspace(id: string): Promise<void> {
    ...
    try {
      ...
      console.warn('[kimi-web] deleteWorkspace registry cleanup failed for', id, error);
    }
  ...
  async function readFileContent(path: string): Promise<{
    ...
    try {
      ...
      console.warn('[kimi-web] readFileContent failed for', path, error);
      return null;
    }
  }
