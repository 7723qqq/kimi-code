...
/** Full usage record of the active session (null when none selected). */
const activeSessionUsage = computed<AppSessionUsage | null>(() => {
  const activeSession = rawState.sessions.find((s) => s.id === rawState.activeSessionId);
  return activeSession?.usage ?? null;
});

/** Output speed (tokens/s) of the most recent completed assistant turn that
 *  carries token usage, for the /status panel. Undefined when nothing
 *  qualifies yet (no usage data or the turn had zero output tokens). */
const lastTurnSpeed = computed<number | undefined>(() => {
  for (let i = turns.value.length - 1; i >= 0; i--) {
    ...
    if (output > 0 && typeof durationMs === 'number' && durationMs > 0) {
      return Math.round((output / (durationMs / 1000)) * 10) / 10;
    }
  }
  return;
});
