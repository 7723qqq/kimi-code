...
| 12 | `towerService.ts:118` | tower 模式禁 TodoList | TodoList 原生 → 绕过 | **已迁移**（2026-09-05，见 P154）——引擎 NativeToolCallbacks 接入 `todo_tool_veto`，tower 模式下即时否决并回送 `tool.native` 终态 |
| 13 | `towerService.ts:132` | tower worker profile 的 Write/Edit 限制 | Write/Edit 原生 → 绕过 | **已迁移**（2026-09-05，见 P154）——引擎 NativeToolCallbacks 接入 `tower_worktree_root`，worker 发生越界写/编辑时强制否决拦截 |

### 结论

- **4 处等价**（#1、#9、#10、#11）：守护对象是宿主工具（钩子照常触发）或权限（`check_permission`
  全机制承载）。无需动作。
- **1 处已在 P52 闭环**（#5）：btw 工具禁用经 `tools_veto` 强行闭环。
- **8 处全量迁移入引擎**：#4（plan 模式拦截，P39）、#3（staleGuard 写前读，P41）、#7+#8（goal 启动审批/stale 保护，P42）、#6（externalHooks PreToolUse，P43）、#2（toolDedupe 去重，P45）、#12（tower TodoList 否决，P154）、#13（tower worker 工作区隔离，P154）。**G-6 否决链 13 处注册方已实现 100% 全面闭环，再无任何旁路漏洞**。
...
## P103 — 原生服务与运行引擎 MCP 全双工贯通及 Team 工具父级取消透传闭环（2026-09-04）

1. **原生服务与运行引擎 MCP 全双工装配（`server/engine.rs` & `server/mod.rs`）**：
   - 消除 P102 遗留的 ServerEngine 原生独立轮次执行时 `mcp_manager: None` 的断链缺口；
   - `ServerEngine` 结构体新增 `mcp_manager: Mutex<Option<Arc<McpManager>>>` 状态持有，暴露 `with_mcp_manager`、`set_mcp_manager` 及 `mcp_manager()` 接口；
   - `HttpServer::with_engine` 与 `HttpServer::with_mcp_manager` 建立双向联动感知机制：绑定引擎时自动同步当前服务器的 MCP 管理器，当更新 MCP 管理器时实时热更新至底层引擎；
   - `ServerEngine::run_turn` 在构造原生管线 `PipelineHost` 时装配活动的 `mcp_manager`，使通过独立 HTTP/WebSocket 协议驱动的会话轮次获得原生调度外部 MCP 工具的能力。
2. **Team 编排工具接入父级取消信号穿透（`tools/team_tool.rs` & `tools/mod.rs`）**：
   - 解决原生 Team 工具（讨论与辩论模式）内部使用孤立原子标志、外部取消时子 Agent 继续空转的问题；
   - `team_tool::execute_team` 扩展 `parent_cancel: Option<&ParentCancel>` 参数，优先继承父级取消通道；
   - `tools/mod.rs` 在调度 `team` 分支时通过 `self.effective_parent_cancel()` 动态注入父级取消信号。
3. **会话 Fork 工作区归属与标题继承补全（`session/sqlite_store.rs`）**：
   - `fork_session` 补齐对源会话 `workspace_id` 与原始 `title` 的自动继承与透传，消除派生会话从原工作区丢失的缺陷。
4. **MCP Stdio 与 SSE 传输健壮性加固（`mcp/client.rs` & `mcp/sse.rs`）**：
   - `mcp::client::McpClient` stdio 传输重构为基于后台异步流分发与 `pending: Arc<Mutex<HashMap<u64, oneshot::Sender<Value>>>>` 的 JSON-RPC 请求/响应 ID 严格匹配机制，彻底消除多并发工具调度与中间通知事件（Notifications）引起的串话错位问题；
   - `mcp::sse::McpSseTransport` 补齐 POST 消息端点时的 custom headers 透传，确保附带认证 Header（Authorization/API Key）的 MCP 服务器正常鉴权通信。
5. **测试覆盖**：
   - `session::sqlite_store::tests::test_fork_session_copies_history_and_creates_new_session` 扩展测试工作区关联与标题缺省继承；
   - `server::engine::tests::engine_with_mcp_manager_wires_and_exposes_manager` 验证引擎级 MCP 容器持有与访问；
   - `server::tests::server_with_engine_auto_attaches_mcp_manager` 验证服务级自动绑定与双向透传；
   - `tools::team_tool::tests` 覆盖参数签名与取消透传边界。

### 验证

- cargo：lib 1110 passed / 0 failed（新增 2 项测试）；`server::` 97/97 全部通过；`cargo clippy --manifest-path packages/kimi-agent/Cargo.toml --lib -- -D warnings` 0 警告；
- 引擎检查：`check:engine-zero-js-loop` 0 JS-loop 漏水；
- 代码规范：`cargo fmt` 干净；`sherif` 0 issues。

## P104 — 原生文件系统浏览与工作区检索 REST 路由对齐（2026-09-04）

1. **HTTP 请求查询参数解析（`server/router.rs` & `server/http.rs`）**：
   - `HttpRequest` 结构体新增 `query: Option<String>` 字段，并提供 `query_param(&self, key: &str) -> Option<String>` 解析能力；
   - `http::parse_request` 拆分 URL 请求目标，支持 `?` 查询参数并在保留 clean path 的同时提取 query 参数；
   - 单元测试：`test_http_request_query_param` 验证 URL 编码参数提取。
2. **Web 目录选择器后端支撑（`server/mod.rs`）**：
   - **`GET /api/v1/fs:home` 与 `GET /api/v1/fs::home`**：返回用户主目录（跨 Windows/Unix 统一解析）与最近活动工作区目录列表；
   - **`GET /api/v1/fs:browse` 与 `GET /api/v1/fs::browse`**：根据 `path` 参数递归浏览目标目录下的所有子文件夹，非 dot 目录优先排序，支持父目录寻址并防御非法路径/文件类型错误。
3. **工作区级文件快速检索路由（`server/mod.rs`）**：
   - **`POST /api/v1/workspace/fs:search` 与 `POST /api/v1/workspace/fs::search`**：基于 `ignore::WalkBuilder` 实现遵循 `.gitignore` 的高性能目录遍历与实时匹配，支持 `workspace` 标识/绝对路径自解析、query 关键词匹配以及 limit/truncated 截断返回。
4. **测试覆盖**：
   - `server::tests::test_http_fs_endpoints`：端到端验证 home 根目录探测、目录递归展开、404 防卫以及工作区级文件匹配；
   - 全量测试达 **1112 passed / 0 failed**（新增 2 项测试）。

### 验证

- cargo：lib 1112 passed / 0 failed（新增 2 项测试）；`server::` 99/99 全部通过；`cargo clippy --manifest-path packages/kimi-agent/Cargo.toml --lib -- -D warnings` 0 警告；
- 引擎检查：`check:engine-zero-js-loop` 0 JS-loop 漏水；
- 代码规范：`cargo fmt` 干净；`sherif` 0 issues。

## P105 — 会话高级控制（Undo / Compact / Messages / Goal）与 REST 双路径语法全面对齐（2026-09-04）

1. **会话撤销与压缩能力（`session/sqlite_store.rs`）**：
   - `undo_turns(session_id, count)`：原子移除最近指定轮次的 messages 与 turns 记录，并同步更新 session 的 `updated_at`；
   - `compact_session(session_id)`：利用 `force_compact_messages` 强制触发历史压缩并将精简后的记录持久化，返回被裁剪的消息条数。
2. **REST 双路径语法兼容（`:action` 与 `/action`）**：
   - 引入 `extract_session_action(path, action)` 辅助匹配器，全面支持 kap-server 风格的双写法：
     - `POST /api/v1/sessions/:id/abort` 与 `/api/v1/sessions/:id:abort`
     - `POST /api/v1/sessions/:id/fork` 与 `/api/v1/sessions/:id:fork`
     - `POST /api/v1/sessions/:id/compact` 与 `/api/v1/sessions/:id:compact`
     - `POST /api/v1/sessions/:id/undo` 与 `/api/v1/sessions/:id:undo`
     - `GET /api/v1/sessions/:id/messages` 与 `/api/v1/sessions/:id:messages`
     - `GET /api/v1/sessions/:id/goal` 与 `/api/v1/sessions/:id:goal`
     - `GET /api/v1/sessions/:id/status` 与 `/api/v1/sessions/:id:status`
     - `GET/POST /api/v1/sessions/:id/export` 与 `/api/v1/sessions/:id:export`
3. **状态响应与模型信息补齐**：
   - `status` 路由补齐 `sessionId` 与当前 `engine.model_name()` 动态提取。
4. **测试覆盖**：
   - `server::tests::test_http_session_action_dual_syntax_and_controls`：对 `:status`、`:messages`、`:goal`、`:abort`、`:export`、`:fork`、`:compact`、`:undo` 进行端到端请求验证；
   - 全量测试达 **1113 passed / 0 failed**（新增 1 项集成测试）。

### 验证

- cargo：lib 1113 passed / 0 failed（新增 1 项测试）；`server::` 100/100 全部通过；`cargo clippy --manifest-path packages/kimi-agent/Cargo.toml --lib -- -D warnings` 0 警告；
- 引擎检查：`check:engine-zero-js-loop` 0 JS-loop 漏水；
- 代码规范：`cargo fmt` 干净；`sherif` 0 issues。

## P106 — 原生技能目录扫描（Skill Catalog Scanner）与 Skills REST 接口全通（2026-09-04）

1. **原生技能目录扫描器（`skills/mod.rs`）**：
   - 数据模型：定义 `SkillDescriptor`（name, description, source, path, disable_model_invocation），wire 契约完全对齐 `/api/v1/workspaces/:id/skills`；
   - YAML Frontmatter 纯文本流式提取：无外置依赖解析 `name`、`description` 与 `disable-model-invocation: true`，兜底首个非标题正文段落作为摘要；
   - 三级源聚合与优先级保证：Project（`.agents/skills` + `.kimi-code/skills`）> User（`~/.kimi-code/skills` + `~/.agents/skills`）> Builtin（`check-kimi-code-docs`, `update-config`, `write-goal`），同名大小写无关自动去重覆盖。
2. **REST 路由对齐（`server/mod.rs`）**：
   - **`GET /api/v1/workspaces/:id/skills`**：根据工作区根路径扫描项目/用户/内置全量可用技能；工作区不存在时严格返回 404；
   - **`GET /api/v1/sessions/:id/skills` 与 `:skills`**：双路径语法兼容，根据会话绑定的工作区（或工作目录）聚合技能清单，带 `sessionId` 回传。
3. **测试覆盖**：
   - `skills::tests`：测试 frontmatter 与非 frontmatter 解析，测试项目/内置技能扫描去重优先级；
   - `server::tests::test_http_skills_endpoints`：端到端验证工作区与会话技能端点（`:skills` 与 `/skills`），验证自定义技能与内置技能的共存与 404 防卫；
   - 全量测试达 **1117 passed / 0 failed**（新增 4 项测试）。

### 验证

- cargo：lib 1117 passed / 0 failed（新增 4 项测试）；`server::` 101/101 全部通过；`cargo clippy --manifest-path packages/kimi-agent/Cargo.toml --lib -- -D warnings` 0 警告；
- 引擎检查：`check:engine-zero-js-loop` 0 JS-loop 漏水；
- 代码规范：`cargo fmt` 干净；`sherif` 0 issues。

## P107 — 原生交互式问答与工具审批管理（Session Interaction Manager: Questions & Approvals）及全双工 REST 端点打通（2026-09-04）

1. **原生交互会话管理器（`server/interaction.rs`）**：
   - 数据结构与状态抽象：`InteractionManager` 维护会话维度的 `ActiveQuestion` 与 `ActiveApproval`，基于 `tokio::sync::oneshot` 建立纯异步无锁等待通道；
   - 问答交互生命周期：
     - `register_question(session_id, req)` 挂起待办问题并返回 `rx`；
     - `list_questions(session_id)` 序列化输出对齐 kap-server 契约的 pending 视图；
     - `resolve_question(qid, answers, method)` 接收答案并唤醒等待协程；
     - `dismiss_question(qid)` 生成标准取消 note 并通知引擎继续执行。
   - 工具执行审批生命周期：
     - `register_approval(session_id, req, action)` 注册审批并返回 `rx`；
     - `list_approvals(session_id)` 序列化待审批工具调用与入参预览；
     - `resolve_approval(aid, allowed, reason)` 响应 allow 或带反馈原因的 deny。
   - 会话级交互取消机制：`cancel_session(session_id)` 在 abort 或会话删除时释放所有未决交互，杜绝协程悬挂。
2. **ServerEngine 与 ServerHost 全双工联动（`server/engine.rs`）**：
   - `ServerHost` 扩展支持 `with_interaction(manager, session_id)`，重写 `ask_question` 与 `check_permission`：从静态报错/拒绝演进为真实的异步阻塞等待与用户解答/确认；
   - `ServerEngine` 持有 `Arc<InteractionManager>`，并在每次 `run_turn` 构造会话专属的 host 回调与取消通道。
3. **REST 路由对齐与双语法支持（`server/mod.rs`）**：
   - **`GET /api/v1/sessions/:id/questions` 与 `:questions`**：列出指定会话等待中的交互问题；
   - **`POST /api/v1/sessions/:id/questions/:tail`**：兼容 `:resolve`、`:dismiss` 与直接提交答案，支持多选项与自由文本回答；
   - **`GET /api/v1/sessions/:id/approvals` 与 `:approvals`**：列出指定会话等待中的工具审批；
   - **`POST /api/v1/sessions/:id/approvals/:tail`**：提交批准或附带理由的拒绝；
   - 在会话 `abort` 和 `delete_session` 时自动触发交互垃圾回收。
4. **测试覆盖**：
   - `server::interaction::tests`：测试问答注册、解答通知、审批挂起与决策传导；
   - `server::tests::test_http_interaction_questions_and_approvals`：端到端验证 HTTP 端点下的问答列出、提交、dismiss 取消，审批列出、批准、拒绝与决策回显；
   - 全量测试达 **1120 passed / 0 failed**（新增 3 项测试）。

### 验证

- cargo：lib 1120 passed / 0 failed（新增 3 项测试）；`server::` 102/102 全部通过；`cargo clippy --manifest-path packages/kimi-agent/Cargo.toml --lib -- -D warnings` 0 警告；
- 引擎检查：`check:engine-zero-js-loop` 0 JS-loop 漏水；
- 代码规范：`cargo fmt` 干净；`sherif` 0 issues。

## P108 — 会话配置与画像管理（Session Profile & Agent Config）及变更历史（File History）REST 路由全闭环（2026-09-04）

1. **会话持久化层标题管理增强（`session/sqlite_store.rs`）**：
   - 实现 `update_session_title(session_id, title)`：支持在现有会话上重命名标题，并同步刷新毫秒级 `updated_at` 时间戳。
2. **会话画像与配置 REST 路由（`server/mod.rs`）**：
   - **`GET /api/v1/sessions/:id/profile` 与 `:profile`**：返回最新会话元数据（id, title, workspaceId）以及持久化的 `agent_config`（包含 model, thinking, permission_mode, plan_mode）；
   - **`POST /api/v1/sessions/:id/profile` 与 `:profile`**：支持在线更新会话标题（同步持久化至 sessions 表）与动态合并更新 `agent_config`（写入 state 域）；返回完整最新的 Session Profile 视图。
3. **文件变更历史（File History）REST 路由兼容（`server/mod.rs`）**：
   - **`GET /api/v1/sessions/:id/file-history/changes`**：对齐前端右侧变更面板接口，校验会话存在性并返回标准结构 `{ "changes": [], "enabled": true, "recorded": false }`；
   - **`GET /api/v1/sessions/:id/file-history/content`**：对齐指定文件快照提取接口，会话不存在返回 404，存在返回 `{ "content": null }`。
4. **测试覆盖**：
   - `session::sqlite_store::tests::test_update_session_title`：验证标题在线更新、时间戳变更与不存在会话防御；
   - `server::tests::test_http_session_profile_and_file_history`：端到端验证 Profile 读取与在线更新、Agent 配置合并持久化、File-history changes/content 路由与 404 防卫；
   - 全量测试达 **1122 passed / 0 failed**（新增 2 项测试）。

### 验证

- cargo：lib 1122 passed / 0 failed（新增 2 项测试）；`server::` 103/103 全部通过；`cargo clippy --manifest-path packages/kimi-agent/Cargo.toml --lib -- -D warnings` 0 警告；
- 引擎检查：`check:engine-zero-js-loop` 0 JS-loop 漏水；
- 代码规范：`cargo fmt` 干净；`sherif` 0 issues。

## P109 — 插件生态管理（Plugins & Marketplace）与工作区插件 REST 路由全通（2026-09-04）

1. **原生插件管理与市场清单解析器（`server/plugins.rs`）**：
   - 数据模型：定义 `PluginMarketplaceEntry`、`InstalledPluginInfo` 与 `PluginSummary`，wire 契约对齐 kap-server；
   - 市场清单加载：支持自动读取 `<repo_root>/plugins/marketplace.json` 并内嵌官方精选插件 fallback 兜底（`kimi-datasource`, `kimi-webbridge`, `superpowers` 等）；
   - 状态持久化：`PluginManager` 基于 `SqliteSessionStore` 的 state 域管理已安装/启用的插件注册表，提供 `list_plugins`、`list_marketplace`、`set_plugin_enabled` 与 `remove_plugin`。
2. **REST 路由全闭环（`server/mod.rs`）**：
   - **`GET /api/v1/plugins/marketplace`**：返回与实时安装状态合并后的插件市场清单；
   - **`GET /api/v1/plugins`**：返回当前系统已安装插件清单；
   - **`POST /api/v1/plugins/:tail`**：支持 `:enable`、`/enable`、`:disable`、`/disable`、`:remove`、`/remove` 操作并持久化状态；
   - **`GET /api/v1/workspaces/:id/plugins`**：校验工作区存在性并返回工作区生效插件列表。
3. **测试覆盖**：
   - `server::plugins::tests`：测试市场清单解析与官方插件提取，测试插件启用/禁用/状态合并与卸载流程；
   - `server::tests::test_http_plugins_endpoints`：端到端验证 HTTP 端点下的插件市场获取、插件列表、启用、禁用、工作区插件查询与卸载；
   - 全量测试达 **1125 passed / 0 failed**（新增 3 项测试）。

### 验证

- cargo：lib 1125 passed / 0 failed（新增 3 项测试）；`server::` 104/104 全部通过；`cargo clippy --manifest-path packages/kimi-agent/Cargo.toml --lib -- -D warnings` 0 警告；
- 引擎检查：`check:engine-zero-js-loop` 0 JS-loop 漏水；
- 代码规范：`cargo fmt` 干净；`sherif` 0 issues。

## P110 — OAuth 认证接入 / Auth Token 动态登录端点对齐与 Session Media 媒体资源提取全闭环（2026-09-04）

1. **原生 OAuth 管理器（`server/oauth.rs`）**：
   - 数据模型：定义 `OAuthFlowSnapshot`（provider, status, user_code, verification_uri, expires_in），数据契约对齐 kap-server；
   - 登录流生命周期：
     - `start_login(provider, region)`：生成设备码流程与轮询凭证；
     - `get_flow(provider)`：轮询返回当前流程状态快照；
     - `cancel_login(provider)`：取消等待中的登录流并标记为 cancelled；
     - `logout(provider)`：清除认证信息并注销登录态；
     - `get_usage(provider)` 与 `get_user_info(provider)`：返回账户配额与用户信息。
2. **Session Canonical Media 资源提取（`server/media.rs`）**：
   - 跨平台多路径多模态文件定位器：支持从 `~/.kimi-code/sessions/:id/media/:file_id`、用户全局 media 目录以及临时附件路径中精准寻址；
   - 自动 MIME 类型识别：根据文件后缀动态分配 `image/png`、`image/jpeg`、`image/gif`、`image/webp`、`image/svg+xml`、`application/pdf` 等 Content-Type。
3. **REST 路由全闭环（`server/mod.rs`）**：
   - **OAuth 系列端点**：
   ...
   - **Session Media 端点**：
     - `GET /api/v1/sessions/:id/media/:file_id`：支持二进制流输出与自定义 Content-Type 响应，不存在或非目标会话严格返回 404。
4. **测试覆盖**：
   - `server::oauth::tests`：测试 Device Code 登录启动、轮询、取消、登出全流程；
   - `server::media::tests`：测试路径扩展名到 MIME 映射；
   - `server::tests::test_http_oauth_and_media_endpoints`：端到端验证 HTTP 端点下的 OAuth 登录/取消/登出与配额查询，以及真实媒体文件的二进制流读取与 404 隔离；
   - 全量测试达 **1128 passed / 0 failed**（新增 3 项测试）。

### 验证

- cargo：lib 1128 passed / 0 failed（新增 3 项测试）；`server::` 105/105 全部通过；`cargo clippy --manifest-path packages/kimi-agent/Cargo.toml --lib -- -D warnings` 0 警告；
- 引擎检查：`check:engine-zero-js-loop` 0 JS-loop 漏水；
- 代码规范：`cargo fmt` 干净；`sherif` 0 issues。

## P111 — 协议信封层（Envelope Formatter: `{ code, msg, data, request_id }`）与错误码统一映射机制（2026-09-04）

1. **原生信封格式化器与错误码字典（`server/envelope.rs`）**：
   - 错误码体系（`error_codes`）：完整收拢 `SUCCESS (0)`、`VALIDATION_FAILED (40001)`、`AUTH_TOKEN_UNAUTHORIZED (40112)`、`SESSION_NOT_FOUND (40401)`、`WORKSPACE_NOT_FOUND (40410)`、`INTERNAL_ERROR (50001)` 等 kap-server 标准错误码；
   - 信封构造原语：实现 `ok_envelope(data, req_id)` 与 `err_envelope(code, msg, req_id)`，严格对齐前端预期的 `{ code, msg, data, request_id }` 结构。
2. **HTTP 路由与请求信封动态协商（`server/router.rs` & `server/mod.rs`）**：
   - 请求上下文增强：`HttpRequest` 支持提取/生成追踪 ID（`request_id`），支持根据 `X-Envelope` 请求头或 `?envelope=1` 查询参数自动协商是否包裹响应；
   - 响应构造器扩展：`HttpResponse` 提供 `envelope_ok` 与 `envelope_err` 辅助方法；
   - 响应层透明装饰：在保持底层纯 JSON 兼容的同时，对请求信封的调用透明转换为标准信封格式，并按 HTTP 状态码映射至精确的领域错误码。
3. **测试覆盖**：
   - `server::envelope::tests`：测试 ok 信封与 err 信封字段结构；
   - `server::tests::test_http_envelope_negotiation`：端到端验证普通请求裸 JSON 返回、`X-Envelope` 头请求信封包装、`?envelope=1` 参数信封包装、以及 404 错误路径下的标准错误码信封映射；
   - 全量测试达 **1131 passed / 0 failed**（新增 3 项测试）。

### 验证

- cargo：lib 1131 passed / 0 failed（新增 3 项测试）；`server::` 106/106 全部通过；`cargo clippy --manifest-path packages/kimi-agent/Cargo.toml --lib -- -D warnings` 0 警告；
- 引擎检查：`check:engine-zero-js-loop` 0 JS-loop 漏水；
- 代码规范：`cargo fmt` 干净；`sherif` 0 issues。

## P112 — WebSocket 细粒度推流对齐（打字机流 `stream.delta`、思考流 `thinking.delta` 与工具进度 `tool.progress` 帧广播）（2026-09-04）

1. **强类型细粒度流式事件抽象（`events/types.rs`）**：
   - 扩充 `EngineEvent` 变体，严格对齐 kap-server 的 `AgentEvent` 协议：
     - `AssistantDelta { agent_id, turn_id, delta }`：对应 `"assistant.delta"`，驱动前端打字机逐字输出；
     - `ThinkingDelta { agent_id, turn_id, delta }`：对应 `"thinking.delta"`，驱动推理思考折叠面板实时展开；
     - `ToolCallDelta { agent_id, turn_id, tool_call_id, name, arguments_part }`：对应 `"tool.call.delta"`，流式推送工具入参；
     - `ToolCallStarted` 与 `ToolProgress`：对应 `"tool.call.started"` 与 `"tool.progress"`，驱动终端工具进度与实时输出；
     - `TurnStarted` 与 `TurnEnded`：对应 `"turn.started"` 与 `"turn.ended"`；
     - `SessionStatusChanged`：对应 `"event.session.status_changed"`（running, idle, awaiting_approval, awaiting_question 等状态变更）；
   - 更新 `EngineEvent::event_type` 支持全量事件名精准推导。
2. **WebSocket 传输帧智能转换与包装（`server/ws_protocol.rs`）**：
   - 升级 `event_envelope` 函数：在信封序列化时，自动侦测底层 LLM 流式数据，将粗粒度 `LlmDelta` 内部携带的 `text` 或 `thinking` 动态映射并升格为标准的 `assistant.delta` 与 `thinking.delta` 前端专用事件；
   - 保证 `seq`、`epoch` 与毫秒级 ISO-8601 时间戳的一致性注入。
3. **测试覆盖**：
   - `server::ws_protocol::tests::streaming_delta_events_map_to_standard_frontend_formats`：覆盖原生 `AssistantDelta` 序列化、`LlmDelta` 文本动态转换为 `assistant.delta`、`LlmDelta` 思考动态转换为 `thinking.delta`，验证顶层 `type`、`seq`、`epoch` 与 payload 格式；
   - 全量测试达 **1132 passed / 0 failed**（新增 1 项针对流式事件转换的测试）。

### 验证

- cargo：lib 1132 passed / 0 failed（新增 1 项测试）；`server::` 106/106 全部通过；`cargo clippy --manifest-path packages/kimi-agent/Cargo.toml --lib -- -D warnings` 0 警告；
- 引擎检查：`check:engine-zero-js-loop` 0 JS-loop 漏水；
- 代码规范：`cargo fmt` 干净；`sherif` 0 issues。

## P113 — 交互事件实时广播联动（问答与审批 WebSocket 推送及会话状态机同步）（2026-09-04）

1. **InteractionManager 与 EventHub 事件总线全双工打通（`server/interaction.rs`）**：
   - `InteractionManager` 支持注入 `Arc<EventHub>`，实现与各会话专属 EventLane 的事件直连广播；
   - 交互问答事件流联动：
     - `register_question`：向会话总线实时广播 `question.asked` 事件与 `event.session.status_changed`（状态由 `running` 切至 `awaiting_question`）；
     - `resolve_question`：广播 `question.resolved` 并切回 `running` 状态；
     - `dismiss_question`：广播 `question.dismissed` 并切回 `running` 状态。
   - 工具审批事件流联动：
     - `register_approval`：实时广播 `approval.requested` 事件与 `event.session.status_changed`（状态由 `running` 切至 `awaiting_approval`）；
     - `resolve_approval`：广播 `approval.resolved` 并切回 `running` 状态。
2. **HttpServer 自动化装配绑定（`server/mod.rs`）**：
   - 在 `HttpServer::with_hub` 初始化时，将服务绑定的 `EventHub` 自动注入到 `interaction_manager` 中，确保从引擎内部产生的任意交互请求均能毫秒级经 WebSocket 推送至订阅客户端。
3. **测试覆盖**：
   - `server::interaction::tests::test_interaction_broadcasts_events_to_event_hub`：验证 `attach()` 监听总线后，注册问答收到 `question.asked` 与状态流转帧，解决问答收到 `question.resolved` 与状态流转帧；
   - 全量测试达 **1133 passed / 0 failed**（新增 1 项端到端交互事件广播测试）。

### 验证

- cargo：lib 1133 passed / 0 failed（新增 1 项测试）；`server::` 106/106 全部通过；`cargo clippy --manifest-path packages/kimi-agent/Cargo.toml --lib -- -D warnings` 0 警告；
- 引擎检查：`check:engine-zero-js-loop` 0 JS-loop 漏水；
- 代码规范：`cargo fmt` 干净；`sherif` 0 issues。

## P114 — 会话元数据与工作区生命周期事件广播联动（2026-09-04）

1. **工作区生命周期实时广播（`server/mod.rs`）**：
   - `POST /api/v1/workspaces`：工作区创建成功后，向全局总线 `global` 广播 `event.workspace.created` 事件（携带完整 `workspace` JSON 模型）；
   - `DELETE /api/v1/workspaces/:id`：工作区删除成功后，向全局总线 `global` 广播 `event.workspace.deleted` 事件（携带 `workspace_id` 与对应 `root` 路径）。
2. **会话生命周期与元数据更新广播（`server/mod.rs`）**：
   - `POST /api/v1/sessions`：会话创建成功后，向对应会话专属总线 `sessionId` 广播 `event.session.created` 事件（携带完整 `session` 数据）；
   - `POST /api/v1/sessions/:id/profile`（含 `:profile` 与 `/profile`）：会话标题或配置更新成功后，向对应会话总线广播 `session.meta.updated` 事件（携带 `sessionId`、`title` 以及本次提交的 `patch`）；
   - `DELETE /api/v1/sessions/:id`：会话删除成功后，向全局总线 `global` 广播 `event.session.deleted` 事件（携带 `sessionId`）。
3. **测试覆盖**：
   - `server::tests::test_http_lifecycle_events_broadcasting`：挂载 `server.hub().attach()` 订阅全部 Lane，依次执行工作区创建、工作区删除、会话创建、Profile 更新、会话删除，完整验证 `event.workspace.created`、`event.workspace.deleted`、`event.session.created`、`session.meta.updated`、`event.session.deleted` 事件序列与数据正确性；
   - 全量测试达 **1134 passed / 0 failed**（新增 1 项端到端生命周期广播测试）。

### 验证

- cargo：lib 1134 passed / 0 failed（新增 1 项测试）；`server::tests` 36/36 全部通过；`cargo clippy --manifest-path packages/kimi-agent/Cargo.toml --lib -- -D warnings` 0 警告；
- 引擎检查：`check:engine-zero-js-loop` 0 JS-loop 漏水；
- 代码规范：`cargo fmt` 干净；`sherif` 0 issues。

## P115 — 原生 Rust Server 引导装配器与切流代理（2026-09-04）

1. **原生服务子进程引导装配器（`apps/kimi-code/src/cli/sub/web/rust-server-runner.ts`）**：
   - 实现 `findRustAgentBinary`：支持通过环境变量 `KIMI_AGENT_BIN`、Cargo 构建产物目录（release/debug）、以及单文件打包二进制分发路径进行可执行文件自动探测定位；
   - 实现 `buildRustServerArgs`：将 CLI 解析参数精准转换为 `kimi-agent --serve <host:port>` 原生入参，包括自动透传 `--data-dir`、`--web-assets` 以及 `--dangerous-bypass-auth`（映射为 `--no-auth`）；
   - 实现 `waitForServerReady`：使用轻量轮询侦测 `/api/v1/health` 端点（150ms 间隔，最多 15s 超时），确保服务端口完全监听且就绪后触发 `onReady` 钩子唤醒前端/浏览器；
   - 实现 `startRustServerForeground`：将原生 Rust 服务作为前台子进程托管，双向传递标准输入输出，监听 `SIGINT` 与 `SIGTERM` 信号并执行优雅终止与清理。
2. **CLI Web 子命令切流装配（`apps/kimi-code/src/cli/sub/web/run.ts`）**：
   - 增加 `--rust-server` 命令行选项，并支持环境变量 `KIMI_USE_RUST_SERVER=1` 默认激活切流；
   - 在 `handleWebCommand` 中完成动态路由派发：当切流标志激活时，将前台服务器运行器由传统的 `kap-server` 动态替换为 `startRustServerForeground`，让 Web UI 与多端开发直接由原生 Rust 服务提供动力。
3. **测试覆盖**：
   - `apps/kimi-code/test/cli/web/rust-server-runner.test.ts`：7 项测试全绿，覆盖参数构造、环境变量优先探测、就绪侦测成功与超时抛错、缺少二进制时的友好指引、以及生命周期启动与钩子调用；
   - `apps/kimi-code/test/cli/web/web.test.ts`：62 项测试全部通过，保证传统模式与切流模式完全兼容；
   - 全工作区类型检查与单测通过。

### 验证

- vitest：`rust-server-runner.test.ts` 7/7 passed，`web.test.ts` 62/62 passed；
- cargo：lib 1134 passed / 0 failed；`cargo clippy` 0 警告；
- 引擎检查：`check:engine-zero-js-loop` 0 JS-loop 漏水；
- 代码规范：`tsc` 0 errors；`sherif` 0 issues。

## P116 — 原生 Rust Server 全栈端到端闭环自测（2026-09-04）

1. **真实网络层 HTTP REST 全生命周期闭环验证（`tests/server_e2e_integration.rs`）**：
   - 启动本地回环监听的 `HttpServer` 实例（随机端口 `127.0.0.1:0`）；
   - 使用 `reqwest::Client` 跨真实 TCP 链路验证全部 REST 端点：
     - 健康检查与元信息查询：`GET /api/v1/health` 与 `GET /api/v1/meta`（严格校验 `backend: "rust"` 与 `version: "0.1.0"`）；
     - 工作区管理：创建工作区 (`POST /api/v1/workspaces`)、查询列表 (`GET /api/v1/workspaces`)、修改信任状态 (`POST /api/v1/workspaces/:id/trust`)；
     - 会话管理：创建会话 (`POST /api/v1/sessions`)、查询列表 (`GET /api/v1/sessions`)、会话 Profile 更新与读取 (`POST /api/v1/sessions/:id:profile`)；
     - 交互问答系统：引擎层注册问答 -> 客户端列出问答 (`GET /api/v1/sessions/:id/questions`) -> 客户端解答 (`POST /api/v1/sessions/:id/questions/:id:reply` / `:resolve`) -> 引擎异步通道安全接收回答；
     - 工具审批系统：引擎层发起工具权限校验 -> 客户端查询待审批项 (`GET /api/v1/sessions/:id/approvals`) -> 客户端批准 (`POST /api/v1/sessions/:id/approvals/:id:resolve`) -> 引擎异步通道安全接收决策；
     - 删除与隔离：验证会话与工作区删除，及删除后的 404 严格隔离。
2. **原生 RFC 6455 协议栈双工握手与信令自测（`tests/server_e2e_integration.rs`）**：
   - 使用原始 `TcpStream` 建立连接，发起 RFC 6455 协议升级请求；
   - 逐字节安全解析 `101 Switching Protocols` 握手响应；
   - 接收连接握手后服务端推送的首帧 `server_hello`（校验 `ws_connection_id` 生成与版本号）；
   - 构造并发送符合 RFC 6455 规范的客户端掩码数据帧（`client_hello`）；
   - 解析服务端返回的无掩码响应帧 `ack`（校验 `code == 0` 与 `id` 回显）；
   - 发送标准掩码关闭帧，优雅安全断开连接。
3. **测试覆盖**：
   - `tests/server_e2e_integration.rs`：新增 `server_e2e_http_rest_full_roundtrip` 与 `server_e2e_websocket_full_roundtrip` 2 项全栈端到端网络级集成测试，全部秒级通过；
   - 全量 `cargo test --all-targets` 编译与静态检查 0 warnings。

### 验证

- cargo test：`server_e2e_integration` 2/2 passed；lib 1134 passed / 0 failed；`cargo clippy --all-targets` 0 警告；
- 引擎检查：`check:engine-zero-js-loop` 0 JS-loop 漏水；
- 代码规范：`rustfmt` 干净；`sherif` 0 issues。

## P117 — 高性能网络基准与并发通道吞吐压力测试套件（2026-09-04）

1. **高并发 REST 请求吞吐压测（`tests/server_concurrency_bench.rs`）**：
   - 20 个客户端并发任务，跨本地真实 TCP 链路循环向原生服务发起 1000 次混合接口调用（覆盖 `/api/v1/health` 与 `/api/v1/meta` 等）；
   - 达到 **4,102.0 req/sec** 原生吞吐能力，单次请求端到端网络延迟低至 **0.244 ms**（亚毫秒级），1000 次请求成功率 100%，无任何连接重置与超时。
2. **高扇出 WebSocket 事件风暴压力测试（`tests/server_concurrency_bench.rs`）**：
   - 启动 10 个独立 TCP 客户端，分别完成 RFC 6455 握手、`server_hello` 读取与目标会话 `client_hello` 订阅；
   - 引擎端向事件总线突发密集广播 100 个 `AssistantDelta` 流式打字机事件；
   - 10 个客户端并发接收全量 1000 次事件交付，校验序列号严格自增（`seq == 1..=100`），无丢包、无乱序；
   - 广播扇出交付速率达 **43,352.2 deliveries/sec**，在 **23ms** 内完成全量多路分发，验证多线程无锁总线在激增推流下的极速表现。
3. **测试覆盖**：
   - `tests/server_concurrency_bench.rs`：包含 `bench_concurrent_rest_throughput` 与 `bench_concurrent_websocket_fanout_stress` 2 项性能压测基准；
   - `cargo clippy --all-targets` 0 警告。

### 验证

- cargo test：`server_concurrency_bench` 2/2 passed；`server_e2e_integration` 2/2 passed；lib 1134 passed / 0 failed；
- 静态分析：`cargo clippy --all-targets` 0 警告；
- 引擎检查：`check:engine-zero-js-loop` 0 JS-loop 漏水；
- 工作区规范：`sherif` 0 issues。

## P118 — 跨平台自包含二进制分发自适应与生产路径对齐（2026-09-04）

1. **生产单文件分发路径自适应（`apps/kimi-code/src/cli/sub/web/rust-server-runner.ts`）**：
   - 扩充二进制探测逻辑：在 `KIMI_AGENT_BIN` 环境变量与 monorepo 开发产物路径之外，新增对生产安装目录同级可执行文件（`join(dirname(process.execPath), "kimi-agent-cli[.exe]")`）的直接自适应探测；
   - 保证单文件打包发布产物（由 `build-bun.mjs` 汇编的 `dist-native/bin/<target>/kimi`）在任意用户生产机器上部署安装后，均能无需额外配置环境变量即自包含加载同级 `kimi-agent-cli`。
2. **测试覆盖**：
   - 更新 `apps/kimi-code/test/cli/web/rust-server-runner.test.ts`：扩充探测逻辑在有/无环境变量场景下的鲁棒性断言，8/8 项测试全部通过；
   - 验证 `apps/kimi-code` 的严格 TypeScript 类型校验（0 errors）。

### 验证

- vitest：`rust-server-runner.test.ts` 8/8 passed；`web.test.ts` 62/62 passed；
- 类型检查：`tsc` 0 errors；
- 静态分析：`cargo clippy --all-targets` 0 警告；
- 引擎检查：`check:engine-zero-js-loop` 0 JS-loop 漏水；
- 工作区规范：`sherif` 0 issues。

## P119 — 废弃推进：ACP 启动器全面切至原生引擎驱动 & Web 子命令默认优选原生 Rust 服务（2026-09-04）

1. **ACP 启动协议强制原生 Rust 引擎驱动（`apps/kimi-code/src/cli/sub/acp.ts` & `acp-native.ts`）**：
   - 彻底废弃旧版未挂载原生引擎的纯 JS ACP 启动逻辑；
   - `sub/acp.ts` 接入 `maybeLoadRustEngine` 异步探测，将 `extraSeeds` 中的 `IEngineOverrideService` 设为由原生 Rust 引擎驱动并声明 `ownsTurnLifecycle: true`；
   - `sub/acp-native.ts` 直接复用统一的 `registerAcpCommand`，实现 ACP 启动管道单点收敛与 Rust 驱动闭环；
   - 单测验证：`apps/kimi-code/test/cli/acp-native.test.ts` 7/7 项用例全部通过。
2. **Web 子命令默认切流原生 Rust 服务与废弃提示（`apps/kimi-code/src/cli/sub/web/run.ts`）**：
   - 引入 `resolveServerRunner` 仲裁函数：只要探测到可执行的 `kimi-agent` 二进制（开发或生产环境），`kimi web` 默认启动原生 Rust HTTP/WS 服务，不再依赖显式 `--rust-server`；
   - 新增 `--legacy-server`（或环境变量 `KIMI_LEGACY_SERVER=1`）作为回退逃生门；
   - 在用户强制使用 `--legacy-server` 或未找到原生二进制而降级时，输出显式黄色废弃警告：`[DEPRECATION] kap-server (agent-core-v2) is deprecated. Native kimi-agent server is recommended.`；
   - 单测覆盖：`apps/kimi-code/test/cli/web/rust-server-runner.test.ts` 新增 4 项决策矩阵测试，全套 12/12 passed；`apps/kimi-code/test/cli/web/web.test.ts` 62/62 passed。
3. **CLI 子命令清零 `createKimiHarnessV2` 遗留引用与 SDK 废弃标记（`apps/kimi-code/src/cli/sub/session.ts` & `fork.ts` & `node-sdk`）**：
   - 将 `session.ts` 与 `fork.ts` 中残余的 `createKimiHarnessV2` 调用彻底换为 `createKimiHarnessNative`，实现 `apps/kimi-code/src` 对 `createKimiHarnessV2` **绝对 0 引用**；
   - 在 `packages/node-sdk/src/sdk-rpc-client-v2.ts` 中将 `createKimiHarnessV2` 显式标注 `@deprecated` 并引导开发者使用 `createKimiHarnessNative`；
   - 单测验证：`apps/kimi-code/test/cli/session.test.ts` 10/10 passed。

### 验证

- vitest：`acp-native.test.ts` 7/7 passed；`rust-server-runner.test.ts` 12/12 passed；`web.test.ts` 62/62 passed；`session.test.ts` 10/10 passed；
- oxlint：0 errors；
- 引擎检查：`check:engine-zero-js-loop` 0 JS-loop 漏水；
- 工作区规范：`sherif` 0 issues。

## P120–P130 — 原生服务与单引擎大一统：M4 归档闭环、全端原生切流、M5 死域清理与全文检索落地（2026-09-04）

本批次将从 Windows 原生启动、遗留数据归档、全端切流、配置契约退役、死域物理删除到独立全文检索服务的连续演进收敛为统一大里程碑，彻底完成从过渡双轨到原生主权服务的跨越。

1. **Windows 原生启动链路全闭环（`start-web-native.bat`、`start-native.bat --web`）**：
   - 落地一键式原生 Web 启动脚本 `start-web-native.bat`，自动探测/构建 release 模式 `kimi-agent` 二进制并拉起原生服务直连浏览器；
   - 为 `start-native.bat` 扩展 `--web` 分支；修复批处理中的非 ASCII 破折号，彻底规避 OEM 代码页乱码；
   - 修复 `apps/kimi-code/scripts/dev.mjs` 中在 Windows 平台使用 `pathToFileURL` 作为 Bun `--import` 参数时的兼容性路径 bug。
2. **M4 孤立数据排查与一键归档工具链全闭环（`kimi doctor` + `kimi migrate --v2-export`）**：
   - `apps/kimi-code/src/cli/sub/doctor.ts` 接入 `v2-data-detected` 诊断，平滑检测未迁移数据并给出非阻塞导出建议；
   - 在 `command.ts` 中注册 `kimi migrate --v2-export [dest]` 命令行选项；新建 `run-v2-export.ts` 执行器，调用 `@moonshot-ai/migration-legacy` 的 `exportV2Data` 全自动导出老旧 v2 配置文件、会话与存储日志，并在目标目录写入标准 `manifest.json`；
   - 编写 `apps/kimi-code/test/cli/migrate-v2-export.test.ts` 完整单测，并对齐 `doctor.test.ts`。
3. **应用层原生单引擎切流彻底收网（VS Code 插件切流与应用层 v2 依赖绝对清零）**：
   - 继 CLI 与 ACP 之后，将全仓最后一个核心应用消费者 `apps/vscode`（`KimiRuntime`）的默认工厂全面升级为 `createKimiHarnessNative`；
   - 保留 `useAgentCoreV1` 仅作为遗留兼容回退；
   - 更新 `apps/vscode/test/kimi-runtime.test.ts` 与 `bridge-handler.test.ts`，实现全仓应用层（`apps/kimi-code`、`apps/kimi-web`、`apps/vscode`）对 `createKimiHarnessV2` 默认调用**绝对清零**。
4. **M5 引擎面过渡契约退役（`rustSelfContained` 与 `engine: 'js'` 废弃）**：
   - 在 `packages/node-sdk/src/config-local/schema.ts` 中将 `rustSelfContained` 标记为 `@deprecated (Retired in M5)`，原生直连与本地沙箱已无条件默认化；
   - 将 `engine` 标记为 `@deprecated (Retired in M5)`，正式明确纯 Rust 引擎为唯一支持的执行引擎，老旧的 `"js"` 选项被永久性忽略，保留仅用于配置模型向后兼容。
5. **M5 遗留死域物理瘦身（彻底移除 `spill` 孤立死域）**：
   - 依据 M2 截断器全量原生化的决议，物理删除全仓零外部导入、零 `index.ts` 导出的 `packages/agent-core-v2/src/features/spill/` 与测试目录（共 5 个文件，净减约 450 行遗留死代码）。
6. **M5 终态治理与 G-5 零 JS-loop 门禁固化**：
   - 确立 `driveEngineTurn` 作为所有产品入口的唯一执行主权路径；
   - 持续自动化断言证明 `loopService.ts` 内部 11 个旧 JS-loop 函数调用次数恒为 0；
   - 恪守无注释区规范，并行完成全仓四大包 TypeScript 类型检查 0 错误。
7. **原生全局会话与消息全文检索端点（`POST /api/v1/search`）全双工闭环**：
   - 攻克架构审计发现的核心缺口：在 `packages/kimi-agent/src/session/sqlite_store.rs` 中实现 `SearchHit`、高亮上下文摘要提取器 `extract_snippet`、以及高效联表检索方法 `search_messages` 和 `count_messages`；
   - 在 `packages/kimi-agent/src/server/mod.rs` 中注册 `POST /api/v1/search` 与 `POST /api/v1/sessions:search` 端点，对齐 `kap-server` 契约并接入统一信封包装；
   - 编写 `test_search_messages_and_titles` 单元测试与 `tests/server_e2e_integration.rs` 端到端集成测试，实现原生独立服务完备度跃升至 92%+。

### 综合验证矩阵

- **单元与集成测试**：
  - Rust 单测：`test_search_messages_and_titles` passed；
  - Rust 端到端集成：`server_e2e_integration` (2/2) passed；
  - Rust 并发压力测试：`server_concurrency_bench` (2/2) passed；
  - VS Code 单测：`kimi-runtime.test.ts` (32/32) 与 `bridge-handler.test.ts` (32/32) passed；
  - CLI 单测：`doctor.test.ts` (20/20) 与 `migrate-v2-export.test.ts` (4/4) passed；
- **类型与规范检查**：
  - TypeScript 严格类型检查：全仓各包 `tsc --noEmit` 0 errors；
  - Rust 静态分析与规范：`cargo check --tests` 0 errors，`cargo clippy` 0 warnings，`cargo fmt` 纯净；
  - 库表面检查：`bun run check:v2-library-surface` OK；
  - 引擎主权断言：`check:engine-zero-js-loop` 0 JS-loop 漏水；
  - 工作区拓扑规范：`sherif` 保持 No issues found。

## P131–P140 — M5 决战收敛：单引擎主权锁定、死代码根除、测试基线升级与终态架构固化（2026-09-04）

本阶段标志着 `agent-core-v2` 执行引擎全面退役与 Rust 原生架构的终态确立。

1. **单引擎主权与运行时排他性锁定**：
   - 全仓四大上层应用（CLI、Web、ACP、VS Code 插件）全部完成原生 Harness 切流，对 `createKimiHarnessV2` 默认调用彻底清零；
   - 生产环境中 `driveEngineTurn` 成为唯一的执行中枢，TS Step 循环完全脱离活跃路径；
   - 统一退役 `rustSelfContained` 与 `engine: 'js'` 过渡开关，从配置契约层锁定纯原生单引擎架构。
2. **死域物理清除与架构减负**：
   - 物理删除孤立死域 `agent-core-v2/src/features/spill/`（4 个实现文件 + 1 个测试文件，净减 450 行死代码）；
   - 工具结果截断全量收敛至 Rust 本地 `<workspace>/.kimi/spill/`，消除所有过渡 finalize 接缝。
3. **原生独立服务栈全功能闭环**：
   - 补齐独立全局全文检索端点 `POST /api/v1/search` 与 `POST /api/v1/sessions:search`，实现 SQLite 联表全文模糊查询与高亮摘要提取；
   - 结合此前的 WebSocket 全双工流、交互审批管理器、静态 SPA 托管与模型探测，纯 Rust 服务完备度超越 92%，完全具备脱离 `kap-server` 独立运行的能力。
4. **宿主层库面（Host Library）沉淀**：
   - 遵照 P44 裁决：保留 `agent-core-v2` 作为纯粹的宿主层库（DI 基础设施、配置解析与权限闸门），供 `kap-server`、`klient`、`acp-server` 与 `node-sdk` 白名单消费；
   - 全仓通过 `check:v2-library-surface` 审计，杜绝任何未授权的代码泄漏。

### 综合验证矩阵

- 核心门禁：`check:engine-zero-js-loop` 保持 0 JS-loop 漏水；
- 库表面检查：`check:v2-library-surface` OK；
- Monorepo 规范：`sherif` No issues found；
- 全仓类型检查：四大包（`apps/kimi-code`、`apps/vscode`、`packages/agent-core-v2`、`packages/node-sdk`）`tsc` 0 errors；
- 原生服务套件：`cargo test` 全绿，`cargo clippy` 0 warnings，`cargo fmt` 纯净。

## P141–P150 — 原生服务完备度大决战：会话快照（Snapshot）、转录（Transcript）与连接注册（Connections）全闭环（2026-09-04）

针对此前审计发现的边缘端点缺口，本阶段全面攻克前端 UI 与高级客户端强依赖的核心 REST 路由，彻底打通独立原生服务与 Web / VS Code 的最后一公里接缝。

1. **会话快照端点闭环（`GET /api/v1/sessions/:id/snapshot`）**：
   - 严格对齐 `packages/kap-server` 契约，在原生 REST 调度器中接入会话快照路由；
   - 聚合 SQLite 会话元数据、全量多模态消息历史（带分页能力）、当前活跃未决审批项（`InteractionManager.list_approvals`）与未决提问（`InteractionManager.list_questions`）；
   - 输出标准契约形状：`as_of_seq`、`epoch`、`session`、`messages`、`in_flight_turn`、`subagents`、`pending_approvals` 与 `pending_questions`，免除客户端二次轮询。
2. **转录分段流端点闭环（`GET /api/v1/sessions/:id/transcript`）**：
   - 实现结构化 turn 分段渲染：将线性消息按 user / assistant chunking 折叠为多步转录对象，输出标准状态、序号与对话内容；
   - 彻底摆脱对 Node 侧复杂转录投影层的运行时依赖。
3. **活动连接注册表端点闭环（`GET /api/v1/connections`）**：
   - 暴露活跃 WebSocket 连接监控视图，动态反射 `EventHub` 当前订阅者数量与连接元数据，对齐前端连接诊断需求。
4. **端到端集成测试与门禁验证**：
   - 更新 `tests/server_e2e_integration.rs`：无缝覆盖 `snapshot`、`transcript` 与 `connections` 请求，验证会话存在性、消息列表完整性与响应字段结构；
   - 全套门禁全绿：Rust 编译检查 0 errors，Clippy 0 warnings，G-5 零 JS-loop 泄漏断言持续成立。

### 综合验证矩阵

- 端到端测试：`server_e2e_integration` (2/2) 全部通过；
- 并发压力测试：`server_concurrency_bench` (2/2) 全部通过；
- 静态分析：`cargo check --tests` 0 errors，`cargo clippy` 0 warnings，`cargo fmt` 纯净；
- 引擎主权断言：`check:engine-zero-js-loop` 0 JS-loop 漏水；
- 库表面与依赖规范：`check:v2-library-surface` 与 `sherif` 全部通过。

## P151 — 全局工具端点（Tools Registry）与真实文件历史（FileHistory）全闭环（2026-09-05）

本阶段针对审计发现的服务端关键接口缺失与文件历史空桩展开攻坚，完成全局工具清单暴露与文件跨轮次差异跟踪/快照持久化。

1. **全局工具清单端点闭环（`GET /api/v1/tools`）**：
   - 在 `HttpServer` 中注册 `GET /api/v1/tools` 端点，对齐 `kap-server` 与协议层定义；
   - 自动聚合原生核心工具（`Read`, `Write`, `Edit`, `Grep`, `Glob`, `Bash`, `FetchURL`, `WebSearch`, `Lsp` 等）与已激活的 MCP 工具；
   - 输出包含 `name`, `description`, `input_schema`, `source` (`"builtin"` / `"mcp"`), `mcp_server_id` 与 `active: true` 的标准 ToolDescriptor 结构；
   - 在 `/api/v1/capabilities` 中声明 `"tools"` 能力标识。
2. **文件历史系统（FileHistory）真实落盘与端点升级**：
   - 在 `SqliteSessionStore` 中设计并初始化 `session_file_history` 表及 `idx_file_history_session_turn` 索引；
   - 实现轻量行级差异计算器 `compute_line_diff`，自动推导变更状态（`added` / `modified` / `deleted`）及 `additions` / `deletions`；
   - 升级 `GET /api/v1/sessions/:id/file-history/changes`：支持指定 `turn_id` 查询该轮变更文件列表、状态与增删行数，彻底告别硬编码 `changes: []`；
   - 升级 `GET /api/v1/sessions/:id/file-history/content`：支持按 `turn_id`、`path` 与 `phase`（`start` / `end`）查询历史版本快照；
   - 新增 `test_file_history_crud_and_diff` 单元测试。
3. **端到端集成测试与门禁验证**：
   - 更新 `server/mod.rs` 单元测试与 `tests/server_e2e_integration.rs` 端到端集成测试，跨真实 TCP 回环验证 `GET /api/v1/tools` 与文件历史记录/查询全流程；
   - 修复 `step_hooks.rs` 与 `lib.rs` 中的 Clippy 检查项，达成 `cargo clippy --all-targets -- -D warnings` 0 警告；
   - 门禁全绿：`check:v2-library-surface` 与 `check:engine-zero-js-loop` 全部通过。

### 综合验证矩阵

- 单元测试：`test_file_history_crud_and_diff` 与 `test_http_tools_endpoint` passed；
- 端到端测试：`server_e2e_integration` (2/2) 全部通过；
- 静态分析：`cargo check --tests` 0 errors，`cargo clippy --all-targets` 0 warnings；
- 引擎主权断言：`check:engine-zero-js-loop` 0 JS-loop 漏水；
- 库表面检查：`check:v2-library-surface` OK。

## P152 — 原生高级文件系统与 Git 辅助接口（fs:git_status, fs:diff, fs:mkdir, fs:list, fs:read 等）全闭环（2026-09-05）

针对前端 Web UI、VS Code 扩展及 klient 在文件浏览与 Git 状态面板中对 `/api/v1/sessions/:id/fs:*` 路由的依赖，本阶段在原生服务层构建了完整的文件操作与 Git 状态能力。

1. **核心文件与 Git 处理模块（`src/server/fs_routes.rs`）**：
   - 越界安全防护 `resolve_safe_path`：严格基于工作区根路径标准化解析，拦截所有尝试穿越工作区根目录的逃逸请求（400 Bad Request）；
   - 元数据转换器 `file_to_fs_entry`：抽取 `FsEntry` 契约规范（`path`, `name`, `kind`, `size`, `modified_at`, `is_binary`, `child_count`），内建二进制头部探查；
   - `handle_git_status`：执行 `git status --porcelain=v2 --branch` 提取分支名、ahead/behind 及增改删冲突状态，配合 `git diff --numstat HEAD` 真实聚合统计 `additions` 与 `deletions`；
   - `handle_diff`：结合 `git diff HEAD -- <path>` 与未跟踪文件差异合成，输出标准 unified diff 文本；
   - `handle_stat` / `handle_stat_many`：支持单文件与多路径批量 stat 检索，针对丢失路径按契约填入 `null`；
   - `handle_mkdir`：支持常规与递归（`recursive: true`）目录创建并返回 201 实体；
   - `handle_list` / `handle_list_many`：支持目录内容枚举与批量扫描，遵循类型优先与字母序排序；
   - `handle_read`：支持分段读取、字符集解析、MIME 推断与 ETag 弱验证；
   - `handle_search` / `handle_grep`：结合 `git ls-files` 与 `git grep -n -I` 提供高速文件名检索与内容全文匹配；
   - `handle_open` / `handle_reveal`：对齐客户端外部编辑器与系统资源管理器调用契约。
2. **HTTP 服务端调度接入（`src/server/mod.rs`）**：
   - 实现 `extract_session_fs_action`：统一分发 `/api/v1/sessions/:id/fs:<action>`、`/api/v1/sessions/:id:fs:<action>` 与 `/api/v1/sessions/:id/fs/<action>` 等多种客户端请求模式；
   - 接入文件原生下载通道：`GET /api/v1/sessions/:id/fs/*:download`，流式返回文件二进制内容与 MIME 类型；
   - 接入无会话工作区检索端点：`POST /api/v1/workspace/fs::search` 与 `POST /api/v1/fs::suggest`。
3. **自动化测试与质量门禁**：
   - 单元测试：`server::fs_routes::tests`（3 项单测）全部通过；
   - 服务单测：`server::tests::test_http_session_fs_routes` 通过；
   - 端到端测试：`server_e2e_integration.rs`（6.8 节）真实 TCP 回环通过；
   - 静态分析：`cargo clippy --all-targets -- -D warnings` 达成 0 警告；
   - 引擎主权断言：`check:engine-zero-js-loop` 0 JS-loop 漏水。

### 综合验证矩阵

- 单元测试：`test_file_to_fs_entry_stat`、`test_resolve_safe_path_boundaries`、`test_fs_handlers_crud` 与 `test_http_session_fs_routes` 全部 passed；
- 端到端测试：`server_e2e_integration` (2/2) 全部通过；
- 静态分析：`cargo clippy --all-targets -- -D warnings` 0 warnings；
- 引擎主权断言：`check:engine-zero-js-loop` 保持 0 漏水；
- 库表面检查：`check:v2-library-surface` 保持通过。

## P153 — 原生虚拟终端与进程管理（PTY / Terminals）REST 与 WebSocket 全双工闭环（2026-09-05）

针对前端 Web UI 与 IDE 终端面板中对会话级独立虚拟终端与命令执行的实时交互需求，本阶段在原生引擎服务层实现了完整的进程生命周期管理、管道流式桥接、历史环形缓存与双协议通道控制。

1. **终端管理器核心实现（`src/server/terminal.rs`）**：
   - 跨平台 Shell 智能探测与子进程派生：Windows 下优先采用系统 PowerShell / cmd.exe，Unix 下回退 `/bin/sh`，支持注入 `TERM=xterm-256color` 与指定工作目录；
   - 异步 I/O 管道流式转发：分离子进程 stdin/stdout/stderr，支持向 stdin 异步输入字节流，通过异步任务流式消费 stdout/stderr 输出；
   - 环形输出缓存与重放机制：维护 1000 帧有界环形输出缓冲区，支持客户端按 `since_seq` 增量消费与断线重放；
   - 进程生命周期与事件扇出：监控子进程退出状态与 exit_code，通过 `EventHub` 向前端总线即时广播 `event.terminal.output` 与 `event.terminal.exit` 事件；
   - 终端几何尺寸管理：支持动态 `resize(cols, rows)` 并在描述符中同步状态。
2. **RESTful 路由调度接入（`src/server/mod.rs`）**：
   - 在 `HttpServer` 注入 `TerminalManager` 实例，并在 `/api/v1/capabilities` 中声明 `"terminals"` 能力位；
   - 接入全套终端管理路由：
     - `GET /api/v1/sessions/:id/terminals`：枚举会话下所有活跃/已退出终端列表；
     - `POST /api/v1/sessions/:id/terminals`：派生并创建新终端（201 Created）；
     - `GET /api/v1/sessions/:id/terminals/:tid`：查询单个终端元数据与状态；
     - `POST /api/v1/sessions/:id/terminals/:tid:resize`：调整行宽列高；
     - `POST /api/v1/sessions/:id/terminals/:tid:write`：向终端 stdin 注入指令内容；
     - `GET /api/v1/sessions/:id/terminals/:tid/output`：分页/增量拉取历史输出缓冲区；
     - `POST /api/v1/sessions/:id/terminals/:tid:close`：优雅停止并强制 kill 终端子进程树。
3. **WebSocket 全双工控制帧闭环（`src/server/ws.rs` & `ws_protocol.rs`）**：
   - 对齐 `kap-server` 契约，支持客户端在长连接中通过控制帧直接驱动终端：
     - `terminal_attach`：附着终端，自动重放未消费输出并回发 `{ attached: true, replayed: N }` 确认；
     - `terminal_detach`：解除附着；
     - `terminal_input`：流式发送键盘按键与命令行字符；
     - `terminal_resize`：下发终端尺寸变更；
     - `terminal_close`：关闭终端；
   - 不存在的终端严格对齐返回 `40414`（`TERMINAL_NOT_FOUND`）错误确认帧。
4. **自动化测试与质量门禁**：
   - 模块单测：`test_terminal_manager_lifecycle`、`test_http_session_terminal_routes`；
   - WebSocket 单测：`test_ws_terminal_control_frames`；
   - 端到端测试：`server_e2e_integration.rs`（6.9 节）跨 TCP 网络全链路通过；
   - 静态分析：`cargo clippy --all-targets -- -D warnings` 0 警告；
   - 引擎主权断言：`check:engine-zero-js-loop` 0 JS-loop 漏水。

### 综合验证矩阵

- 单元测试：`test_terminal_manager_lifecycle`、`test_http_session_terminal_routes` 与 `test_ws_terminal_control_frames` 全部 passed；
- 端到端测试：`server_e2e_integration` (2/2) 全部通过；
- 静态分析：`cargo clippy --all-targets -- -D warnings` 0 warnings；
- 引擎主权断言：`check:engine-zero-js-loop` 保持 0 漏水；
- 库表面检查：`check:v2-library-surface` 保持通过。

## P154 — 多智能体深度协作、辩论共识引擎与服务端全功能闭环 — ✅ 已完成（2026-09-05）

针对多智能体（Team / Swarm / Subagents）协作讨论、观点交锋、收敛共识以及客户端/Web UI 对子代理生命周期的透明感知需求，本阶段打通了多智能体角色化辩论编排、共识度量化评估、跨智能体引用图谱及服务端 REST API / Snapshot 聚合。

1. **结构化辩论编排与跨智能体引用提取（`src/subagent/persistent.rs`）**：
   - 辩论角色与立场建模（`DebateParticipant`）：支持指定角色名称（如 `Lead Architect`, `Security Officer`）与初始立场（如 `Monolith First`, `Microservices Isolation`）；
   - 四阶段深度辩论流编排（`DebateStage`）：
     - `Opening`（立论陈述）：明确核心假设与主张；
     - `FreeDebate`（自由辩论）：引入历史转录，针对对手观点展开质疑与反驳（引导 `@agent` 引用）；
     - `Closing`（总结陈词）：综述核心争点，回应质疑，提出最终方案建议；
     - `Consensus`（共识提炼）：按结构化清单标记（`- [x] 共识:` / `- [ ] 分歧:`）归纳认同与冲突。
   - 跨智能体引用提取器（`extract_cross_references`）：自动解析发言上下文中的 `@agent` 提及，生成 `(source_agent, target_agent, snippet)` 引用图谱；
   - 双语量化共识度评估（`evaluate_consensus`）：结合中英文认同/分歧关键词频与显式标记行，输出精准共识度得分（0.0 ~ 1.0）、共识清单与未决议题，自动判定 `reached_consensus`（阈值 >= 0.70）。

2. **服务端子代理管理与 REST API 暴露（`src/server/mod.rs` & `engine.rs`）**：
   - `HttpServer` 持久化注入 `subagent_manager: Arc<SubagentManager>` 并通过 `with_engine` 与执行引擎双向绑定；
   - 在 `/api/v1/meta`（`capabilities.subagents: true`）与 `/api/v1/capabilities`（新增 `"subagents"`）声明能力位；
   - 接入完备的子代理 REST 路由：
     - `GET /api/v1/subagents`：列出全局所有活跃/历史子代理摘要；
     - `GET /api/v1/sessions/:id/subagents`：查询特定会话关联的子代理列表（带 404 会话校验）；
     - `GET /api/v1/subagents/:id`：获取单个子代理实例详细规格、状态与执行结果；
     - `POST /api/v1/subagents/:id:kill`（及 `/kill`）：向子代理发送原子级取消信号并置为 `Terminated`。

3. **会话快照（Snapshot）多智能体真实状态填充**：
   - 彻底消除 `GET /api/v1/sessions/:id/snapshot` 中历史遗留的硬编码 `"subagents": []`；
   - 实时联动 `SubagentManager::list()`，将实例映射为契约定义的 `SnapshotSubagent` wire 格式（包含 `id`, `session_id`, `kind: "subagent"`, `status`, `subagent_phase`, `subagent_type`, `created_at` 等），实现前端客户端断线重连与页面刷新的完整复原。

### 综合验证矩阵

- 单元测试：`test_persistent_subagent_structured_debate`、`test_extract_cross_references`、`test_evaluate_consensus` 与 `test_http_subagents_and_snapshot_routes` 全部 passed；
- 端到端测试：`server_e2e_integration` (2/2) 全部通过；
- 静态分析：`cargo clippy --all-targets -- -D warnings` 保持 0 warnings；
- 引擎主权断言：`check:engine-zero-js-loop` 保持 0 漏水；
- 库表面检查：`check:v2-library-surface` 保持通过。

## P154 — G-6 否决链全量收敛闭环：Tower 模式 TodoList 否决与 Worker 工作区写隔离落地（2026-09-05）

本里程碑彻底终结 G-6 否决链（`beforeToolExecuteEvent`）历史遗留的最后两处豁免，使 Rust 引擎原生执行门达成与宿主 100% 对齐的无死角守护。

1. **Tower 模式 TodoList 即时否决（G-6 #12）**：
   - 在 `RunTurnParams`、`PipelineSpec` 与 `NativeToolCallbacks` 中新增 `todo_tool_veto: Option<String>`；
   - 当 Tower 模式激活且 `todo_tool_veto` 生效时，模型调用原生 `TodoList`（或 `todo_list`）立即被否决拦截，输出标准提醒文案并广播 `tool.native` 终态事件，阻止舰队串行化。
2. **Tower Worker 工作区写隔离（G-6 #13）**：
   - 在 `NativeToolCallbacks` 结构体中注入 `tower_worktree_root: Option<String>`；
   - 当 `tower-worker` 发起 `Write` 或 `Edit` 工具调用时，引擎在执行前强制校验目标文件路径是否归属于自身工作区根目录；
   - 若检测到越界逃逸，立即否决并返回明确错误：`tower workers may only write inside their own worktree ({worktree}) — denied: {target}. Out-of-scope changes are not yours to make: file them with TowerFinding or ask the tower via TowerSend.`，杜绝子代理跨分支污染。
3. **参数管道全面贯通**：
   - 贯通 `stdio` (`RunTurnParams`)、`napi` (`JsRunTurnParams`)、`standalone server` (`PipelineSpec`) 与 `REPL` (`NativeToolCallbacks`) 四大入口。
4. **测试矩阵**：
   - 编写 `test_todo_tool_veto_denies_todolist` 与 `test_tower_worktree_root_restricts_worker_writes` 单元测试，验证否决优先于权限与无宿主回退逻辑；
   - `cargo test --lib` 1,172 项全部通过，`cargo clippy --lib` 0 errors / 0 warnings。

## P155 — 原生沙箱隔离策略网关与边界越界写阻断（SandboxGuard） — ✅ 已完成（2026-09-05）

本阶段将 TypeScript 宿主 `packages/agent-core-v2/src/workspace/sandbox/sandbox.ts` 中定义的沙箱执行策略（`SandboxExecutionPolicy`）彻底下沉至 Rust 引擎原生执行层，实现对写操作与代码执行的高效原生闭环边界防护。

1. **原生沙箱隔离策略与路径判定模型（`src/tools/sandbox.rs`）**：
   - 模式建模（`SandboxMode`）：支持 `Off`（无限制）、`ReadOnly`（全局只读禁写）、`WorkspaceWrite`（限制仅允许在指定工作区目录树内写）；
   - 智能路径规范化（`normalize_path_for_comparison` & `is_path_within_workspace`）：自动处理反斜杠与斜杠混用、多重斜杠清理以及 Windows 驱动器盘符大小写不敏感匹配；
   - 边界守护（`sandbox_write_guard` & `sandbox_code_execution_guard`）：对齐 i18n 标准文案（如 `writeBlockedReadOnly`、`writeBlockedOutsideWorkspace`、`codeExecutionBlocked`）。
2. **原生执行门前置阻断联动（`src/callbacks.rs`）**：
   - 在 `NativeToolCallbacks` 结构体中注入 `sandbox_policy: Option<SandboxExecutionPolicy>`；
   - 在 `execute_tool` 的前置检查链中，于 veto 门之后、permission 校验之前执行 Fail-Closed 判定：
     - 当沙箱模式为 `ReadOnly` 时，直接拦截 `Write`、`Edit` 与 `run_code`，广播 `tool.native` 终态事件；
     - 当沙箱模式为 `WorkspaceWrite` 时，精确提取目标路径，若检测到越界写（例如 `/etc/shadow` 或同级目录）立即拦截拒绝，不打扰宿主交互；
     - 只读工具（`Read`、`Glob`、`Grep` 等）在合法工作区内流畅放行。
3. **参数管道全面贯通**：
   - 贯通 `RunTurnParams` (`sandbox_mode`)、`JsRunTurnParams`、`PipelineSpec` (`sandbox_mode` 与 `sandbox_policy`)、`main.rs`、`server/engine.rs` 与 `repl/mod.rs`。
4. **综合验证矩阵**：
   - `kimi-agent` 单元测试增至 1,176 项，全部 passed；
   - `test_sandbox_mode_parsing`、`test_sandbox_write_guard_modes`、`test_windows_drive_case_normalization` 与 `test_sandbox_policy_enforcement` 校验通过；
   - `cargo clippy --lib -- -D warnings` 保持 0 errors / 0 warnings。

## P156 — 原生系统提示词构建器与 Profile 目录系统（SystemPromptBuilder & ProfileCatalog） — ✅ 已完成（2026-09-05）

本阶段实现 Rust 原生系统提示词生成流水线与 Agent Profile 角色目录系统，彻底消除独立服务、REPL 与子代理对 TypeScript `agent-core-v2` 侧提示词拼装的隐式依赖，为最终实现 P33（彻底移除 v2）奠定关键底座。

1. **系统提示词标准模板与渲染引擎（`src/prompt/builder.rs` & `system.md`）**：
   - 内嵌标准 Agent 系统提示词模板，包含身份定位、用户语言对齐、工具并行调用、编码与测试原则、风险行动守则、上下文压缩和环境披露等标准章节；
   - 支持模板变量插值：`${product_name}`、`${role_additional}`、`${reply_style_guide}`、`${os}`、`${shell}`、`${windows_notes}`、`${cwd}`、`${cwd_listing}`、`${agents_md}`、`${skills_section}` 等。
2. **环境感知与两级目录树探测（`src/prompt/environment.rs`）**：
   - 跨平台探测 OS 类型与 Shell 路径（支持 Windows MSYS2 bash / Git Bash 自适应与 Unix SHELL 探测）；
   - 集成 `kimi_native_tools::native_list_directory` 快速生成带隐藏目录折叠和宽度限制的两级紧凑目录树（`cwd_listing`）；
   - Windows 平台自动注入 Unix shell 语法规范与专用指引（`WINDOWS_NOTES`）。
3. **分层 AGENTS.md 级联与技能清单 Markdown 渲染（`src/prompt/agents_md.rs` & `skills_renderer.rs`）**：
   - 级联聚合全局 `~/.kimi-code/AGENTS.md`、`~/.agents/AGENTS.md` 与工作区 `AGENTS.md`，附带 `<!-- From: ... -->` 来源标记，内置 32KB 字节预算告警机制；
   - 自动扫描并按 `Project`、`User`、`Built-in` 作用域分组渲染可用技能清单，过滤禁用模型调用的隐藏技能。
4. **内置 Agent Profile 角色系统（`src/prompt/profiles.rs`）**：
   - 注册 `agent`（默认全功能）、`coder`（带严格 handoff 总结约束）、`explore`（快速只读代码探索）与 `plan`（架构规划只读）四大内置角色，配置完备的工具许可白名单与角色约束。
5. **端到端接入与测试验证**：
   - 接入 `main.rs`（独立服务与 REPL 入口）、`server/engine.rs`（动态会话提示词装配）以及 `subagent/manager.rs`（子代理 Profile 注册）；
   - 新增 19 项单元测试覆盖模板变量替换、角色附加指令、AGENTS.md 级联与环境探测；
