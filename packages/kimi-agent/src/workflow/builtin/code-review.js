...
const filesToReview = scope?.files ?? fileContents.map(f => f.path);
const context = scope?.context ?? 'Unknown project context';
...
const allFindings = [
  ...
  ...(perfReview?.findings ?? []),
  ...(correctnessReview?.findings ?? []),
  ...(maintainabilityReview?.findings ?? []),
];
...
return {
  ...
  summaries: { security: securityReview?.summary ?? '', performance: perfReview?.summary ?? '', correctness: correctnessReview?.summary ?? '', maintainability: maintainabilityReview?.summary ?? '' },
  recommendations: report?.recommendations ?? ['Address critical and high-severity findings first.', 'Run automated linting and type checking in CI.'],
};