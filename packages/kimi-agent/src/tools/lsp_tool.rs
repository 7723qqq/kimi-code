...
pub async fn execute_lsp_tool(workspace_root: &Path, args: &Value) -> Option<ExecutableToolResult> {
    ...
    let server_bin = match LanguageServerManager::locate_server(language_id) {
        ...
        None => {
            return Some(ExecutableToolResult {
                ...
                stop_turn: false,
                content: format!(
                    "Language server for language '{language_id}' is not installed or not in PATH."
                ),
                is_error: true,
                note: None,
            });
    ...
    let action_res = match action {
        "definition" => {
            ...
            session
                .goto_definition(&target_path, line, character, Duration::from_secs(5))
                .await
        }
        "references" => {
            let line = args.get("line").and_then(|v| v.as_u64()).unwrap_or(0) as u32;
        ...
        "hover" => {
            ...
            let character = args.get("character").and_then(|v| v.as_u64()).unwrap_or(0) as u32;
            session
                .get_hover(&target_path, line, character, Duration::from_secs(5))
                .await
        }
        "symbols" | "document_symbols" => {
            session
                .get_document_symbols(&target_path, Duration::from_secs(5))
                .await
        }
        other => {
            let _ = session.shutdown().await;
            return Some(ExecutableToolResult {
                ...
                stop_turn: false,
                content: format!(
                    "Unsupported LSP action: '{other}'. Currently supported: 'definition', 'references', 'hover', 'symbols'."
                ),
                is_error: true,
                note: None,
            });
    ...
    match action_res {
        Ok(val) => Some(ExecutableToolResult {
            delivery: None,
            stop_turn: false,
...
mod tests {
    ...
    fn test_detect_language_id() {
        ...
        assert_eq!(
            detect_language_id(Path::new("src/index.ts")),
            Some("typescript")
        );
    ...
    async fn test_lsp_tool_unsupported_action() {
        ...
        assert!(res.is_error);
        assert!(
            res.content.contains("Unsupported LSP action") || res.content.contains("not installed")
        );
    }
}
