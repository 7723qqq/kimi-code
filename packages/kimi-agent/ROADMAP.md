...
| #3934（门禁盲区：code-app bundle 同步）2.0.2 web 交互改进与 bug 修复 | **缺口：fork 的 dist-web 停在 2.0.2 之前的同步点**——#3934 把 `CodeBlockNode-BMkbTGvt.js` 改名为 `CodeBlockNode-CGnsnQxn.js` 等，fork bundle 仍是旧文件名，`index.html` 时间戳 09-17 早于该同步（09-19）。按仓库约定 bundle 只能从 code-app 同步，待有 code-app 访问权限的一方重sync | `apps/kimi-code/dist-web/`（521 文件，index.html 2026-09-17）；上游 `2e605b1`（code-app `44d7281c7a`） |
