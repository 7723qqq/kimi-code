...
describe('plan-mode bootstrap from config.defaultPlanMode', () => {
  ...
  it('enters plan mode via core enterPlan and records it on wire', async () => {
    await writeFile(configPath, BASE_CONFIG);
    const rpc = await createTestRpc();
    const created = await rpc.createSession({ workDir });
  ...
  it('survives cancelPlan without any prior plan', async () => {
    await writeFile(configPath, BASE_CONFIG);
    const rpc = await createTestRpc();
    const created = await rpc.createSession({ workDir });
  ...
  it('clears plan without any prior plan', async () => {
    await writeFile(configPath, BASE_CONFIG);
    const rpc = await createTestRpc();
    const created = await rpc.createSession({ workDir });
