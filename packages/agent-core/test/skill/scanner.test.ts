...
// On Windows, node:fs/promises' realpath returns native backslashes, but
// the skill scanner uses pathe internally and emits forward slashes.
// Wrap realpath with pathe.normalize so the test expectations match.
const normalizedRealpath = (p: string): Promise<string> => realpath(p).then(path.normalize);
...
describe('resolveSkillRoots brand home (KIMI_CODE_HOME)', () => {
  it('resolves the user brand skills root under brandHomeDir while keeping generic under the real home', async () => {
    ...
    expect(userRoots).toContain(await normalizedRealpath(path.join(brandHomeDir, 'skills')));
    expect(userRoots).toContain(await normalizedRealpath(path.join(homeDir, '.agents', 'skills')));
  });
  ...
  it('never nests a second .kimi-code under the brand home', async () => {
    ...
    expect(userRoots).not.toContain(
      await normalizedRealpath(path.join(brandHomeDir, '.kimi-code', 'skills')),
    );
  });
  ...
  it('falls back to <userHomeDir>/.kimi-code/skills when brandHomeDir is omitted', async () => {
    ...
    const userRoots = roots.filter((r) => r.source === 'user').map((r) => r.path);
    expect(userRoots).toContain(await normalizedRealpath(path.join(homeDir, '.kimi-code', 'skills')));
  });
});
