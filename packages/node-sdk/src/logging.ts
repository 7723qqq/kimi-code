...
export function resolveLoggingConfig(input: { homeDir: string; env: NodeJS.ProcessEnv }): LoggingConfig {
  const env = input.env;
  const parseLevel = (v: string | undefined): LogLevel | undefined => {
    if (!v) return undefined;
    const s = v.toLowerCase().trim();
    if (s === 'off' || s === 'error' || s === 'warn' || s === 'info' || s === 'debug') return s;
    return undefined;
  };
  const parsePositiveInt = (v: string | undefined): number | undefined => {
    if (!v) return undefined;
  ...
  return {
    ...
    globalMaxBytes: parsePositiveInt(env['KIMI_LOG_GLOBAL_MAX_BYTES']) ?? 6 * 1024 * 1024,
    globalFiles: parsePositiveInt(env['KIMI_LOG_GLOBAL_FILES']) ?? 5,
    sessionMaxBytes: parsePositiveInt(env['KIMI_LOG_SESSION_MAX_BYTES']) ?? 5 * 1024 * 1024,
    sessionFiles: parsePositiveInt(env['KIMI_LOG_SESSION_FILES']) ?? 3,
  };
}
