 ...
 * against {@link KimiHarness}. `prompt` is wired in step 3.4. The remaining
 * `Agent` surface (e.g. `authenticate`) still throws `methodNotFound`.
 */
...
export class AcpServer implements Agent {
   ...
   * (via `KimiHarness.resumeSession`) instead of creating a new one.
   * After the AcpSession is wired up, replays the persisted history as
   * a synchronous batch of `session/update` notifications so the
   * client sees the prior turns before the response settles.
   *
   ...
   */
  private trackSessionStarted(sessionId: string, mode: 'new' | 'load'): void {
 ...
 */
export async function runAcpServerWithStream(harness: KimiHarness, stream: Stream): Promise<void> {
  const conn = new AgentSideConnection((c) => new AcpServer(harness, c), stream);
  await conn.closed;
}
