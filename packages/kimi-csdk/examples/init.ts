 ...
 * (kap-server) over the `/api/v2` channel, using `@moonshot-ai/kimi-csdk`.
 *
 * Flow:
 *   1. read base url / token / default model from the local Kimi home
 *   2. create a session (v1 REST — kimi-csdk is v2-only) pointed at a temp cwd
...
import { CsdkRpcError, KimiCsdkClient } from '@moonshot-ai/kimi-csdk';
...
async function main(): Promise<void> {
  ...
  const cwd = await mkdtemp(join(tmpdir(), 'kimi-csdk-init-'));
  await writeFile(
  ...
  const client = new KimiCsdkClient({ url: baseUrl, token });
...
main().catch((error: unknown) => {
  if (error instanceof CsdkRpcError) {
