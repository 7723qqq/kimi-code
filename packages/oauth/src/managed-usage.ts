 ...
 *   }
 *
 * Windows arrive as used ratios rather than absolute used/limit rows, and
 * every window is optional — the plan the account is on decides which ones
 * the backend serves. The parser normalizes the payload into a structured,
 * camelCase domain model; presentation (labels, reset hints) is left to the
 * consumer.
 */
