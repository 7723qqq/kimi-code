...
output goes to `dist/` (generated by `bun run build`).
...
```bash
bun run build      # compile src/ -> dist/ (JS + .d.ts)
...
> Within this repo, import from `./src/index.ts` (Bun resolves TS directly). As an installed
...
```bash
bun src/server.ts --dir ./data --port 6379
```
...
Measured on Node v24, 100-byte values (`bun run bench`, `N=100000`):
...
| `soak.test.ts` | sustained ops + heap stability (opt-in: `SOAK=30 bun run test`) |
