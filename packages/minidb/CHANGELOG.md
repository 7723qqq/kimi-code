...
- [`3dcaa6b`](https://github.com/MoonshotAI/kimi-code/commit/3dcaa6bb0fe0cea562c56d2708dec056115e8df6) Thanks [@7723qqq](https://github.com/7723qqq)! - Make the open path cooperative: WAL replay, index image loading, and bulk store fills now yield to the event loop in bounded slices instead of blocking, with a `bulkLoading` guard against expiry races and a process-wide bounded text-build worker slot. Open lifecycle telemetry is exposed through `LifecycleTracker`, and cancellation is no longer swallowed into a full index rebuild.
