  ...
  if (!db.readOnly) {
    ...
    if (!got) {
      if (opts.onLockFail === 'readonly') {
        ...
        // first thing a reader of this error needs to know. The detail is
        // BEST-EFFORT: describeHolder() reads the lock line, and a failing
        // read must never replace the LockError with a raw fs error — the
        // lock pool retries `instanceof LockError` (an EACCES here would
        // abort its retry loop) and openOrRebuild classifies on it too.
        let detail = '';
        try {
          ...
          // Losing the annotation is acceptable; losing the LockError type is not.
        }
...
export async function openOrRebuildMiniDb<T>(
  ...
  try {
    ...
    if (hooks.allowDestructiveRebuild !== true) {
      throw new Error(
        `MiniDb.openOrRebuild: ${opts.dir} failed to open with an unrecoverable corruption error (${error instanceof Error ? error.message : String(error)}); refusing to erase it. Repair the directory manually, or pass allowDestructiveRebuild: true only if this database is a rebuildable cache.`,
        { cause: error },
      );
    }
