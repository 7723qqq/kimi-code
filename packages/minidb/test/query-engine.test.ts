...
test('comparison operators $eq $ne $gt $gte $lt $lte', async () => {
  ...
  try {
    assert.deepEqual(
      db.query({ filter: { age: { $eq: 25 } } }).map((r) => r.key),
      ['c'],
    );
    assert.deepEqual(
      db
        .query({ filter: { age: { $ne: 25 } } })
        .map((r) => r.key)
        .toSorted(),
      ['a', 'b', 'd'],
    );
...
test('$in / $nin membership', async () => {
  ...
  try {
    assert.deepEqual(
      db
        .query({ filter: { city: { $in: ['Paris', 'Berlin'] } } })
        .map((r) => r.key)
        .toSorted(),
      ['a', 'c', 'd'],
    );
    ...
    assert.deepEqual(
      db.query({ filter: { city: { $in: 'Paris' as unknown as string[] } } }).map((r) => r.key),
      [],
    );
    assert.deepEqual(
      db.query({ filter: { city: { $nin: 'Paris' as unknown as string[] } } }).map((r) => r.key),
      [],
    );
    await db.close();
  } finally {
...
test('$exists and $type', async () => {
  ...
  try {
    ...
    assert.deepEqual(
      db
        .query({ filter: { active: { $exists: true } } })
        .map((r) => r.key)
        .toSorted(),
      ['a', 'b', 'd'],
    );
    assert.deepEqual(
      db.query({ filter: { active: { $exists: false } } }).map((r) => r.key),
      ['c'],
    );
    assert.deepEqual(
      db
        .query({ filter: { name: { $type: 'string' } } })
        .map((r) => r.key)
        .toSorted(),
      ['a', 'b', 'c', 'd'],
    );
    assert.deepEqual(
      db.query({ filter: { age: { $type: 'string' } } }).map((r) => r.key),
      [],
    );
    await db.close();
  } finally {
...
test('$regex with string, [pattern, flags], RegExp and stateful global RegExp', async () => {
  ...
  try {
    ...
    assert.deepEqual(
      db
        .query({ filter: { name: { $regex: ['^a', 'i'] as unknown as string } } })
        .map((r) => r.key),
      ['a'],
    );
    // RegExp instance form.
    assert.deepEqual(
      db.query({ filter: { name: { $regex: /^B/ } } }).map((r) => r.key),
      ['b'],
    );
    ...
    const g = /^E/g;
    assert.deepEqual(
      db.query({ filter: { name: { $regex: g } } }).map((r) => r.key),
      ['c'],
    );
    // Non-string field never matches a regex.
    assert.deepEqual(
      db.query({ filter: { age: { $regex: '3' } } }).map((r) => r.key),
      [],
    );
    await db.close();
  } finally {
...
test('scalar RegExp value resets lastIndex between documents', async () => {
  ...
  try {
    const g = /^A/g;
    assert.deepEqual(
      db.query({ filter: { name: g } }).map((r) => r.key),
      ['a'],
    );
    // Non-string value against a scalar RegExp never matches.
    assert.deepEqual(
      db.query({ filter: { age: /^3/ } }).map((r) => r.key),
      [],
    );
    await db.close();
  } finally {
...
test('logical operators $and $or $nor $not', async () => {
  ...
  try {
    assert.deepEqual(
      db
        .query({ filter: { $and: [{ city: 'Paris' }, { age: { $gt: 20 } }] } })
        .map((r) => r.key)
        .toSorted(),
      ['a', 'c'],
    );
    assert.deepEqual(
      db
        .query({ filter: { $or: [{ name: 'Ann' }, { name: 'Bob' }] } })
        .map((r) => r.key)
        .toSorted(),
      ['a', 'b'],
    );
...
test('unknown operator and non-array logical args never match', async () => {
  ...
  try {
    ...
    assert.deepEqual(
      db.query({ filter: { age: { $bogus: 1 } as never } }).map((r) => r.key),
      [],
    );
    // $and/$or/$nor with a non-array argument are treated as non-matching.
    assert.deepEqual(
      db.query({ filter: { $and: { city: 'Paris' } as never } }).map((r) => r.key),
      [],
    );
    assert.deepEqual(
      db.query({ filter: { $or: { city: 'Paris' } as never } }).map((r) => r.key),
      [],
    );
    assert.deepEqual(
      db.query({ filter: { $nor: { city: 'Paris' } as never } }).map((r) => r.key),
      [],
    );
    await db.close();
  } finally {
...
test('nested dot + bracket paths in filter and projection', async () => {
  ...
  try {
    ...
    assert.deepEqual(
      db.query({ filter: { 'user.name': 'Bob' } }).map((r) => r.key),
      ['p2'],
    );
    assert.deepEqual(
      db.query({ filter: { 'user.addr.zip': '75001' } }).map((r) => r.key),
      ['p1'],
    );
    // Bracket index into an array element.
    assert.deepEqual(
      db.query({ filter: { 'items[1].sku': 'B' } }).map((r) => r.key),
      ['p1'],
    );
    ...
    const projected = db.query({
      filter: { 'user.name': 'Ann' },
      project: ['user.addr.zip', 'items[0].sku'],
    });
    assert.deepEqual(projected[0]!.value, {
      user: { addr: { zip: '75001' } },
      items: [{ sku: 'A' }],
    });
...
test('sort ascending/descending, skip and limit', async () => {
  ...
  try {
    assert.deepEqual(
      db.query({ sort: { age: 1 } }).map((r) => r.key),
      ['b', 'c', 'a', 'd'],
    );
    assert.deepEqual(
      db.query({ sort: { age: -1 } }).map((r) => r.key),
    ...
    assert.deepEqual(
      db.query({ sort: { age: 1 }, skip: 1, limit: 2 }).map((r) => r.key),
      ['c', 'a'],
    );
