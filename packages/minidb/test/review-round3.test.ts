...
test('non-ASCII key: range index, dt index, compound index, text index', async () => {
  ...
  try {
    ...
    assert.deepEqual(
      db.findRange('byAge', { min: 25, max: 35 }).map((r) => r.key),
      ['用户A'],
    );
    assert.deepEqual(
      db.dtRange('created', { gte: 100, lte: 100 }).map((r) => r.key),
      ['用户A'],
    );
    assert.deepEqual(
    ...
    assert.deepEqual(
      db.search('body', 'world').map((r) => r.key),
      ['用户A'],
    );
    // values resolve too
...
test('non-ASCII key: unified query by exact key and by prefix', async () => {
  ...
  try {
    ...
    assert.deepEqual(
      db.query({ key: 'post:北京' }).map((r) => r.key),
      ['post:北京'],
    );
    const pref = db
      .query({ key: { prefix: 'post:' } })
      .map((r) => r.key)
      .toSorted();
