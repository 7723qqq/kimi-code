...
test(
  'compaction-race: concurrent writes + frequent compaction lose nothing',
  { timeout: 30_000 },
  async () => {
...
test(
  'compaction-race: heavy writes during compaction grow a WAL tail that survives recovery',
  { timeout: 30_000 },
  async () => {
...
test(
  'compaction-race: valueMode disk preserves concurrent writes and remaps pointers',
  { timeout: 30_000 },
  async () => {
    const dir = await tmpDir();
    let db = await MiniDb.open({
