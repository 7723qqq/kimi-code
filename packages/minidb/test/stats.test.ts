...
test('batch writes a single frame (lower write amplification than per-key sets)', async () => {
  ...
  try {
    ...
    assert.ok(
      db.stats.walBytesWritten < individual,
      `batch wrote ${db.stats.walBytesWritten} < ${individual}`,
    );
  } finally {
    await db.close();
    await fs.rm(dir, { recursive: true, force: true });
...
test('recovery stats capture scanned bytes, frames and duration at open', async () => {
  ...
  const reopened = await MiniDb.open({
    dir,
    valueCodec: 'string',
    fsyncPolicy: 'no',
    autoCompact: false,
...
test('compaction phase stats break down into snapshot/rotation/postings/total', async () => {
  ...
  try {
    ...
    for (let i = 0; i < 100; i++)
      await db.set(`k${i}`, { body: `message number ${i} hello world` });
    await db.compact();
    ...
    assert.ok(
      db.stats.compactionSnapshotDurationMs + db.stats.compactionRotationDurationMs <=
...
test('query stats count candidates, decodes and sorted rows', async () => {
  ...
  try {
    ...
    assert.equal(
      db.stats.queryDecoded - d0,
      100,
      'every candidate was decoded to match the filter',
    );
...
test('index rebuild stats: values decoded once per record, 0 without value-derived indexes', async () => {
  ...
  try {
    ...
    assert.equal(
      db.dtRange('created', { gte: 0 }).length,
      20,
      'dt index rebuilt from metadata alone',
    );
  } finally {
    await db.close();
    await fs.rm(dir, { recursive: true, force: true });
  ...
  for (let i = 0; i < 20; i++)
    await db.set(`k${i}`, { n: i, grp: 'g', body: `b${i}`, title: `t${i}` });
  await db.close();
  db = await MiniDb.open({
    dir: dir2,
    valueCodec: 'json',
  ...
  try {
    assert.equal(
      db.stats.indexRebuildDecoded,
      20,
      'one decode per record fanned out to all builders',
    );
