...
export class SearchWorkerHost {
  ...
  private async reapLockFile(token: string | undefined): Promise<void> {
    ...
    try {
      ...
      if (parsed.pid !== process.pid || parsed.token !== token) return;
      // Compare-then-delete (same discipline as recoverOrphanedLock): re-read
      // the line right before removing it — a new owner may have replaced it
      // since the read above, and deleting a live owner's lock would admit a
      // second writer.
