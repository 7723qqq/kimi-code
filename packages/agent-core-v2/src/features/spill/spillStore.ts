 ...
 * local users read spilled tool output or pre-create symlinks; `mkdtemp`
 * gives an unpredictable suffix and 0700 semantics.
 *
 ...
 * JS (UTF-16) strings. A session id / suggested name is untrusted input, so
 ...
 * `.`/`..` are escaped so they can never traverse. An empty string encodes to
 ...
 * return its path + byte length. The filename is a random hex prefix plus the
 * sanitized `suggestedName`, so it is unpredictable (defeats symlink planting
 * in a shared root) AND stays readable. The open is exclusive + owner-only
