...
export const goalKey = defineState('goal', (): GoalModelState => null).replayable({
  ...
  .on(GoalUpdate, (s, e) => {
    ...
    if (e.inputTokensUsed !== undefined && e.inputTokensUsed !== s.inputTokensUsed) {
      s.inputTokensUsed = e.inputTokensUsed;
      changed = true;
    }
    if (e.outputTokensUsed !== undefined && e.outputTokensUsed !== s.outputTokensUsed) {
      s.outputTokensUsed = e.outputTokensUsed;
      changed = true;
    }
    ...
    if (changed) s.updatedAt = e.updatedAt ?? Date.now();
  })
  .on(GoalClear, () => null)
  .on(GoalForked, () => null);
