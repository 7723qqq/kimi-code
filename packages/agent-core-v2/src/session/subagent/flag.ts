...
export const subagentForkFlag: FlagDefinitionInput = {
  ...
  description:
    'Let the Agent and AgentSwarm tools start a subagent with a snapshot of the calling agent\'s conversation history via the fork parameter. The forked subagent shares the caller\'s profile, model, and tool set so the prompt prefix cache is reused.',
