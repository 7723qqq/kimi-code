...
async function fetchSearchPage(query: string, page: number, options: SearchEngineOptions): Promise<string> {
  ...
  if (!response.ok) {
    throw new Error2(ErrorCodes.WEB_FETCH_FAILED, `CSDN search request failed: HTTP ${String(response.status)}.`, {
