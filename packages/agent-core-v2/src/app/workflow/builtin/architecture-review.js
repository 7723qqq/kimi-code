export const meta = {
  ...
  description:
    'Architecture review and dependency analysis — scans module structure, detects circular dependencies, measures coupling/cohesion, identifies layer violations, and generates architecture documentation.',
  whenToUse:
    'Use when evaluating system architecture, detecting design decay, planning modularization, or onboarding new team members to the codebase.',
    ...
    modules: {
      ...
      items: {
        ...
        properties: {
          ...
          type: {
            type: 'string',
            enum: ['entry', 'module', 'util', 'config', 'type', 'test', 'other'],
          },
          exports_count: { type: 'number' },
          imports: { type: 'array', items: { type: 'string' } },
          loc: { type: 'number' },
        },
        required: ['path', 'type', 'exports_count', 'imports', 'loc'],
      },
      minItems: 1,
    },
  ...
  properties: {
    ...
    hub_modules: {
      ...
      items: {
        ...
        required: ['path', 'fan_in', 'fan_out', 'instability'],
      },
      maxItems: 10,
    },
    layer_violations: {
      ...
      items: {
        ...
        required: ['from', 'to', 'violation', 'fix'],
      },
    },
  },
...
const ARCH_REVIEW_SHAPE = {
  ...
  properties: {
    ...
    tech_debt_estimate: { type: 'string' },
  ...
  properties: {
    ...
    layers: {
      ...
      items: {
        ...
        required: ['name', 'purpose', 'modules', 'rules'],
      },
    },
...
try {
  ...
  return { error: `Glob failed for pattern: ${globPattern}` };
}
...
log(
  `Circular deps: ${(depAnalysis && depAnalysis.circular_dependencies && depAnalysis.circular_dependencies.length) || 0}`,
);
...
return {
  stats: {
    ...
    modules_mapped: moduleMap.modules.length,
    entry_points: moduleMap.entry_points,
    layer_suggestion: moduleMap.layer_suggestion,
  },
  ...
  architecture_review: archReview || {
    rating: 'fair',
    strengths: [],
    weaknesses: [],
    recommendations: ['Run a full architecture review with human experts.'],
    tech_debt_estimate: 'Unknown',
    modularization_score: 50,
  },
  documentation: docs || {
    title: 'Architecture Overview',
    overview: 'Generated from automated analysis.',
    layers: [],
    data_flow: 'See architecture review.',
    decisions: 'See architecture review.',
  },
};
