export const meta = {
  name: 'security-audit',
  description:
    'Security vulnerability audit — scans source files for OWASP Top 10 categories, hardcoded secrets, injection points, auth flaws, and dependency risks, producing a prioritized remediation plan.',
  whenToUse:
    'Use when auditing code for security vulnerabilities, before deploying to production, or when onboarding a security review process.',
  ...
  properties: {
    target_files: {
      ...
      items: {
        ...
        properties: {
          path: { type: 'string' },
          risk_exposure: { type: 'string', enum: ['high', 'medium', 'low', 'none'] },
          reasons: { type: 'array', items: { type: 'string' } },
        },
        required: ['path', 'risk_exposure', 'reasons'],
      },
      minItems: 1,
    },
...
const REMEDIATION_SHAPE = {
  ...
  properties: {
    ...
    secure_config_notes: { type: 'string' },
    summary: { type: 'string' },
...
if (Array.isArray(input.paths) && input.paths.length > 0) {
  ...
  try {
    fileList = await glob(input.glob);
  } catch {
    return { error: `Glob failed: ${input.glob}` };
  }
} else {
  try {
    ...
    return { error: 'No files specified and auto-detect failed.' };
  }
}
...
for (const p of selectedFiles) {
  try {
    ...
    sources.push({ path: p, content: content.split('\n').slice(0, MAX_LINES).join('\n') });
  } catch {
    log(`Cannot read: ${p}`);
  }
}
...
let depContent = '';
try {
  depContent = await readFile('package.json');
} catch {}
try {
  if (!depContent) depContent = await readFile('Cargo.toml');
} catch {}
try {
  if (!depContent) depContent = await readFile('requirements.txt');
} catch {}
...
return {
  ...
  remediation: remediation || {
    critical_count: findingsList.filter((f) => f.severity === 'critical').length,
    high_count: findingsList.filter((f) => f.severity === 'high').length,
    medium_count: findingsList.filter((f) => f.severity === 'medium').length,
    low_count: findingsList.filter((f) => f.severity === 'low').length,
    immediate_actions: ['Review findings manually.'],
    remediation_plan: [],
    summary: `Security audit found ${findingsList.length} issues.`,
    overall_risk: findingsList.some((f) => f.severity === 'critical' || f.severity === 'high')
      ? 'high'
      : 'medium',
  },
};
