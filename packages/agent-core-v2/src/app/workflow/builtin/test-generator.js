export const meta = {
  name: 'test-generator',
  description:
    'Automated test generation — analyzes source files, identifies testworthy boundaries and edge cases, generates unit/integration test files, and writes them to disk.',
  whenToUse:
    'Use when the user wants to generate tests for existing code, improve coverage, or add tests for a specific module.',
...
const ANALYSIS_SHAPE = {
  ...
  properties: {
    test_framework: {
      type: 'string',
      description: 'Detected or recommended test framework (vitest, jest, mocha, pytest, etc.).',
    },
    source_files: {
      type: 'array',
      items: { type: 'string' },
      description: 'Source files that need tests.',
      minItems: 1,
    },
    functions: {
      type: 'array',
      items: {
        ...
        properties: {
          name: { type: 'string' },
          type: { type: 'string', enum: ['function', 'method', 'arrow', 'async', 'generator'] },
          complexity: { type: 'string', enum: ['simple', 'moderate', 'complex'] },
          has_tests: { type: 'boolean' },
          risk: { type: 'string', enum: ['low', 'medium', 'high'] },
        },
        required: ['name', 'type', 'complexity', 'has_tests', 'risk'],
      },
    },
  },
...
const PLAN_SHAPE = {
  ...
  properties: {
    test_file_map: {
      ...
      description: 'Map of source path -> test file path.',
    },
    test_cases: {
      type: 'array',
      items: {
        ...
        required: ['target', 'type', 'scenarios', 'edge_cases'],
      },
      minItems: 1,
    },
  },
...
const TEST_OUTPUT_SHAPE = {
  ...
  properties: {
    files: {
      type: 'object',
      additionalProperties: { type: 'string' },
      description: 'Map of test file path -> full file content.',
    },
...
const VALIDATION_SHAPE = {
  ...
  required: ['issues', 'coverage_estimate', 'ready', 'summary'],
};
...
if (userPaths.length === 0)
  return {
    error: 'No source files specified. Pass file paths as args, e.g. "src/utils.ts,src/api.ts".',
  };
...
for (const p of userPaths.slice(0, MAX_SOURCES)) {
  try {
    ...
    sources.push({ path: p, content: truncateSource(content, MAX_LINES) });
  } catch {
    log(`Skipping unreadable: ${p}`);
  }
}
...
log(
  `Framework: ${analysis.test_framework} | Functions: ${analysis.functions.length} | Untested: ${analysis.functions.filter((f) => !f.has_tests).length}`,
);
...
if (!plan || !plan.test_cases || plan.test_cases.length === 0)
  return { error: 'Test planning failed.', analysis };
...
if (!testGen || !testGen.files || Object.keys(testGen.files).length === 0)
  return { error: 'Test generation failed.', analysis, plan };
...
for (const [testPath, content] of Object.entries(testGen.files)) {
  try {
    ...
    log(`Failed to write ${testPath}: ${String(error)}`);
  }
}
...
return {
  analysis: { framework: analysis.test_framework, functions: analysis.functions },
  plan: { test_file_map: plan.test_file_map, test_cases: plan.test_cases },
  generated: { files: Object.keys(testGen.files), warnings: testGen.warnings || [] },
  written,
