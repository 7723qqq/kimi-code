export const meta = {
  name: 'bug-triage',
  description:
    'Automated bug triage — analyzes error logs, stack traces, and source code to identify root causes, assess severity, and suggest fixes with reproduction steps.',
  whenToUse:
    'Use when encountering bugs, crashes, test failures, or error reports. Provide error messages, stack traces, or log files as input.',
...
const PARSE_SHAPE = {
  ...
  properties: {
    ...
    suspect_files: { type: 'array', items: { type: 'string' }, maxItems: MAX_SUSPECT_FILES },
...
if (input.log_file) {
  try {
    logContent = await readFile(input.log_file);
  } catch {
    return { error: `Could not read log file: ${input.log_file}` };
  }
} else if (errorInput) {
  logContent = errorInput;
} else {
  return { error: 'No error input provided. Pass error text, log_file path, or error string.' };
}
...
for (const filePath of (parsed.suspect_files || []).slice(0, MAX_SUSPECT_FILES)) {
  try {
    ...
    log(`Could not read suspect file: ${filePath}`);
  }
}
...
const fix = await agent(
  `You are a fix engineer. Generate a fix for this bug.\n\nError:\n${JSON.stringify(parsed, null, 2)}\n\nRoot cause:\n${JSON.stringify(rootCause, null, 2)}\n\nSource code:\n${codeBlock}\n\n${suspectCode.length === 0 ? 'NOTE: Source code unavailable. Provide a general fix strategy.' : ''}Produce a concrete fix: file path, corrected code, side effects, and workaround.`,
...
return {
  parsed: {
    ...
    suspect_files: parsed.suspect_files,
  ...
  report: report || {
    severity: 'medium',
    confidence: 'medium',
    summary: `Bug triage completed for ${parsed.error_type}.`,
    reproduction_steps: parsed.reproducible
      ? ['See stack trace.']
      : ['Reproduction conditions unknown.'],
  },
