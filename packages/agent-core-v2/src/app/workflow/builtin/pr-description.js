export const meta = {
  name: 'pr-description',
  description:
    'Pull request description generator — analyzes code diffs or file changes, extracts the semantic intent, and generates a comprehensive PR description with summary, changelog entries, testing notes, and reviewer guidance.',
  whenToUse:
    'Use when preparing a pull request, writing commit messages, or generating changelog entries from code changes.',
...
const ANALYSIS_SHAPE = {
  ...
  properties: {
    ...
    type: {
      type: 'string',
      enum: ['feat', 'fix', 'refactor', 'docs', 'test', 'chore', 'perf', 'ci', 'style'],
    },
    scope: { type: 'string' },
  ...
  properties: {
    groups: {
      ...
      items: {
        ...
        required: ['category', 'files', 'description'],
      },
    },
  ...
  if (input.diff_file) {
    try {
      ...
      filePaths = fileMatches
        ? [...new Set(fileMatches.map((m) => m.replace(/^[+]{3}\s+[ab]\//, '').trim()))]
        : ['diff_input'];
    } catch {
      return { error: `Could not read diff file: ${input.diff_file}` };
    }
  } else if (branch) {
    return {
      error: `Branch comparison (${branch}...${baseBranch}) requires git. Provide explicit file paths instead.`,
    };
  } else {
    return { error: 'No file paths provided. Pass files as args or use diff_file.' };
  }
}
...
for (const p of filePaths.slice(0, MAX_FILES)) {
  if (p === 'diff_input' && input.diff_file) {
    try {
      diffContent = (await readFile(input.diff_file))
        .split('\n')
...
return {
  ...
  stats: {
    ...
    new_files: (categorized && categorized.new_files) || [],
    deleted_files: (categorized && categorized.deleted_files) || [],
  },
  review_guide: reviewGuide || {
    focus_areas: [],
    risks: [],
    test_instructions: 'Manual testing recommended.',
    changelog_entry: `- ${analysis.type}: ${analysis.title}`,
  },
};
