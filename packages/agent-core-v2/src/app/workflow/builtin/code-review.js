export const meta = {
  name: 'code-review',
  description:
    'Multi-dimensional code review — runs parallel security, performance, correctness, and maintainability reviews, then produces a consolidated report with severity-ranked findings.',
  whenToUse:
    'Use when the user wants a thorough, multi-perspective code review on a set of files, a diff, or a pull request.',
...
const SCOPE_SHAPE = {
  ...
  properties: {
    files: {
      ...
      minItems: 1,
      maxItems: FILE_BUDGET,
    },
    context: {
      type: 'string',
      description: 'Project language / framework context gleaned from the files.',
    },
  },
...
const REVIEW_SHAPE = {
  ...
  properties: {
    ...
    summary: {
      type: 'string',
      description: 'One-paragraph summary of findings in this dimension.',
    },
  },
...
if (allFindings.length === 0) {
  return {
    ...
    low_count: 0,
    findings: [],
    summary: 'No issues found by any reviewer dimension.',
    recommendations: ['Consider adding automated linting tools to catch issues earlier.'],
  };
}
...
return {
  ...
  context,
  ...(report || {
    overall_rating: allFindings.some((f) => f.severity === 'critical') ? 'fail' : 'needs-work',
  ...
  summaries: {
    security: (securityReview && securityReview.summary) || '',
    performance: (perfReview && perfReview.summary) || '',
    correctness: (correctnessReview && correctnessReview.summary) || '',
    maintainability: (maintainabilityReview && maintainabilityReview.summary) || '',
  },
  recommendations: (report && report.recommendations) || [
    'Address critical and high-severity findings first.',
    'Run automated linting and type checking in CI.',
  ],
};
