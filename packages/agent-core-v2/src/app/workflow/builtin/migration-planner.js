export const meta = {
  ...
  description:
    'Migration and upgrade planner — analyzes current dependency versions, identifies breaking changes, maps required code changes, and generates a phased migration plan with compatibility shims and rollback strategies.',
  whenToUse:
    'Use when upgrading major framework/library versions, migrating between build tools, or planning large-scale dependency updates.',
    ...
    deps: {
      ...
      items: {
        ...
        properties: {
          ...
          current: { type: 'string' },
          target: { type: 'string' },
          type: { type: 'string', enum: ['prod', 'dev', 'peer'] },
          changelog_url: { type: 'string' },
          breaking: { type: 'boolean' },
        },
        required: ['name', 'current', 'target', 'type', 'breaking'],
      },
      minItems: 1,
    },
...
const BREAKING_CHANGES_SHAPE = {
  ...
  items: {
    ...
    properties: {
      dep: { type: 'string' },
      change: { type: 'string', description: 'What changed in the new version.' },
      impact: { type: 'string', enum: ['low', 'medium', 'high'] },
      pattern: {
        type: 'string',
        description: 'Code pattern to search for (e.g. "oldApi(", "import from \'old-module\'").',
      },
        ...
        properties: {
          ...
          changes: {
            ...
            items: {
              ...
              required: ['lines', 'original', 'replacement', 'reason'],
            },
          },
          effort: { type: 'string', enum: ['trivial', 'small', 'medium', 'large'] },
        },
        required: ['path', 'changes', 'effort'],
      },
      minItems: 1,
    },
  ...
  properties: {
    phases: {
      ...
      items: {
        ...
        required: ['name', 'steps', 'verification', 'rollback', 'estimated_time'],
      },
      minItems: 1,
    },
    compatibility_shims: {
      type: 'array',
      items: { type: 'string' },
      description: 'Temporary compatibility shims needed.',
    },
...
if (depTargets.length === 0) {
  return {
    error:
      'No migration targets specified. Pass dependency names as args, e.g. "react@19,vue@4" or {"deps":["react@19"],"targets":{"react":"19.0.0"}}.',
  };
}
...
let pkgContent = '';
try {
  pkgContent = await readFile('package.json');
} catch {
  log('No package.json found');
}
let cargoContent = '';
try {
  cargoContent = await readFile('Cargo.toml');
} catch {}
...
let candidateFiles;
try {
  candidateFiles = await glob(globPattern);
} catch {
  candidateFiles = [];
}
...
for (const p of relevantFiles) {
  try {
    ...
    sources.push({ path: p, content: truncate(content, MAX_LINES) });
  } catch {}
}
...
return {
  inventory: {
    deps: inventory.deps,
    migration_scope: inventory.migration_scope,
  ...
  plan: migrationPlan || {
    phases: [
      {
        name: 'Manual',
        steps: ['Review breaking changes manually.'],
        verification: 'N/A',
        rollback: 'N/A',
    ...
    compatibility_shims: [],
    test_strategy: 'Manual testing recommended.',
    total_estimated_time: 'Unknown',
    precautions: ['Back up your codebase before starting.'],
  },
