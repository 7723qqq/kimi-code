export const meta = {
  ...
  description:
    'Refactoring impact analysis and step-by-step migration planner — analyzes code structure, identifies improvement opportunities, assesses risk, and generates a phased refactoring plan with rollback steps.',
  whenToUse:
    'Use when the user wants to refactor a codebase, improve code structure, reduce technical debt, or prepare for a larger migration.',
  ...
  properties: {
    modules: {
      ...
      items: {
        ...
        required: ['name', 'responsibilities', 'loc', 'dependencies'],
      },
    },
    ...
    breaking_changes: {
      ...
      items: {
        ...
        properties: {
          ...
          affected_callers: { type: 'array', items: { type: 'string' } },
          mitigation: { type: 'string' },
        },
        required: ['description', 'impact', 'affected_callers', 'mitigation'],
      },
    },
  ...
  properties: {
    phases: {
      ...
      items: {
        ...
        required: ['name', 'steps', 'verification', 'rollback'],
      },
      minItems: 1,
    },
...
if (userPaths.length === 0)
  return { error: 'No files specified. Pass file paths or a directory path as args.' };

phase('Analyze');
const sources = [];
for (const p of userPaths.slice(0, MAX_FILES)) {
  try {
...
const plan = await agent(
  `You are a migration planner. Create a phased refactoring plan.\n\n${goal ? `Goal: ${goal}\n\n` : ''}Structure:\n${JSON.stringify(structure, null, 2)}\n\nOpportunities:\n${JSON.stringify(
    opps.toSorted((a, b) => {
      ...
      const e = { trivial: 0, small: 1, medium: 2, large: 3 };
      return r[a.risk] - r[b.risk] || e[a.effort] - e[b.effort];
    }),
    null,
    2,
...
return {
  ...
  assessment: assessment || {
    ...
    risk_summary: 'Assessment not available.',
  },
  plan: plan || {
    phases: [
      {
        name: 'Manual',
        steps: ['Review opportunities manually.'],
        verification: 'N/A',
        rollback: 'N/A',
      },
    ],
