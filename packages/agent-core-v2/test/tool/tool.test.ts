...
describe('Agent tool execution contract', () => {
  ...
  it('emits spawned with the registered task id ahead of started', async () => {
    ...
    expect(eventOrder.indexOf('subagent.started')).toBeGreaterThan(
      eventOrder.indexOf('subagent.spawned'),
    );
    completion.resolve({ summary: 'finished later' });
  });
        ...
        [
          {
            event: 'PreToolUse',
            matcher: 'Bash',
            // `&&` separates commands on both sh and cmd; a bare `;` is echo
            // text on cmd.exe, which would silently turn the block into allow.
            command: "echo 'blocked by PreToolUse' 1>&2 && exit 2",
          },
          {
      ...
      ctx = createTestAgent(
        execEnvServices({
          processRunner: createFakeProcessRunner({
            spawn: exec as unknown as IHostProcessService['spawn'],
          }),
        }),
  ...
  describe('registered user tools', () => {
    ...
    it('routes registered user tools through tool.call request/response', async () => {
      ...
      expect(
        ...
        [emit] tool.call.delta             { "time": "<time>", "turnId": 0, "toolCallId": "call_lookup", "name": "Lookup", "argumentsPart": "{\\"query\\":\\"moon\\"}" }
      ...
      expect(await ctx.untilTurnEnd()).toMatchInlineSnapshot(`
        ...
        [wire] llm.request                 { "kind": "loop", "provider": "openai", "model": "mock-model", "modelAlias": "mock-model", "thinkingEffort": "off", "maxTokens": 1000000, "toolSelect": false, "systemPromptHash": "ec9c34379c88babbc468ef2f3e0e08cd2f422c8c4a910664fb8bb394d703a575", "toolsHash": "2cf01745f26232e091e88186546f93de5362e5941e7717ff00dc587a0fb17842", "messageCount": 4, "turnStep": "0.2", "time": "<time>" }
        [emit] assistant.delta             { "time": "<time>", "turnId": 0, "delta": "The lookup result is moon-result." }
    ...
    `);
      ...
      expect(await ctx.untilTurnEnd()).toMatchInlineSnapshot(`
        [wire] token_counting.turn_recorded   { "turnId": 0, "length": 5, "tokens": 176, "time": "<time>" }
        ...
        [emit] turn.started                   { "time": "<time>", "turnId": 1, "origin": { "kind": "user" }, "prompt": "Can you still use Lookup?" }
    ...
    it('persists oversized registered user tool results before adding them to context', async () => {
      ...
      // The stored path is built with the runtime's posix path class; compare
      // in forward-slash form regardless of the host platform.
...
function hookPayloadAssertCommand(expected: {
  ...
  const script = [
    ...
    expected.errorMessageIncludes === undefined
      ? ''
      : `  const expectedError = ${JSON.stringify(expected.errorMessageIncludes).replaceAll('\\n', '" + String.fromCharCode(10) + "')}; if (typeof payload.error?.message !== 'string' || !payload.error.message.includes(expectedError)) throw new Error('bad error: ' + payload.error?.message);`,
    expected.errorMessageIncludes === undefined
      ? ''
