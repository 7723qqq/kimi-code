...
export class AgentTestContext {
  ...
  restoreRuntimes(): Promise<void> | void {}

  resolve<T>(token: any): any {
    const name = token?.name;
    if (token === IAgentTodoService || name === 'AgentTodo' || name === 'AgentTodoService') {
      return this.get(IAgentTodoService);
    }
    if (token === IAgentGoalService || name === 'AgentGoal' || name === 'AgentGoalService') {
      return this.get(IAgentGoalService);
    }
    if (token === IAgentCronService || name === 'AgentCron' || name === 'AgentCronService') {
      return this.get(IAgentCronService);
    }
