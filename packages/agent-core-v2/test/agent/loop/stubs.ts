...
function registry(): {
  ...
  register: IAgentLoopService['registerLoopErrorHandler'];
} {
  ...
  const register = (
    ...
    if (target === undefined) handlers.push(handler);
    else {
      const i = handlers.findIndex((h) => h.id === target);
      if (i < 0) throw new Error(`Loop error handler target "${target}" is not registered`);
      handlers.splice(options.before !== undefined ? i : i + 1, 0, handler);
    }
...
export function stubLoopWithHooks(options: StubLoopOptions = {}): StubLoop {
  ...
  const startTurn = () => {
    ...
    const result =
      options.pendingTurnResult === true
  ...
  const stub: StubLoop = {
    ...
    enqueue(request, enqueueOptions) {
      let turn = active;
      if (
        request.admission === 'newTurn' ||
        (request.admission === 'activeOrNewTurn' && turn === undefined)
      )
        turn = startTurn();
      if (request.admission === 'activeTurnOnly' && turn === undefined)
        throw new Error('active turn required');
      if (turn === undefined) {
        queue.enqueue(request, enqueueOptions?.at ?? 'tail');
        const assigned = new Promise<never>(() => {});
        void assigned.catch(() => {});
    ...
    drainNextBatch(context) {
      ...
      if (!batch) return;
      materialize(batch.driver, context);
      for (const r of batch.merged) materialize(r, context);
      return batch;
    },
  };
  return stub;
...
export function stubToolExecutor(): IAgentToolExecutorService {
  return {
    ...
    onBeforeExecuteTool: Event.None as Event<BeforeToolExecuteEvent>,
