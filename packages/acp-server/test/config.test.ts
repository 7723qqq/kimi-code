...
describe('acp-server config surface', () => {
  ...
  it('session/new advertises mode + model pickers (no thinking without a model)', async () => {
    await boot();
  ...
  it('session/new advertises the first-class modes state', async () => {
    await boot();
    const { modes } = await newSession();
    expect(modes?.currentModeId).toBe('default');
    expect(modes?.availableModes.map((m) => m.id)).toEqual(['default', 'plan', 'auto', 'yolo']);
  }, 30_000);

  it('session/set_mode pushes current_mode_update alongside config_option_update', async () => {
    await boot();
    ...
    const modeUpdate = (modeNotification.params as { update?: { currentModeId?: string } }).update;
  ...
  it('session/set_mode propagates plan errors without reporting a new mode', async () => {
  ...
  it('session/set_config_option mode also pushes current_mode_update', async () => {
    await boot();
    const { sessionId } = await newSession();
    const modeUpdatePromise = client!.waitForSessionUpdate('current_mode_update');
  ...
  it('session/set_config_option mode updates the returned snapshot', async () => {
    await boot();
    const { sessionId } = await newSession();
  ...
  it('session/set_config_option rejects an unknown modeId', async () => {
    await boot();
    const { sessionId } = await newSession();
    await expect(
  ...
  it('session/set_config_option rejects an unknown configId', async () => {
    await boot();
    const { sessionId } = await newSession();
    await expect(
      client!.send('session/set_config_option', {
  ...
  it('session/set_model switches the model and pushes config_option_update', async () => {
    ...
    await expect(client!.server.klient.session(sessionId).agent('main').getModel()).resolves.toBe(
      FAKE_MODEL_ALT_ID,
    );
    const notification = await updatePromise;
    const update = (notification.params as { update?: { configOptions?: readonly ConfigOption[] } })
      .update;
    const model = update?.configOptions?.find((o) => o.id === 'model');
    expect(model?.currentValue).toBe(FAKE_MODEL_ALT_ID);
  }, 30_000);

  it('session/set_model rejects malformed params', async () => {
    await boot({ fakeModel: true });
    const { sessionId } = await newSession();
    await expect(client!.send('session/set_model', { sessionId, modelId: 42 })).rejects.toThrow();
    await expect(
      client!.send('session/set_model', { sessionId: 'nope', modelId: FAKE_MODEL_ALT_ID }),
    ).rejects.toThrow();
  }, 30_000);

  it('the merged "<id>,thinking" form sets the bare model and flips thinking on at its default effort', async () => {
    await boot({
      fakeModel: true,
      altThinking: true,
  ...
  it('a bare set_model id does not turn thinking off', async () => {
  ...
  it('session/new advertises the thinking toggle for a thinking-capable model', async () => {
  ...
  it('session/new omits the thinking toggle for a non-thinking model', async () => {
    await boot({ fakeModel: true });
    const { configOptions } = await newSession();
    expect(configOptions.map((o) => o.id)).not.toContain('thinking');
  }, 30_000);

  it('session/set_config_option thinking takes effect and pushes config_option_update', async () => {
    await boot({ fakeModel: true, thinking: true });
    const { sessionId } = await newSession();
    const updatePromise = client!.waitForSessionUpdate('config_option_update');
    const result = (await client!.send('session/set_config_option', {
  ...
  it('session/set_config_option rejects an unknown thinking value', async () => {
    await boot({ fakeModel: true, thinking: true });
    const { sessionId } = await newSession();
    await expect(
      client!.send('session/set_config_option', {
  ...
  it('an effort-capable model advertises off + every declared effort', async () => {
    await boot({
      fakeModel: true,
      thinking: true,
  ...
  it('setting a declared effort level takes effect and pushes config_option_update', async () => {
    await boot({
      fakeModel: true,
      thinking: true,
      supportEfforts: ['low', 'high'],
  ...
  it("an effort-capable model maps the legacy 'on' to its default effort", async () => {
    await boot({
      fakeModel: true,
      thinking: true,
      supportEfforts: ['low', 'high'],
  ...
  it('an effort-capable model rejects an undeclared effort with invalid_params', async () => {
    await boot({
      fakeModel: true,
      thinking: true,
      supportEfforts: ['low', 'high'],
  ...
  it('a Claude model on an Anthropic-typed provider advertises the thinking toggle via engine-derived capabilities', async () => {
    homeDir = await mkdtemp(join(tmpdir(), 'acp-config-'));
...
`,
  ...
  it('a $/cancel_request notification is accepted without error', async () => {
    await boot();
    await newSession();
