...
describe('acp-server session lifecycle', () => {
  ...
  async function sessionMcpEntries(
    ...
    readonly { readonly name: string; readonly status: string; readonly error?: string }[]
  > {
    await c.server.core.accessor.get(IWorkspaceInstanceManager).getOrCreate({ root: homeDir! });
  ...
  it('session/new rejects stdio MCP servers without runtime identity', async () => {
    const c = await boot();
    await expect(
      c.send('session/new', {
        cwd: homeDir,
  ...
  it('session/load rejects stdio MCP servers without runtime identity', async () => {
    const c = await boot();
    const created = (await c.send('session/new', { cwd: homeDir, mcpServers: [] })) as {
      sessionId: string;
    };
