type DeepPartial<T> = T extends string
  ? string
  : T extends object
    ? { [K in keyof T]?: DeepPartial<T[K]> }
    : never;

export const zh: DeepPartial<typeof import('./en').default> = {
  errors: {
    configInvalid: '配置无效',
    sessionNotFound: '未找到会话',
    sessionAlreadyExists: '会话已存在',
    sessionIdInvalid: '会话 ID 无效',
    sessionIdRequired: '需要会话 ID',
    sessionIdEmpty: '会话 ID 为空',
    sessionTitleEmpty: '会话标题为空',
    sessionStateNotFound: '会话状态缺失',
    sessionStateInvalid: '会话状态无效',
    sessionForkActiveTurn: '活动轮次期间无法分叉会话',
    sessionExportNotFound: '会话导出目录缺失',
    sessionExportMissingVersion: '缺少导出版本号',
    sessionClosed: '会话已关闭',
    sessionPermissionModeInvalid: '权限模式无效',
    sessionThinkingEmpty: '思考值为空',
    sessionModelEmpty: '模型为空',
    sessionPlanModeInvalid: '计划模式无效',
    sessionApprovalHandlerError: '审批处理程序抛出异常',
    sessionQuestionHandlerError: '提问处理程序抛出异常',
    sessionInitFailed: '会话初始化失败',
    agentNotFound: '未找到 Agent',
    turnAgentBusy: 'Agent 正忙',
    goalAlreadyExists: '已存在活跃的目标',
    goalNotFound: '未找到目标',
    noCurrentGoal: '没有当前目标',
    goalObjectiveEmpty: '目标为空',
    goalObjectiveTooLong: '目标过长',
    goalStatusInvalid: '无效的目标状态转换',
    goalMetadataReserved: '目标元数据为保留字段',
    goalNotResumable: '目标不可恢复',
    modelNotConfigured: '未配置模型',
    providerNotSet: '未设置供应商',
    modelConfigInvalid: '模型配置无效',
    authLoginRequired: '需要登录',
    contextOverflow: '上下文窗口溢出',
    loopMaxStepsExceeded: '轮次超出最大步数',
    providerApiError: '供应商 API 错误',
    providerFiltered: '供应商过滤了响应',
    providerRateLimit: '供应商速率限制',
    providerAuthError: '供应商认证错误',
    providerConnectionError: '供应商连接错误',
    skillNotFound: '未找到 Skill',
    skillTypeUnsupported: '不支持的 Skill 类型',
    skillNameEmpty: 'Skill 名称为空',
    recordsWriteFailed: '写入记录失败',
    compactionFailed: '压缩失败',
    compactionUnable: '无法压缩',
    taskTaskIdEmpty: '后台任务 ID 为空',
    mcpServerNotFound: '未找到 MCP server',
    mcpServerDisabled: 'MCP server 已禁用',
    mcpStartupFailed: 'MCP server 启动失败',
    mcpToolNameCollision: 'MCP 工具名称冲突',
    pluginNotFound: '未找到插件',
    pluginLoadFailed: '插件状态加载失败',
    requestInvalid: '无效请求',
    requestWorkDirRequired: '需要 workDir',
    requestPromptInputEmpty: '提示输入为空',
    shellGitBashNotFound: '未找到 Git Bash',
    notImplemented: '未实现',
    internal: '内部错误',
    paginationMutuallyExclusive: 'before_id 和 after_id 互斥',
    indexHtmlNotAFile: 'index.html 不是文件',
  },
  plugin: {
    manifestNotFound: '在 {{path}} 未找到清单文件',
    manifestParseFailed: '解析 {{name}} 失败: {{error}}',
    manifestMustBeObject: '清单必须是 JSON 对象',
    nameRequired: '"name" 为必填项',
    nameMustMatch: '"name" 必须匹配 {{regex}}（实际为 "{{name}}"）',
    unsupportedField: '"{{field}}" 存在但不受 Kimi 插件支持',
    skillsMustBeStringOrArray: '"skills" 必须是字符串或字符串数组',
    skillsPathDotSlash: '"skills" 路径必须以 "./" 开头（实际为 "{{entry}}"）',
    skillsPathOutside: '"skills" 路径解析到插件外部（{{entry}}）',
    skillsPathNotDir: '"skills" 路径不是目录（{{entry}}）',
    dirFieldMustBeStringOrArray: '"{{field}}" 必须是字符串或字符串数组',
    dirFieldPathDotSlash: '"{{field}}" 路径必须以 "./" 开头（实际为 "{{entry}}"）',
    dirFieldPathOutside: '"{{field}}" 路径解析到插件外部（{{entry}}）',
    dirFieldPathNotDir: '"{{field}}" 路径不是目录（{{entry}}）',
    fieldPathDotSlash: '"{{field}}" 路径必须以 "./" 开头（实际为 "{{value}}"）',
    fieldPathOutside: '"{{field}}" 路径解析到插件外部（{{value}}）',
    sessionStartMustBeObject: '"sessionStart" 必须是对象',
    sessionStartSkillRequired: '存在 sessionStart 时 "sessionStart.skill" 为必填',
    mcpServersMustBeObject: '"mcpServers" 必须是对象',
    mcpServersNameRequired: '"mcpServers" 条目必须具有非空名称',
    mcpServerInvalid: '无效的 MCP server "{{name}}": {{error}}',
    hooksMustBeArray: '"hooks" 必须是数组',
    hookInvalid: '索引 {{index}} 处的 hook 无效: {{error}}',
    commandsMustBeStringOrArray: '"commands" 必须是字符串或字符串数组',
    commandsEntryInvalid: '"commands" 条目必须是目录或 .md 文件（{{entry}}）',
    mcpServerCommandPath: '"mcpServers.{{name}}.command" 必须是 PATH 命令或以 "./" 开头',
  },
  session: {
    newSession: '新会话',
  },
  background: {
    taskStatus: '后台 {{kind}} {{status}}',
    taskTimedOut: '{{description}} 已超时。',
    taskWasKilled: '{{description}} 被终止: {{reason}}。',
    taskCompleted: '{{description}} 已完成。',
    taskFailed: '{{description}} 失败: {{reason}}。',
    outputLimitExceeded:
      '超出输出限制: 命令产生了超过 {{mib}} MiB 的输出并被终止。请将大量输出重定向到文件（例如 `command > out.txt`）并分片查看。',
    outputFile: '读取输出文件以获取结果: {{path}}',
    outputPreviewTruncated: '显示最后 {{bytes}} 字节。没有持久化的完整输出可用。',
    outputPreviewBuffered: '没有持久化的完整输出可用；这是当前缓冲的任务输出。',
    fullOutputSaved: '[完整输出已保存]',
    recoverPrompt:
      '要恢复或继续此子 Agent，请调用 Agent(resume="{{agentId}}", prompt="从上次中断的地方继续；如果上次工具调用的结果从未被观察到，则重新执行。"）。',
    recoverAgentId:
      '使用 agent_id（"{{agentId}}"），而不是 source_id / task_id（"{{taskId}}"）——两者看起来相似，但 resume 参数只接受 agent_id。',
    recoverBackground:
      '添加 run_in_background=true 以保持后台运行，或省略它以在当前轮次中内联获取结果。',
    recoverContext:
      '子 Agent 在重启后保留其完整的先前上下文，但任何进行中的工具调用已丢失其结果，可能需要重新执行。',
  },
  shell: {
    userInterrupt: '被用户中断',
    pausedAfterInterruption: '中断后已暂停',
    backgroundStarted: '后台任务已启动',
    commandTimedOutToBackground: '命令超时并移至后台',
    taskMovedToBackground: '任务移至后台',
    backgroundStartedBrief: '已启动 {{taskId}}',
    backgroundedAfterTimeout: '{{taskId}} 超时后在后台运行',
    backgroundedBrief: '{{taskId}} 在后台运行',
    abortedBeforeStart: '命令启动前中止',
    commandCannotBeEmpty: '命令不能为空。',
    backgroundNotAvailable: '此 Agent 不支持后台执行，因为未启用 TaskOutput 和 TaskStop。',
    descriptionRequired: 'run_in_background 为 true 时 description 为必填。',
    killedByTimeout: '命令因超时被终止（{{label}}）',
    killedByTimeoutBrief: '因超时被终止（{{label}}）',
    executedSuccessfully: '命令执行成功。',
    failedWithExitCode: '命令失败，退出码: {{exitCode}}。',
    failedWithExitCodeBrief: '失败，退出码: {{exitCode}}',
    exitedWithCode: '进程退出码 {{exitCode}}',
    taskIdLabel: 'task_id: {{taskId}}',
    pidLabel: 'pid: {{pid}}',
    descriptionLabel: 'description: {{description}}',
    statusLabel: 'status: {{status}}',
    automaticNotification: 'automatic_notification: true',
    humanShellHint: 'human_shell_hint: 告诉用户在终端运行 /tasks 打开交互式后台任务面板。',
    nextStepWithTaskOutput: '不要等待、轮询或调用 TaskOutput',
    nextStepAwaitNoTaskOutput: '不要等待或轮询',
    nextStepForegroundDetached:
      'next_step: 任务现在在后台运行。完成后你会自动收到通知——不要等待、轮询或调用 TaskOutput；继续当前工作。\n',
    nextStepForegroundDetachedNoBg:
      'next_step: 任务现在在后台运行。完成后你会自动收到通知——不要等待或轮询；继续当前工作。\n',
    nextStepBackgroundStarted:
      'next_step: 完成结果会在后续轮次自动到达——不要等待、轮询或调用 TaskOutput；继续当前工作。\nnext_step: 仅在需要取消任务时使用 TaskStop。\n',
    nextStepBackgroundStartedNoBg: 'next_step: 完成后你会自动收到通知。\n',
    taskOutputHint: '，或 TaskOutput(task_id="{{taskId}}", block=false)',
    outputPathLabel: 'output_path: {{path}}',
    outputSizeLabel: 'output_size_bytes: {{bytes}}',
    nextStepReadOutput: 'next_step: 使用 Read 配合 output_path 分页查看完整日志{{hint}}。',
  },
  tools: {
    grepTimedOut: 'Grep 在 {{seconds}}s 后超时。请尝试更具体的路径或模式。',
    grepPartial: 'Grep 在 {{seconds}}s 后超时；返回部分结果',
    grepAborted: 'Grep 已中止',
    agentResumeTimedOut: 'Agent 在 {{timeout}} 后超时。',
    listingBackgroundTasks: '列出后台任务',
    creatingGoal: '创建目标',
    readingTodoList: '读取待办列表',
    clearingTodoList: '清空待办列表',
    updatingTodoList: '更新待办列表',
    noMatchesFound: '未找到匹配项',
    noNonSensitiveMatches: '未找到非敏感匹配项',
    askingUserQuestions: '正在向用户提问',
    askUserQuestion: '向用户提问',
  },
  flags: {
    toolSelectTitle: '工具选择（渐进式工具暴露）',
    toolSelectDesc:
      '将 MCP 工具 schema 保持在不可变的顶层 tools[] 之外；模型通过 select_tools 工具按需加载。仅在能力目录声明动态加载工具的模型上生效。',
    nativeToolsTitle: '原生工具',
    nativeToolsDesc: '对 Read、Write、Edit、Grep、Glob 和 Bash 工具使用 Rust 原生实现。',
    rpcMicrotaskTitle: 'RPC 微任务调度',
    rpcMicrotaskDesc:
      '对进程内 RPC 模拟使用 queueMicrotask 而非 setTimeout(0)，降低每次调用的延迟。',
  },
  v2Errors: {
    internal: '内部错误',
    notImplemented: '未实现',
    loopMaxStepsExceeded: '循环超出最大步数',
    loopMaxStepsAction:
      '请在 config.toml 中提高 loop_control.max_steps_per_turn，或运行 "/update-config" 然后 "/reload"。',
    loopMaxStepsMessage:
      '轮次超出最大步数 maxSteps={{maxSteps}}。如果 max_steps_per_turn 太小，请在 config.toml 中提高它（loop_control.max_steps_per_turn），或运行 "/update-config" 更新，然后 "/reload"。',
    providerSafetyBlocked: '提供商安全策略拦截了响应。',
    goalAlreadyExists: '目标已存在',
    goalAlreadyExistsAction: '请使用 "/goal replace <objective>" 替换当前目标。',
    goalNotFound: '未找到目标',
    goalNotFoundAction: '请先使用 "/goal <objective>" 启动一个目标。',
    goalObjectiveEmpty: '目标描述为空',
    goalObjectiveEmptyAction: '请提供非空的目标描述。',
    goalObjectiveTooLong: '目标描述过长',
    goalObjectiveTooLongAction: '请将目标描述控制在 4000 字符以内；长内容请通过文件路径引用。',
    goalCompletionCriterionEmpty: '目标缺少完成标准',
    goalCompletionCriterionEmptyAction:
      '请提供一个具体、可验证的完成标准（例如某个测试通过或命令以 0 退出）。需求含糊时请先询问用户。',
    goalCompletionCriterionTooShort: '目标完成标准过短',
    goalCompletionCriterionTooShortAction: '请描述一个具体、可检查的完成条件，而不是占位符。',
    goalStatusInvalid: '无效的目标状态转换',
    goalStatusInvalidAction: '只能暂停活跃的目标；已阻塞的目标请使用 "/goal resume" 恢复。',
    goalMetadataReserved: '目标元数据为保留字段',
    goalMetadataReservedAction: '请勿直接写入 metadata.custom.goal；请使用目标生命周期方法。',
    goalNotResumable: '目标不可恢复',
    goalNotResumableAction: '仅暂停或阻塞的目标可以恢复。',
    goalUnsupportedAgent: '子 Agent 不支持目标',
    goalUnsupportedAgentAction: '请在主 Agent 上运行目标生命周期命令。',
    authLoginRequired: '需要登录',
    authLoginRequiredAction: '请运行 /login 使用 OAuth 提供商进行认证。',
    authProvisioningRequired: '需要配置提供商',
    authProvisioningRequiredAction: '请通过 /login 或提供商端点配置提供商。',
    authTokenMissing: '提供商凭据缺失',
    authTokenMissingAction: '请为提供商配置 API 密钥或完成 OAuth 登录。',
    authTokenUnauthorized: '提供商凭据未授权',
    authTokenUnauthorizedAction: '请重新通过 OAuth 提供商进行认证。',
    authModelNotResolved: '模型未解析',
    authModelNotResolvedAction: '请设置默认模型或配置请求的模型别名。',
    providerRateLimit: '提供商速率限制',
    providerRateLimitAction: '请在提供商速率限制重置后重试。',
    providerFiltered: '提供商过滤了响应',
    providerFilteredAction: '请修改提示词或模型配置以避免提供商安全过滤。',
    providerAuthError: '提供商认证失败',
    providerAuthErrorAction: '请检查提供商凭据和认证配置。',
    providerOverloaded: '提供商过载',
    providerOverloadedAction: '请在提供商恢复后重试。',
    contextOverflow: '上下文溢出',
    contextOverflowAction: '请压缩对话或用更少的令牌重试。',
    providerNotFound: '未找到提供商',
    providerNotFoundAction: '请检查提供商 ID 或先配置提供商。',
    modelNotFound: '未找到模型',
    modelNotFoundAction: '请检查模型别名或先配置模型。',
    fileNotFound: '未找到文件',
    fileNotFoundAction: '请检查 file_id 或重新上传文件。',
    fileTooLarge: '上传文件过大',
    fileTooLargeAction: '请上传较小的文件（限制为 50 MiB）。',
    pathNotFound: '路径未找到',
    pathIsDirectory: '路径是目录',
    pathNotDirectory: '路径不是目录',
    pathAlreadyExists: '路径已存在',
    permissionDenied: '权限被拒绝',
    permissionDeniedAction: '请检查目标路径的文件权限。',
    directoryNotEmpty: '目录非空',
    fsUnavailable: '文件系统不可用',
    fsError: '文件系统错误',
    processSpawnFailed: '进程启动失败',
    processSpawnFailedAction: '请检查命令是否存在且可执行。',
    processKillFailed: '进程终止失败',
    storageNotFound: '存储值未找到',
    storageDecodeFailed: '存储数据已损坏',
    storageDecodeFailedAction: '请检查存储的文档；其格式无效。',
    storageCorrupted: '存储数据已损坏',
    storageCorruptedAction: '请检查后端存储；损坏的条目需要修复或删除。',
    storageIoFailed: '存储 I/O 失败',
    storageLocked: '存储已被锁定',
    storageLockedAction: '另一个进程持有该存储；请关闭它或稍后重试。',
    wireDuplicateOp: '重复的 wire 操作类型',
    wireDuplicateOpAction: '两个操作注册了相同的类型；请重命名其中一个。这是构建时的错误。',
    wireCycle: 'Wire 调度循环',
    wireCycleAction: 'onChange 处理程序无限重新调度；请打破操作循环。',
    wireUnknownRecord: '未知的 wire 记录',
    wireUnknownRecordAction: '该记录由更新的版本写入；请升级或删除它。',
    wireWriteFailed: 'Wire 日志写入失败',
    compactionNoMessages: '当前历史中没有可压缩的消息。',
    compactionActiveTurn: '轮次进行中无法压缩。请等待轮次结束后再试。',
    compactionOverflowFailed: '压缩在 {{attempts}} 次尝试后仍未能将上下文降至模型窗口以下。',
    compactionLimitExceeded: '压缩次数超限（{{limit}}）。',
    mcpFileReadFailed: '读取 {{filePath}} 失败：{{error}}',
    mcpJsonParseFailed: '{{filePath}} 中的 JSON 无效：{{error}}',
    mcpSchemaInvalid: '{{filePath}} 中的 MCP 服务器配置无效：{{error}}',
    mcpResultTooLarge: 'MCP 工具结果过大：约 {{mb}} MB 超过 {{limitMb}} MB 限制',
    swarmMinInputs: 'AgentSwarm 需要至少 2 个 items，除非提供了 resume_agent_ids。',
    swarmMaxSubagents: 'AgentSwarm 最多支持 {{count}} 个子智能体。',
    swarmPromptRequired: '提供 items 时必须提供 prompt_template。',
    swarmPromptPlaceholder: 'prompt_template 必须包含 {{placeholder}} 占位符。',
    swarmDuplicatePrompts:
      'items {{indexA}} 和 {{indexB}} 的子智能体提示重复。AgentSwarm 需要不同的子智能体。',
    tooManyBackgroundTasks: '后台任务过多，无法继续运行。',
    planModeAlreadyActive: '已处于计划模式',
    mainAgentNotFound: '未找到主智能体',
    mainAgentNoModel: '主智能体未绑定模型',
    initFailed: '初始化失败',
    callerAgentNotFound: '调用方智能体 "{{agentId}}" 不存在',
    subagentNotFound: '智能体实例 "{{agentId}}" 不存在',
    unknownAgentType: '未知的智能体类型："{{type}}"',
    callerAgentNoModel: '调用方智能体未绑定模型',
    subagentNotSubagent: '智能体实例 "{{agentId}}" 不是子智能体',
    subagentWrongParent: '智能体实例 "{{agentId}}" 不属于此父智能体',
    subagentAlreadyRunning: '智能体实例 "{{agentId}}" 已在运行，无法并发执行',
    subagentDepthExceeded:
      '子代理委托深度超出限制（最大 {{maxDepth}}）——请使用持久子代理或调整委托结构',
    rgNotAvailable: 'PATH 中未找到 ripgrep (rg)',
    pathRejectedEmpty: '路径 "{{path}}" 被拒绝（为空）',
    pathRejectedAbsolute: '路径 "{{path}}" 被拒绝（绝对路径）',
    pathRejectedDotdot: '路径 "{{path}}" 被拒绝（包含 .. 段）',
    pathEscapesWorkspace: '路径 "{{path}}" 超出工作区范围',
    permissionPatternEmpty: '权限模式：空字符串',
    permissionPatternMissingParen: '权限模式："{{pattern}}" 缺少右括号',
    permissionPatternEmptyTool: '权限模式："{{pattern}}" 工具名为空',
    unknownSession: "未知会话 '{{sessionId}}'",
    unknownAgent: "未知智能体 '{{agentId}}'",
    agentDoesNotExist: '智能体 "{{agentId}}" 不存在',
    agentTurnCouldNotStart: '无法启动智能体轮次',
    oauthTokenProviderNotConfigured: 'OAuth 令牌提供程序未配置。',
    cronExpressionMustBeString: 'cron 表达式必须是字符串',
    cronExpressionEmpty: 'cron 表达式为空',
    cronStepEmpty: 'cron {{name}} 字段在 "{{term}}" 中步长为空',
    bashToolNotRegistered: 'Bash 工具未注册。',
    mcpSseClientClosed: 'MCP SSE 客户端已关闭',
    mcpSseClientClosedDuringStartup: 'MCP SSE 客户端在启动期间已关闭',
    mcpOAuthStoreKeyInvalid: '无效的 MCP OAuth 存储键："{{name}}"',
    mcpOAuthFlowAlreadyDone: 'OAuth 流程已完成或已取消',
    mcpStdioCommandEmpty: 'MCP stdio 命令不能为空',
    mcpStdioClientClosed: 'MCP stdio 客户端已关闭',
    mcpStdioClientClosedDuringStartup: 'MCP stdio 客户端在启动期间已关闭',
    mcpHttpClientClosed: 'MCP HTTP 客户端已关闭',
    mcpHttpClientClosedDuringStartup: 'MCP HTTP 客户端在启动期间已关闭',
    sourceAgentDoesNotExist: '源智能体 "{{sourceAgentId}}" 不存在',
    agentAlreadyExists: '智能体 "{{agentId}}" 已存在',
    promptIdsEmpty: 'prompt_ids 不能为空',
    noActivePrompt: '没有可转向的活跃提示',
    promptsNotPending: '一个或多个提示未处于待处理状态',
    noActiveTurnToSteer: '没有可转向的活跃轮次',
    unknownCommand: '未知命令 "{{name}}"',
    agentProfileSystemPromptRequired:
      'Agent 配置 "{{name}}" 必须定义 systemPrompt 或 renderSystemPrompt。',
    webbridgeDaemonNotUp:
      'WebBridge 守护进程未在 {{baseUrl}} 上启动 — 请检查 ~/.kimi-webbridge/logs',
    webbridgeUnsupportedPlatform: 'kimi-webbridge 不支持 {{platform}}/{{arch}}',
    webbridgeStartFailed: 'kimi-webbridge 启动失败: {{detail}}',
  },
  v2Goal: {
    cancelledReminder:
      '用户取消了当前目标。请忽略该目标的早期活跃目标提醒。除非用户启动或恢复目标，否则正常处理下一个用户请求。',
    forkClearedReminder:
      '此复刻没有当前目标。请忽略源会话中的早期活跃目标提醒。除非用户启动新目标，否则正常处理请求。',
    completionCriterionEmpty: '目标需要一个完成标准——一个具体、可验证的“完成”条件。',
    completionCriterionTooShort:
      '完成标准过短（至少 {{min}} 个字符）。请描述一个具体、可检查的“完成”条件。',
    pausedRateLimit: '提供商速率限制后暂停',
    pausedConnectionError: '提供商连接错误后暂停',
    pausedAuthError: '提供商认证错误后暂停',
    pausedApiError: '提供商 API 错误后暂停',
    pausedModelConfig: '模型配置错误后暂停',
    pausedRuntime: '运行时错误后暂停',
    pausedContinuationFailure: '目标继续失败后暂停',
    pausedProviderFiltered: '提供商安全策略阻止后暂停',
    budgetLimited: '目标预算达到上限',
    llmNotSet: '未设置 LLM，请发送 "/login" 登录',
    budgetStopReminder:
      '目标的硬预算已耗尽。请立即停止。不要再调用任何工具。写一条简短的最后状态消息。',
    budgetToolsRejected: '目标预算已耗尽；工具调用被拒绝。请写你的最后消息。',
    staleToolResult: '自本轮开始以来目标已更改；忽略过时的目标工具调用。',
    continuationPromptFallback: '继续朝着活跃目标工作。保持自我审核简短。',
    onlyMainAgent: '目标仅由主 Agent 支持',
    objectiveCannotBeEmpty: '目标描述不能为空',
    objectiveTooLong: '目标描述不能超过 {{max}} 个字符',
    goalAlreadyExistsReplace: '目标已存在；请使用 replace 启动新目标',
    cannotPause: '无法暂停状态为 "{{status}}" 的目标',
    cannotResume: '无法恢复状态为 "{{status}}" 的目标',
    blockedByHook: '被 UserPromptSubmit 钩子阻塞',
    pausedAfterInterruption: '中断后暂停',
    pausedAfterResume: 'Agent 恢复后暂停',
    noCurrentGoal: '没有当前目标',
    budgetBlockReason:
      '目标预算达到上限：{{budgets}}（轮次 {{turnBudget}}，令牌 {{tokenBudget}}，时钟 {{wallClockBudget}}ms）',
  },
  v2Mcp: {
    authToolDescription: '通过 OAuth 认证 MCP 服务器 "{{serverName}}"',
    authToolDescriptionBlock:
      '此服务器需要尚未完成的 OAuth 登录。调用此工具将启动授权流程：\n\n1. 工具将打印授权 URL。\n2. **你必须向用户逐字展示该 URL**，并请他们在浏览器中打开、登录并批准 kimi-code 客户端。\n3. 工具将阻塞（最长 {{timeoutMinutes}} 分钟），直到浏览器重定向回本地回调监听器。\n4. 成功后，kimi-code 重新连接 MCP 服务器，真实工具将替换此合成工具。\n\n无需参数。请将 URL 视为敏感信息——不要修改它或剥离查询参数。',
    authTimeoutSuffix:
      '（超时 {{timeoutMinutes}} 分钟）。如果取消，请再次调用此工具以重新启动流程。',
    authErrorUrlSuffix: '\n\n授权 URL（如果监听器未超时则仍然有效）：{{authorizationUrl}}',
    discoveringOAuth: '正在发现 {{serverName}} 的 OAuth 元数据…',
    alreadyAuthorized: '已授权；正在重新连接 {{serverName}}…',
    authorizedReconnected:
      'MCP 服务器 "{{serverName}}" 已有有效的 OAuth 凭据。已重新连接；真实工具现在可用。',
    openUrl: '在浏览器中打开此 URL 以授权 "{{serverName}}"：',
    waitingForCallback: '等待 OAuth 回调...',
    authorizedReconnecting: '已授权 — 正在重新连接 {{serverName}}…',
    authenticated: 'MCP 服务器 "{{serverName}}" 认证成功。真实 MCP 工具已替换此合成认证工具。',
    oauthFailed: 'MCP 服务器 "{{serverName}}" 的 OAuth 流程未完成：{{message}}',
    requiresOAuth: '{{name}} 需要 OAuth — 运行 /mcp-config login {{name}}',
    toolNameCollision: '"{{toolName}}" -> {{qualified}}（与同一服务器的 "{{collidesWith}}" 冲突）',
    serverCollisionSummary: 'MCP 服务器 "{{serverName}}" 注册了 {{count}} 个冲突的工具名称',
    losingToolsDropped: '被丢弃的工具：{{summary}}',
    outputTruncated:
      '\n\n[输出已截断：超出 {{limit}} 字符限制。请使用分页或更精确的查询获取剩余内容。]',
    binaryPartTooLarge:
      '[{{kind}}_url 已丢弃：约 {{approxMb}} MB 超过每部分 {{capMb}} MB 限制。请尝试更小的资源。]',
  },
  v2Auth: {
    noProvider: '未配置提供商；请通过 /login 或提供商端点完成设置',
    loginRequired: 'OAuth 提供商 "{{providerKey}}" 需要先登录才能使用。',
  },
  v2Loop: {
    maxStepsExceeded:
      '轮次超出最大步数 maxSteps={{maxSteps}}。如果 max_steps_per_turn 太小，请在 config.toml 中提高它（loop_control.max_steps_per_turn），或运行 "/update-config" 更新，然后 "/reload"。',
    providerSafetyBlocked: '提供商安全策略拦截了响应。',
  },
  v2Fs: {
    pathNotFound: '路径不存在',
    pathIsDirectory: '路径是目录',
    pathNotDirectory: '路径不是目录',
    pathAlreadyExists: '路径已存在',
    permissionDenied: '权限被拒绝',
    directoryNotEmpty: '目录非空',
    fsUnavailable: '文件系统不可用',
    fsError: '文件系统错误',
    opFailed: '{{op}} 失败：{{reason}}',
  },
  v2Storage: {
    notFound: '存储值未找到',
    decodeFailed: '存储数据已损坏',
    corrupted: '存储数据已损坏',
    ioFailed: '存储 I/O 失败',
    locked: '存储已被锁定',
  },
  v2Wire: {
    duplicateOp: '重复的 wire 操作类型',
    cycle: 'Wire 调度循环',
    unknownRecord: '未知的 wire 记录',
    writeFailed: 'Wire 日志写入失败',
  },
  v2Model: {
    fileNotFound: '未找到文件：{{fileId}}',
    fileTooLarge: '上传大小 {{seen}} 字节超过限制 {{limit}} 字节',
  },
  svc: {
    common: {
      description: 'Kimi Code 本地服务器（由 `kimi server install` 管理）',
    },
    schtasks: {
      alreadyExists: '计划任务 "{{name}}" 已存在。请使用 --force 重新运行以替换它。',
      installButRunFailed: '任务已{{status}}但 /Run 失败：{{error}}。',
      installed: 'Kimi 服务器计划任务已{{status}}（{{name}}，端口 {{port}}）。',
      nothingToRemove: '计划任务未安装；无需移除。',
      deleteFailed: 'schtasks /Delete 失败：{{error}}',
      removed: '计划任务已移除（{{name}}）。',
      notInstalled: '计划任务未安装。请先运行 `kimi server install`。',
      runFailed: 'schtasks /Run 失败：{{error}}',
      started: 'Kimi 服务器已启动（{{name}}）。',
      endFailed: 'schtasks /End 失败：{{error}}',
      stopped: 'Kimi 服务器已停止（{{name}}）。',
      endFailedRestart: '重启期间 schtasks /End 失败：{{error}}',
      runFailedRestart: '重启期间 schtasks /Run 失败：{{error}}',
      restarted: 'Kimi 服务器已重启（{{name}}）。',
      statusReplaced: '替换',
      statusInstalled: '安装',
    },
    systemd: {
      alreadyExists: 'systemd 单元已安装在 {{path}}。请使用 --force 重新运行以替换它。',
      installed: 'Kimi 服务器 systemd 单元已{{status}}于 {{path}}（端口 {{port}}）。',
      nothingToRemove: 'systemd 单元未安装；无需移除。',
      removed: 'systemd 单元已移除（{{path}}）。',
      notInstalled: 'systemd 单元未安装。请先运行 `kimi server install`。',
      startFailed: 'systemctl --user start 失败：{{error}}',
      started: 'Kimi 服务器已启动（{{name}}）。',
      stopFailed: 'systemctl --user stop 失败：{{error}}',
      stopped: 'Kimi 服务器已停止（{{name}}）。',
      restartFailed: 'systemctl --user restart 失败：{{error}}',
      restarted: 'Kimi 服务器已重启（{{name}}）。',
    },
    launchd: {
      alreadyExists: 'LaunchAgent 已安装在 {{path}}。请使用 --force 重新运行以替换它。',
      installed: 'Kimi 服务器 LaunchAgent 已{{status}}于 {{path}}（端口 {{port}}）。',
      nothingToRemove: 'LaunchAgent 未安装；无需移除。',
      removed: 'LaunchAgent 已移除（{{path}}）。',
      notInstalled: 'LaunchAgent 未安装。请先运行 `kimi server install`。',
      kickstartBootstrapFailed: 'launchctl kickstart + bootstrap 均失败：{{error1}} / {{error2}}',
      kickstartFailedAfterBootstrap: 'bootstrap 后 launchctl kickstart 失败：{{error}}',
      started: 'Kimi 服务器已启动（{{name}}）。',
      killBootoutFailed: 'launchctl kill + bootout 均失败：{{error1}} / {{error2}}',
      stopped: 'Kimi 服务器已停止（{{name}}）。',
      kickstartRestartFailed: 'launchctl kickstart -k 失败：{{error}}',
      restarted: 'Kimi 服务器已重启（{{name}}）。',
    },
    systemdUnit: {
      cannotContainCRLF: '{{label}} 不能包含 CR 或 LF 字符。',
    },
  },
  serverErrors: {
    indexHtmlNotAFile: 'index.html 不是一个文件',
    paginationMutuallyExclusive: 'before_id 和 after_id 互斥',
  },
  toolsV2: {
    reading: '正在读取 {{path}}',
    writing: '正在写入 {{path}}',
    editing: '正在编辑 {{path}}',
    searching: '正在搜索 {{pattern}}',
    searchingFor: "正在 {{path}} 中搜索 '{{pattern}}'",
    globAborted: 'Glob 已中止',
    grepAborted: 'Grep 已中止',
    noMatches: '未找到匹配项',
    noNonSensitiveMatches: '未找到非敏感匹配项',
    grepTimedOut: 'Grep 在 {{seconds}}s 后超时。请尝试更具体的路径或模式。',
    grepTimedOutPartial: 'Grep 在 {{seconds}}s 后超时；返回部分结果',
    grepFailed: 'Grep 失败: {{reason}}',
    grepNoResults: '未找到匹配项。请尝试不同的模式或路径。',
    globTimedOut: 'Glob 在 {{seconds}}s 后超时；返回部分结果。',
    globNoSensitiveMatches: '未找到非敏感匹配项（已过滤 {{count}} 个敏感文件）。',
    globTruncated: '仅返回前 {{count}} 个匹配项。',
    sensitiveFilesFiltered: '已过滤 {{count}} 个敏感文件。',
    foundMatches: '找到 {{count}} 个匹配项',
    bufferTruncated: '[stdout 在 {{bytes}} 字节处截断；结果可能不完整 — 请使用更具体的模式]',
    grepBufferTruncated: '[stdout 在 {{bytes}} 字节处截断；不完整的尾部行已省略]',
    paginationTruncated:
      '结果已截断为 {{headLimit}} 行（共 {{total}} 项）。使用 offset={{nextOffset}} 查看更多。',
    writeAppended: '已追加 {{bytes}} 字节到 {{path}}',
    writeWrote: '已写入 {{bytes}} 字节到 {{path}}',
    writeFailedParentNotFound: '写入 {{path}} 失败：父目录不存在。',
    parentNotDirectory: '父路径不是目录: {{parent}}。',
    writeTargetIsDirectory: '无法写入 {{path}}：它是一个目录。',
    editNoChanges: '无需更改：old_string 和 new_string 完全相同。',
    editReplaced: '在 {{path}} 中替换了 {{count}} 处',
    occurrence: '处',
    occurrences: '处',
    webSearch: {
      searching: '正在搜索: {{preview}}',
      noResults: '未找到搜索结果。',
      cancelled: '搜索被取消: {{message}}',
      timedOut: '搜索超时: {{message}}',
      authFailed: '搜索失败（认证错误）: {{message}}',
      networkFailed: '搜索失败（网络错误）: {{message}}',
      failed: '搜索失败: {{message}}',
      citeReminder:
        '当你在回答中引用某个结果时，以内联 markdown 链接形式引用它，例如 [title](url)。',
    },
    fetchUrl: {
      fetching: '正在获取: {{preview}}',
      emptyBody: '响应体为空。',
      failedHttp: '获取 URL 失败。状态: {{status}}。{{message}}',
      networkError: '获取 URL 失败，网络错误: {{url}}。{{message}}',
      passthroughNote: '返回的内容是完整的响应体，按原样返回。',
      extractedNote: '返回的内容是从页面中提取的主要文本。',
      citeReminder:
        '如果你在回答中使用它，请以内联 markdown 链接形式引用此页面，例如 [title](url)。',
      invalidUrl: '无效的 URL: "{{url}}"',
      unsupportedScheme: '不支持的 URL scheme "{{scheme}}" — 仅允许 http(s)。',
      privateAddress: '拒绝获取私有地址: "{{host}}"',
      privateHost: '拒绝获取私有主机: "{{host}}"',
      privateResolved: '拒绝获取主机 "{{host}}": 解析到私有地址 "{{address}}"。',
      tooManyRedirects: '获取 "{{url}}" 时重定向次数过多（限制 {{limit}} 次）。',
      bodyTooLarge: '响应体过大: {{bytes}} 字节超出 maxBytes ({{maxBytes}})。',
      contentExtractionFailed: '无法从页面提取有意义的内容。该页面可能需要 JavaScript 才能渲染。',
    },
    memory: {
      memoryAction: 'Memory: {{action}}',
      unknownAction: '未知的 memory 操作: {{action}}',
      searchQueryRequired: '搜索需要 `query` 参数。',
      noEntriesFound: '未找到匹配 "{{query}}" 的 memory 条目',
      foundEntries: '找到 {{count}} 条 memory 条目:\n',
      readPathRequired: '读取需要 `path` 参数。',
      writePathRequired: '写入需要 `path` 参数（文件名）。',
      writeContentRequired: '写入需要 `content` 参数。',
      deletePathRequired: '删除需要 `path` 参数。',
      invalidFilename: '无效的 memory 文件名: "{{path}}"。请使用不含路径分隔符或 ".." 的纯文件名。',
      written: 'Memory 已写入 {{path}}\ntitle: {{title}}\ntype: {{type}}',
      notFound: '未找到 memory 文件: {{path}}',
      deleted: '已删除: {{path}}',
      noFilesFound: '未找到 memory 文件。',
      listHeader: 'Memory 文件 ({{count}}):\n',
    },
    bash: {
      startingBackground: '正在启动后台: {{preview}}',
      running: '正在运行: {{preview}}',
      sandboxBackendUnavailable:
        '已配置沙箱模式 "{{mode}}"，但当前主机没有可用的沙箱后端 —— 拒绝执行（fail closed）。设置 `[sandbox] mode = "off"` 可无沙箱运行。',
    },
    sandbox: {
      writeBlockedReadOnly:
        '沙箱模式 "{{mode}}" 禁止写入文件系统。设置 `[sandbox] mode = "off"`（或 "workspace-write"）以允许写入。',
      writeBlockedOutsideWorkspace:
        '沙箱模式 "{{mode}}" 禁止在工作区根目录 "{{workspace}}" 之外写入。目标路径 "{{path}}" 在其之外。',
      codeExecutionBlocked:
        '沙箱模式 "{{mode}}" 禁止代码执行（run_code）。设置 `[sandbox] mode = "off"` 以允许执行。',
    },
    spill: {
      retrievalHint: '完整输出已落盘到 {{path}} —— 使用 Read 工具读取以查看完整结果。',
    },
    sessionQuery: {
      mainAgentOnly: 'session_query 工具仅主 agent 可用。',
      unknownOperation: '未知的 session_query 操作。',
    },
    swarm: {
      launching: '正在启动 agent swarm: {{description}}',
      childDescription: '{{description}} #{{index}} ({{profileName}})',
      multipleDenied:
        'AgentSwarm 必须逐个调用。多次 AgentSwarm 调用并非禁止，但需要顺序执行：调用一个 AgentSwarm，等待其结果，然后再调用下一个；或者当单个 swarm 可以覆盖时，将工作合并到一个 AgentSwarm 中。',
      multipleDeniedMixed:
        'AgentSwarm 必须逐个调用。多次 AgentSwarm 调用并非禁止，但需要顺序执行：调用一个 AgentSwarm，等待其结果，然后再调用下一个；或者当单个 swarm 可以覆盖时，将工作合并到一个 AgentSwarm 中。AgentSwarm 也不得与同一响应中的其他工具组合使用。',
      mixedDenied:
        'AgentSwarm 必须是模型响应中唯一的工具调用。请单独重试一个 AgentSwarm 调用，然后在它返回后再调用其他工具。',
      agentDeniedInSwarmMode:
        'Agent 工具在 swarm 模式下不可用。请改用 AgentSwarm 并行分发子智能体。如果只需要单个子智能体，请使用包含一个 item 或一个 resume_agent_ids 条目的 AgentSwarm。',
      solitaryMultipleDenied:
        'AgentSwarm/Team 必须逐个调用。多次调用并非禁止，但需要顺序执行：调用一个，等待其结果，然后再调用下一个；或者当单次调用可以覆盖时，将工作合并。',
      solitaryMultipleDeniedMixed:
        'AgentSwarm/Team 必须逐个调用。多次调用并非禁止，但需要顺序执行：调用一个，等待其结果，然后再调用下一个；或者当单次调用可以覆盖时，将工作合并。这些工具也不得与同一响应中的其他工具组合使用。',
      solitaryMixedDenied:
        'AgentSwarm/Team 必须是模型响应中唯一的工具调用。请单独重试一次调用，然后在它返回后再调用其他工具。',
    },
    team: {
      launching: '团队讨论：{{topic}}',
      launchingDebate: '团队辩论：{{topic}}',
      agentName: '团队 {{mode}}（{{count}} 名参与者）',
    },
    plugin: {
      cannotInstallLocal: '无法安装 {{path}} 处的插件: {{message}}',
      cannotInstallRemote: '无法从 {{source}} 安装插件: {{message}}',
      noMcpServer: '插件 "{{id}}" 未声明 MCP server "{{server}}"',
      notFound: '插件 "{{id}}" 未安装',
      rootNotAbsolute: '插件根目录必须是绝对路径（实际为 "{{path}}"）',
      rootNotExist: '插件根目录不存在: {{path}}',
      rootNotDirectory: '插件根目录不是目录: {{path}}',
      noGithubMeta: '插件 "{{id}}" 没有 GitHub 元数据',
    },
    oldStringNotFound:
      '在 {{path}} 中未找到 old_string，文件内容可能已过时。请使用 Read 工具重新加载内容。',
    oldStringNotUnique:
      'old_string 在 {{path}} 中不唯一（找到 {{count}} 处）。要替换所有出现，请设置 replace_all=true。要仅替换一处，请在 old_string 中包含更多上下文。',
    planMode: {
      alreadyActive: '计划模式已激活。准备好计划后使用 ExitPlanMode。',
      enterFailed: '进入计划模式失败。',
      enterFailedDetail: '进入计划模式失败: {{message}}',
      exitOnlyInPlanMode:
        'ExitPlanMode 只能在计划模式激活时调用。请先使用 EnterPlanMode（或 /plan）。',
      exitFailed: '退出计划模式失败。',
      exitFailedDetail: '退出计划模式失败: {{message}}',
      readFailed: '读取计划文件失败。',
      readFailedDetail: '读取计划文件失败: {{message}}',
      noPlanFile: '未找到计划文件。请先将计划写入当前计划文件，然后调用 ExitPlanMode。',
      noPlanFileDetail: '未找到计划文件。请先将计划写入 {{path}}，然后调用 ExitPlanMode。',
      selectedApproach: '选定方案: {{label}}\n只执行选定的方案。不要从其他方案中挑选个别任务。',
      planSaved: '计划已保存到: {{path}}\n\n',
      planApproved: '计划模式已停用。所有工具现已可用。\n{{savedTo}}## 已批准的计划:\n{{plan}}',
      planAutoApproved:
        '计划模式已停用。所有工具现已可用。\n注意：此计划已自动批准，未经用户审阅——用户尚未明确批准。请遵循用户的原始指示决定是否继续执行；如果用户要求停止、等待或仅在计划后总结，请不要开始执行。\n{{savedTo}}## 计划（自动批准，未经用户审阅）:\n',
      exitedPlanMode: '已退出计划模式。{{prefix}}{{plan}}',
      approvalDismissed: '计划审批已取消。计划模式保持激活。',
      rejected: '计划已被用户拒绝。计划模式已停用。',
      rejectedWithFeedback: '用户拒绝了计划。反馈:\n\n{{feedback}}',
      revisionsRequested: '用户要求修改。计划模式保持激活。',
      rejectedStaysActive: '计划已被用户拒绝。计划模式保持激活。',
    },
    goal: {
      invalidStatus: '无效的目标状态。请使用 `active`、`complete` 或 `blocked`。',
      notCompleted: '目标未完成：没有活跃的目标。',
      notBlocked: '目标未阻塞：没有活跃的目标。',
      notResumed: '目标未恢复：没有当前目标。',
      resumed: '目标已恢复。',
      budgetNotSet: '目标预算未设置：没有当前目标。',
      budgetStale: '目标预算未设置：当前目标已变更。',
      notCreatedStale: '目标未创建：当前目标已变更。',
      goalChanged: '目标未更新：当前目标已变更。',
      creating: '正在创建目标',
    },
    selectTools: {
      notAvailable: '当前模型不支持 select_tools。',
      loaded: '已加载: {{tools}}',
      alreadyAvailable: '已可用: {{tools}}',
      unknown: '未知的工具: {{name}}。请从最新的可用工具列表中选择。',
    },
    glob: {
      rootNotDirectory: '{{root}} 不是目录',
      rootNotExist: '{{root}} 不存在',
    },
    task: {
      notFound: '未找到任务: {{taskId}}',
      outputTruncatedHint:
        '上面仅显示最后 {{bytes}} 个字节。使用 Read 工具配合 output_path 查看完整内容。',
      outputCompleteHint: '上面的预览是完整输出。使用 Read 工具配合 output_path 查看完整内容。',
      stillRunningHint: '任务在等待后仍在运行。请勿再次阻塞——结果会自动到达。',
      truncatedFull: '[已截断。完整输出: {{path}}]',
      truncatedNoLog: '[已截断。此任务没有持久化的完整日志。]',
      noOutput: '[无可用输出]',
      snapshotRetrieved: '已获取任务快照。',
      stoppedByUser: '已通过 TaskStop 停止',
      alreadyTerminal: '任务已处于终止状态',
      stopFailed: '停止任务 {{taskId}} 失败',
    },
    agent: {
      promptEmpty: 'prompt 不能为空或仅含空白字符',
      cannotSetSubagentTypeOnResume:
        '恢复已存在的 agent 时不能设置 subagent_type。仅通过 agent id 恢复。',
      stoppedBeforeFinish: '子 agent 在完成前被停止。',
      statusRunning: 'status: running',
      autoNotification: 'automatic_notification: true',
      completionNotice:
        'next_step: 完成结果会在后续轮次自动到达——不要等待、轮询或调用 TaskOutput；继续当前工作。',
      resumeHint:
        'resume_hint: 要继续或恢复此子 agent，调用 Agent(resume="{{agentId}}", prompt="从上次中断的地方继续；如果上次工具调用的结果从未被观察到，则重新执行。")。',
      statusCompleted: 'status: completed',
      statusFailed: 'status: failed',
      unknownError: '未知错误',
    },
    read: {
      notExist: '"{{path}}" 不存在。',
      notFile: '"{{path}}" 不是文件。',
      pathEmpty: '文件路径不能为空。',
    },
    readMedia: {
      pathEmpty: '文件路径不能为空。',
      isTextFile: '"{{path}}" 是文本文件。请使用 Read 读取文本文件。',
      notSupported:
        '"{{path}}" 不是支持的图片或视频文件。文本文件请使用 Read，其他二进制格式请使用 Bash 或 MCP 工具。',
      noImageSupport: '当前模型不支持图片输入。请告知用户使用支持图片输入的模型。',
      noVideoSupport: '当前模型不支持视频输入。请告知用户使用支持视频输入的模型。',
      fileEmpty: '"{{path}}" 为空。',
      fileTooLarge: '"{{path}}" 的大小为 {{size}} 字节，超过了媒体文件的最大 {{max}}MB。',
      regionOnlyForImages: 'region 和 full_resolution 仅适用于图片文件。',
      cannotReadRegion: '无法从 "{{path}}" 读取区域: {{error}}',
    },
    abort: {
      aborted: '已中止',
      abortedByUser: '已被用户中止',
      operationAborted: '此操作已被中止',
      beforeSearch: '搜索开始前已中止',
    },
    editReplacedOne: '在 {{path}} 中替换了 1 处',
    pathNotFile: '{{path}} 不是文件。',
    cron: {
      matchesNone: 'cron {{name}} 字段没有匹配任何值',
    },
    execution: {
      resolveFailed: '工具 "{{toolName}}" 解析执行失败: {{error}}',
      blocked: '工具调用 "{{toolName}}" 被阻止',
      skipped: '工具已跳过，因为之前的工具调用停止了当前轮次。',
      failed: '工具 "{{toolName}}" 失败: {{error}}',
      abortedHook: '工具 "{{toolName}}" 在 onDidExecuteTool hook 期间中止。',
      hookFailed: '工具 "{{toolName}}" 的 onDidExecuteTool hook 失败: {{error}}',
      notFound: '未找到工具 "{{toolName}}"',
      invalidArgs: '工具 "{{toolName}}" 的参数无效: {{error}}',
      noResult: '工具 "{{toolName}}" 未返回结果。',
      wrongType: '工具 "{{toolName}}" 返回了 {{type}} 而非工具结果。',
      malformedOutput: '工具 "{{toolName}}" 返回了缺少或格式错误的 "output" 字段的结果。',
      userInterrupted:
        '用户手动中断了 "{{toolName}}"（以及同时运行的所有其他操作）。这是用户的主动操作，不是系统错误、超时或容量限制。不要自动重试或猜测原因 — 等待用户的下一条指令。',
      aborted: '工具 "{{toolName}}" 已中止',
      emptyOutput: '工具输出为空。',
      nonTextOutput: '工具返回了非文本内容。',
    },
  },
};

export default zh;
