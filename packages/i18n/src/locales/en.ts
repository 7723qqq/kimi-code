export const en = {
  ...
  plugin: {
    ...
    dirFieldMustBeStringOrArray: '"{{field}}" must be a string or string[]',
    dirFieldPathDotSlash: '"{{field}}" path must start with "./" (got "{{entry}}")',
    dirFieldPathOutside: '"{{field}}" path resolves outside the plugin ({{entry}})',
    dirFieldPathNotDir: '"{{field}}" path is not a directory ({{entry}})',
  ...
  v2Errors: {
    ...
    pathRejectedDotdot: 'path "{{path}}" rejected (dotdot segment)',
    pathEscapesWorkspace: 'path "{{path}}" escapes workspace',
    permissionPatternEmpty: 'permission pattern: empty string',
    permissionPatternMissingParen: 'permission pattern: missing closing paren in "{{pattern}}"',
    permissionPatternEmptyTool: 'permission pattern: empty tool name in "{{pattern}}"',
    unknownSession: "unknown session '{{sessionId}}'",
    unknownAgent: "unknown agent '{{agentId}}'",
    agentDoesNotExist: 'Agent "{{agentId}}" does not exist',
    agentTurnCouldNotStart: 'Agent turn could not be started',
    oauthTokenProviderNotConfigured: 'OAuth token provider is not configured.',
    cronExpressionMustBeString: 'cron expression must be a string',
    cronExpressionEmpty: 'cron expression is empty',
    cronStepEmpty: 'cron {{name}} step is empty in "{{term}}"',
    bashToolNotRegistered: 'Bash tool is not registered.',
    mcpSseClientClosed: 'MCP SSE client is closed',
    mcpSseClientClosedDuringStartup: 'MCP SSE client was closed during startup',
    mcpOAuthStoreKeyInvalid: 'Invalid MCP OAuth store key: "{{name}}"',
    mcpOAuthFlowAlreadyDone: 'OAuth flow already completed or cancelled',
    mcpStdioCommandEmpty: 'MCP stdio command must not be empty',
    mcpStdioClientClosed: 'MCP stdio client is closed',
    mcpStdioClientClosedDuringStartup: 'MCP stdio client was closed during startup',
    mcpHttpClientClosed: 'MCP HTTP client is closed',
    mcpHttpClientClosedDuringStartup: 'MCP HTTP client was closed during startup',
    sourceAgentDoesNotExist: 'Source agent "{{sourceAgentId}}" does not exist',
    agentAlreadyExists: 'Agent "{{agentId}}" already exists',
    promptIdsEmpty: 'prompt_ids must not be empty',
    noActivePrompt: 'no active prompt to steer into',
  ...
  toolsV2: {
    ...
    sandbox: {
      writeBlockedReadOnly:
        'Sandbox mode "{{mode}}" blocks writes to the filesystem. Set `[sandbox] mode = "off"` (or "workspace-write") to allow writes.',
      writeBlockedOutsideWorkspace:
        'Sandbox mode "{{mode}}" blocks writes outside the workspace root "{{workspace}}". Target path "{{path}}" is outside it.',
    ...
    planMode: {
      ...
      rejectedWithFeedback: 'User rejected the plan. Feedback:\n\n{{feedback}}',
      revisionsRequested: 'User requested revisions. Plan mode remains active.',
      rejectedStaysActive: 'Plan rejected by user. Plan mode remains active.',
    },
    goal: {
      invalidStatus: 'Invalid goal status. Use `active`, `complete`, or `blocked`.',
      notCompleted: 'Goal not completed: no active goal.',
      notBlocked: 'Goal not blocked: no active goal.',
      notResumed: 'Goal not resumed: no current goal.',
      resumed: 'Goal resumed.',
      budgetNotSet: 'Goal budget not set: no current goal.',
      budgetStale: 'Goal budget not set: the current goal changed.',
      notCreatedStale: 'Goal not created: the current goal changed.',
      goalChanged: 'Goal not updated: the current goal changed.',
      creating: 'Creating a goal',
    },
    selectTools: {
      notAvailable: 'select_tools is not available for the current model.',
      loaded: 'Loaded: {{tools}}',
      alreadyAvailable: 'Already available: {{tools}}',
      unknown: 'Unknown tool: {{name}}. Pick from the latest announced tools list.',
    },
    glob: {
      rootNotDirectory: '{{root}} is not a directory',
      rootNotExist: '{{root}} does not exist',
    },
    task: {
      ...
      snapshotRetrieved: 'Task snapshot retrieved.',
      stoppedByUser: 'Stopped by TaskStop',
      alreadyTerminal: 'Task already in terminal state',
      stopFailed: 'Failed to stop task: {{taskId}}',
    },
    agent: {
    ...
    abort: {
      ...
      beforeSearch: 'Aborted before search started',
    },
    editReplacedOne: 'Replaced 1 occurrence in {{path}}',
