...
import { encodeWorkDirKey } from '../v1-compat.js';

/**
 * Bucket directory name `wd_<slug>_<hash12>` for a workdir path.
 *
 * Aliases the local v1-compat copy of agent-core's `encodeWorkDirKey` so the
 * migrator and the running app always produce byte-identical buckets. The
 * session picker locates sessions purely by
 ...
 * never reads. Delegating to the shared copy removes that drift for good.
 */
