...
export class TuiAltScreen extends TuiBase implements ViewportTUI {
	...
	protected override afterTerminalStop(options: TuiStopOptions): void {
		...
		if (options.preserveScreen) {
			...
			// Exit the alternate screen before replaying the document: if a
			// component's render throws, the exit sequence must still reach the
			// terminal or the session would stay stuck on the alt buffer.
			this.terminal.write(`${BEGIN_SYNCHRONIZED_OUTPUT}${EXIT_ALT_SCREEN}${DISABLE_AUTOWRAP}`);
	...
	private handleViewportInput(data: string): { consume?: boolean } | undefined {
		...
		if (primaryScrollable && keybindings.matches(data, "tui.altScreen.pageDown")) {
			if (!isRelease) {
				this.scrollBy(Math.max(1, this.getPrimaryScrollView().viewportHeight - PAGE_SCROLL_OVERLAP));
			}
			return { consume: true };
