...
- **测试**：`cargo test -p kimi-agent` = 2011 lib 全绿、0 warnings（2026-08-01 实测）；包内 vitest 38/38（含 stdio e2e）；含全部 crate 的全工作区口径约 **2,700+**（kimi-agent 2011 + 集成 51 + native-tools 617 + kimi-shared 47，GAP_ANALYSIS 2026-08-01 快照；kosong/native 已于 2026-08-03 退役，其 89 测试随包删除）
...
- **待办**：上游 0.31.1+ 同步——A 表 25 项已全部定案（A16/A17/A19 已迁移，A20/A24 已核对为 ⚪，见 `上游更新/迁移计划.md`）；**C3 插件通知已核对（2026-08-03：引擎侧 ✅ 无缺口——数据面 RPC 已完备，功能本体纯 TUI 宿主按 C 表约定跳过，见 `上游更新/迁移计划.md`）**；A8/A23 ⚪ 宿主面；kap-server Rust 化 **阶段 4 已完成**（2026-08-02 复核：src+test 零 v2 import、protocol 19/19 本地化、12 路由去 core）；**v2 harness 已删除**（2026-08-02：apps/kimi-code 的 `cli/v2/`、`cli/experimental-v2.ts`、`--agent/--agent-file` 选项及测试全部移除）；**rust-backed 引擎接入层已收进 SDK**（2026-08-02：`@moonshot-ai/kimi-code-sdk/rust` 新入口——SessionEventTranslator/SessionEngineController/wire 翻译层/RustRpcClient（rust-loop 驱动 CoreAPI，80 方法面与基类对齐，缺口 nativeUnavailable，6 单测绿））；**createKimiHarness 已切换 RustRpcClient 并删除 KimiCore/SDKRpcClient**（2026-08-02：sdk-rpc-client.ts 重写为 Rust-backed 装配；auth.ts/kimi-harness.ts 解绑 agent-core（legacy config/errors + 本地 ImageLimits/withTelemetryContext）；vitest alias 补 subpath 映射；SDK typecheck 绿 + 12 关键测试绿）；**① SDK 集成测试迁移已完成（2026-08-03：node-sdk vitest 34 文件全绿 425 通过 0 失败，见 event-contract 阶段 6）**——已实现：合成事件（config.update/turn.steer/permission.set_mode/session.closed/renamed）、WireEventWriter、configGet/Set（re-read 合并、默认值、KIMI_CONFIG_PATH 对齐）、telemetry withContext、setKimiConfig payload 修正、删除半成品 legacy/plugin/；2026-08-02 再推进：vitest 强制 `KIMI_AGENT_FORCE_STDIO=1`（否则 napi 单例锁定后 session/* 全返 null，+30）、移植 GlobalMcpConfigStore（legacy/global-mcp-config，harness MCP 读写宿主 mcp.json，+4）、createSession workDir 校验、引擎新增 `session/delete` RPC（deleteSession 接通，missing id 抛 session.not_found）；**71 失败已清零（2026-08-03 实测：34 文件 425 通过 0 失败，另有 1 skipped/1 todo）**——三项根因全部关闭：config 兼容经 SDK lenient 读落地（getKimiConfig lenient 读 + getConfigDiagnostics warnings，rpc-client.ts L1016，替代原定 serde alias 专项）；RPC 缺口补齐（session/delete、session/fork、session/export 均在 rpc/types.rs；全局 MCP 由 legacy/global-mcp-config 本地化覆盖）；wire 事件流语义随 event-contract 专项收口（`RUST_MIGRATION_PLAN.event-contract.md` 阶段 1-6 + 收尾）；**② 宿主工具已本地化**（2026-08-02：image compression（legacy/image，178 测试绿）+ loadMcpServers（legacy/mcp-config-loader）+ PluginManager 读面（legacy/plugin，install/remove nativeUnavailable，mcp/plugin 24 测试绿）——index.ts 不再从 agent-core 转发这些）；**③ profile 已本地化**（legacy/profile：context/load/resolve/default + 手写模板渲染器，js-yaml 依赖，12+1 测试绿）；**④ SDK 49 个 v1 wire 公开类型切 Rust wire 形状 + CLI/TUI 联动（用户已确认彻底路线）**；⑤ klient Rust 传输已完成（2026-08-03：rust 传输替换 v2 dispatcher，agent-core-v2 已随 ⑤ 移入 retired/，packages/* 无 TS 引擎包）；⑤ agent-core 已物理移入 retired/（2026-08-03，宿主零引用、kimi-code typecheck 43→0；agent-core-v2 待 klient Rust 传输后随 ⑤ 迁移）；⑥ 发布规划：kimi-agent（private）需随 SDK 发布或 vendor rust-loop；**⑦ 事件契约向引擎靠拢专项（2026-08-02 启动，详见 `RUST_MIGRATION_PLAN.event-contract.md`）**——协议向引擎靠：重写 protocol 会话域 AgentEvent 为引擎 snake_case 形状、删除 node-sdk `event-translate.ts` 翻译层、CLI/ACP/kap-server 消费适配、事件测试重写，6 阶段
