...
Agent 通过 `WaitFor` 等待后台任务时，按 `Ctrl-S` 会提前结束这次等待。后台任务继续运行，已有的工具结果不会丢失。如果同一批还有其他前台工具，Agent 会在它们返回后再处理你的消息。

后台任务属于启动它的那个会话，退出 Kimi Code 后不会继续运行。在下一个会话中，Agent 会被告知上一个会话的任务已失去联系，因此它不会假定这些任务已完成——它会核对记录中的状态，并可选择从先前的上下文恢复某个子代理。

如果轮次在 steer 消息并入之前被中断，该消息不会丢失——下一个轮次会带上它。
