...
- 支持通过 kimi.ai 和 kimi.com 两种 OAuth 方式登录。
- 新增 WaitFor 工具：Agent 可在同一轮内等待后台任务完成，无需结束本轮后被重新调用。WaitFor 工具调用会以静默行展示已完成任务、超时信息和仍在运行的数量。
- 官方 Kimi Datasource 插件新增 13 个数据源——中国政府数据（NDA/NBS）与标准（GB/HB/DB/TT）、八个国际组织数据集（WHO、FAO、UNSD、ECB、Eurostat、UNICEF、OECD、FRED）、新华财经与财新。在 `/plugins` 的 Official 标签页更新插件。
- web: 聊天标题更多菜单新增 Pin 操作，可将会话固定到侧边栏的固定区；拖动固定区与会话列表之间的分隔条可调整两侧大小。
- web: 长输出的 `!` Shell 命令结果默认折叠，不再刷屏；按 `Ctrl-O` 可与工具输出一起展开或折叠。
- web: skill 激活轮次支持撤销，可撤回后重新发送。

### 优化

- 子 Agent 默认不再派生子 Agent，自定义 agent 配置可显式开启。
- web: 优化 web 菜单与面板——菜单项与边框保持同心圆角；菜单外的按钮悬停提示自动隐藏；账户菜单行高与间距收紧；修正侧边栏分组头与会话行之间操作按钮对齐；模型选择器、斜杠命令与 `@` 提及面板保持在视口内。
- web: 升级 `@` 提及菜单——文件与 skill 候选合并并按匹配度排序，文件搜索更快，支持路径片段匹配与命中高亮。
- web: 优化提示队列交互，每条可单独引导与发送。
- web: 选择登录区域后自动打开浏览器授权页；重新设计授权等待页；窗口重新获得焦点时立即刷新登录状态，不再依赖轮询。

### 修复

- 修复 `config.toml` 在文件有语法错误或在应用外编辑后丢失配置项的问题。
- Edit 和 Write 工具现在要求先读取现有文件，且若文件在上次读取后在磁盘上发生变化则拒绝写入。
- 被供应商内容过滤器拦截的请求不再重试，过滤提示立即显示。
- 修复部分 OpenAI 兼容供应商在会话含工具调用时返回 422 错误的问题。
- 修复需要登录的远程 MCP 服务器缺少 OAuth authenticate 工具的问题。
- 修复后台 Agent 行刚出现时无法停止，以及 Agent 启动失败留下残留行的问题。
- 修复模型在遇到图片或二进制文件时被引导至不可用工具的问题。
