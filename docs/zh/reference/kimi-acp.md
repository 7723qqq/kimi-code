...
`kimi acp` 实现了以下 ACP 方法（其它方法返回 `methodNotFound`）：

- `initialize` — 协议版本协商 + 能力声明。
- `session/new` — 创建新会话；接受 `cwd`、`mcpServers` 等字段。
- `session/load` — 续接磁盘上的会话；会将历史用 `session/update` 同步重放给客户端。
- `session/list` — 列出当前所有会话摘要。
- `session/prompt` — 提交一次用户输入。`text`/`image`/`resource`/`resource_link` 都支持，输出按 `agent_message_chunk` 流式推送。
- `session/cancel` — 通知打断当前生成。
- `session/set_mode` — 切换会话权限模式（兼容路径，等价于 `set_config_option({ configId: 'mode', value })`）。
- `session/set_model`（unstable）— 切换会话模型别名（兼容路径，等价于 `set_config_option({ configId: 'model', value })`）。
- `session/set_config_option` — Phase 14 引入的通用 picker dispatch；推荐客户端使用这个路径切换 mode/model（详见下一节）。
