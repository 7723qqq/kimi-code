...
对于观察型事件，即使脚本报错、超时，CLI 也**不会因此中断你的工作**——这种"出错就放行"的设计叫 fail-open（失败开放），避免 hook 异常变成绊脚石。唯一的例外是权限门控事件 `PreToolUse` 和 `Stop`：它们 fail-closed（失败关闭），脚本出错或超时会**阻断**操作，而不是静默放行。

::: warning 注意
正因为大多数事件 fail-open，Hooks 适合做提醒和轻量拦截，但**不应作为唯一的安全防线**。只有权限门控事件（`PreToolUse`、`Stop`）是 fail-closed 的，其余事件出错时都默认放行。对真正高风险的操作，仍需依赖权限审批和人工确认。
:::
...
例外是权限门控事件 `PreToolUse` 和 `Stop`：它们 fail-closed，脚本出错或超时会**阻断**操作而不是放行——坏掉的 hook 绝不能悄悄削弱门禁。
