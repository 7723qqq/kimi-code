...
| `cwd` | `string` | 否 | 命令的工作目录；默认是当前会话的项目目录 |
| `env` | `table` | 否 | 传给命令的额外环境变量，会覆盖继承来的同名变量 |

`[[hooks]]` 只允许这六个字段，多写会导致配置文件加载失败。

**同一事件匹配多条规则时**，所有命中的 hook 并行运行；同一工作目录下 `command` 完全相同的多条规则只运行一次。

Hook 命令的工作目录是当前会话的项目目录，规则里写了 `cwd` 则以它为准。非 Windows 平台上，hook 进程放在独立进程组里，超时时先发信号让它有机会善后，之后才强制终止。
