...
| `Team` | Auto-allow in swarm mode; otherwise requires approval | Assemble multiple agents for a roundtable discussion or structured debate |
...
**`Team`** assembles multiple AI agents for a roundtable discussion (`mode` `"discussion"`, the default) or a structured debate (`mode` `"debate"`), for multi-perspective analysis, cross-review, and consensus. Pass a `topic` and a `participants` array — each entry takes `profileName` (agent profile, default `coder`), `roleDescription`, and optionally `assignedStance` (debate only). `maxRounds` controls the number of rounds; `summaryPrompt` optionally generates a summary or consensus report; `enableVoting` adds a voting phase to debates. Each participant sees the full transcript before their turn, and results include the full transcript, summary, and token usage. Like `AgentSwarm`, `Team` is a solitary tool — it must be the only tool call in a model response. In `manual` permission mode, calling `Team` outside swarm mode triggers an approval prompt; in swarm mode it is auto-approved. You can also launch a discussion directly with the `/team <topic> with <role1>,<role2>` command.
