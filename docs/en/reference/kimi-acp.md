...
`kimi acp` implements the following ACP methods (others return `methodNotFound`):

- `initialize` — protocol version negotiation and capability advertising.
- `session/new` — start a new session; accepts `cwd`, `mcpServers`, etc.
- `session/load` — resume an on-disk session; replays history to the client via `session/update`.
- `session/list` — enumerate session summaries.
- `session/prompt` — submit a user turn. `text`, `image`, `resource`, and `resource_link` content blocks are accepted; output streams as `agent_message_chunk` updates.
- `session/cancel` — interrupt the current turn.
- `session/set_mode` — change the session mode (compatibility path; equivalent to `set_config_option({ configId: 'mode', value })`).
- `session/set_model` (unstable) — change the session model alias (compatibility path; equivalent to `set_config_option({ configId: 'model', value })`).
- `session/set_config_option` — Phase 14's generic picker dispatch; the recommended path for clients to flip mode/model (details in the next section).
