...
| `promptCapabilities.embeddedContext` | `true` | Client may send `resource`/`resource_link` embedded resource blocks; text content is injected into the prompt as `<resource uri="...">...</resource>`; blob resources are dropped |
...
| `auth.logout` | `{}` | Supports ACP `logout`; the standalone server holds no managed token to drop, so it is accepted as a no-op |
...
**Summary: the ACP server implements the full core (3/3) and session (11/11) agent-side surface and 10/11 client reverse-RPC methods. Not implemented: `providers/*`, `nes/*`, `document/*`, `session/set_model`, and `elicitation/complete` — requests for them return `methodNotFound`.**

### Core agent-side — IDE → agent (3 / 3)

| Method | Implemented | Description |
| --- | --- | --- |
| `initialize` | Yes | Version negotiation; returns `agentInfo: { name: 'Kimi Code CLI', version }`, capability matrix, and `authMethods` (a first-class `type:'terminal'` login) |
| `authenticate` | Yes | Validates `methodId='login'`; returns `authRequired (-32000)` if the token is missing, `invalidParams (-32602)` for an unknown ID |
| `logout` | Yes | Accepted as a no-op: the standalone engine holds no managed token, so it does not change the auth gate |

### Session agent-side — IDE → agent (11 / 11)

| Method | Implemented | Description |
| --- | --- | --- |
| `session/new` | Yes | Accepts `cwd` / `mcpServers`; returns `sessionId` + `configOptions[]` + `modes` |
...
| `session/fork` | Yes | Branches a source session; `cwd` / `mcpServers` on the request are ignored |
| `session/close` | Yes | Best-effort teardown: cancels any in-flight turn, disposes per-session resources, and closes the live session; an unknown id returns `invalidParams (-32602)` |
...
| `session/set_config_option` | Yes | Mode picker (`configId: 'mode'`); model/thinking switching is not supported |

### Client-side reverse-RPC — agent → IDE (10 / 11)

| Method | Implemented | Description |
| --- | --- | --- |
| `session/update` | Yes | Streams `agent_message_chunk` / `agent_thought_chunk` / `tool_call` / `tool_call_update` while a turn runs, plus `current_mode_update` on a mode switch and `user_message_chunk` / `tool_call_update` during `session/load` replay |
...
The ACP 0.23 `session/set_model` extension is **not** implemented by the native server: the engine advertises a single model row and has no runtime model catalog, so `session/set_model` and the `model` arm of `session/set_config_option` are unavailable and return `methodNotFound` / `invalidParams`.
