...
`image` controls how images are compressed before being sent to the model, across every ingestion point (pasted images, `Read` reads, images in MCP tool results, and so on).
