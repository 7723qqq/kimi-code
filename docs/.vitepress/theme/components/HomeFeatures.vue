...
const highlightsLede = computed(() =>
  isZh.value ? '装好就能用，关键能力默认就绪。' : 'Install once. The essentials are already there.',
);

const featuresTitle = computed(() => (isZh.value ? '按需扩展' : 'Extend it your way'));
const featuresLede = computed(() =>
  isZh.value
...
<template>
  <section class="KimiHome__section KimiHighlights">
    ...
    <div class="KimiHighlights__grid">
      <div v-for="h in highlights" :key="h.title" class="KimiHighlights__card">
  ...
  <section class="KimiHome__section KimiFeatures">
    ...
    <div class="KimiFeatures__grid">
      <a v-for="f in features" :key="f.title" class="KimiFeatures__card" :href="withBase(f.href)">
